"""
Apply Phase 3: Multi-Label Clustering (MORE CLUSTERS + MULTI-LABEL)

Improvements over v2:
1. Increase number of clusters (7 -> 12-15 clusters)
2. Multi-label assignment: 1 candidate can belong to multiple clusters
3. Use probability threshold for cluster membership
"""
...
print("="*70)
print("PHASE 3: MULTI-LABEL CLUSTERING (MORE CLUSTERS)")
print("="*70)
...
print("\n3. Re-running clustering with MORE clusters...")

# HDBSCAN with smaller min_cluster_size for more clusters
hdbscan_clusterer = hdbscan.HDBSCAN(
    min_cluster_size=3,  # Reduced from 5 to 3
...
for k in range(10, 20):  # Increased from (5, 15) to (10, 20)
...
# Multi-label assignment using GMM probabilities
print("\n4. Computing multi-label cluster assignments...")
gmm_probs = best_gmm.predict_proba(features_umap)

# Strategy: Assign to top K clusters that meet minimum threshold
MIN_PROBABILITY_THRESHOLD = 0.05  # Minimum 5% probability
TOP_K_CLUSTERS = 3  # Consider top 3 clusters

candidate_clusters = []

for i in range(len(candidates)):
    # Get all cluster probabilities
    cluster_probs = gmm_probs[i]

    # Get all clusters with their probabilities
    all_clusters = []
    for cluster_id in range(best_k):
        all_clusters.append({
            'cluster_id': cluster_id,
            'probability': float(cluster_probs[cluster_id])
        })

    # Sort by probability
    all_clusters.sort(key=lambda x: x['probability'], reverse=True)

    # Assign to top K clusters that meet minimum threshold
    assigned_clusters = []
    for cluster_info in all_clusters[:TOP_K_CLUSTERS]:
        if cluster_info['probability'] >= MIN_PROBABILITY_THRESHOLD:
            assigned_clusters.append(cluster_info)

    # Ensure at least 1 cluster (the primary one)
    if not assigned_clusters and all_clusters:
        assigned_clusters = [all_clusters[0]]

    candidate_clusters.append({
        'candidate_id': candidates[i]['candidate_id'],
        'clusters': assigned_clusters,
        'primary_cluster': assigned_clusters[0]['cluster_id'] if assigned_clusters else -1,
...
for cluster_id in range(n_clusters_final):
    # Get members (candidates with this cluster as primary OR secondary)
    ...
    for skill, stats in sorted_technical:
        if stats['avg_score'] >= 70 and stats['frequency_pct'] >= 0.20:  # Lowered from 0.25
            strengths.append({
                'skill': skill,
...
output = {
    ...
    'clustering_method': 'Multi-label GMM (top-K probability-based)',
    ...
    'improvements': [
        'Increased clusters from 7 to 10-15',
        'Multi-label: 1 candidate can belong to top 3 clusters',
        'Minimum probability threshold: 5%',
...
# Summary
print("\n" + "="*70)
print("PHASE 3 COMPLETE - MULTI-LABEL CLUSTERING")
print("="*70)
...
for cluster in clusters_output:
    ...
    print(f"  {cluster['cluster_id']:15} | {size_info:40} | {cluster['cluster_label']}{weaknesses_str}")
