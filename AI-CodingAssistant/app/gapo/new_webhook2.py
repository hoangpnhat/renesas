...
async def handle_webhook(event: Dict):
    ...
    if msg_type == "quick_reply":
        ...
        await asyncio.to_thread(survey.update_feedback, thread_id, feedback, feedback_id)
    ...
    if event_type == 'message_created':
        ...
        if isinstance(message, (SubThreadMessage, DirectMessage)):
            if isinstance(message, SubThreadMessage):
                raw_parent_msg = await asyncio.to_thread(message_getter.get_parent_message, message.parent_thread_id, message.parent_message_id)
            ...
            raw_history = await asyncio.to_thread(message_getter.get_messages, message.thread_id)
        ...
        answer, json_object = await asyncio.to_thread(generate_answer, message, chat_history, chat_history_wt_image)
        mentions = []
        ...
        if isinstance(message, DirectMessage):
            ...
            response = await asyncio.to_thread(message_sender.send_text_message_to_user, int(receiver_id), int(bot_id), answer)
            session_id = str(response.get('thread_id', None) or session_id)
        else:
            parent_thread_id = message.parent_thread_id
            parent_message_id = message.parent_message_id
            response = await asyncio.to_thread(
                message_sender.send_text_message_to_subthread,
