...
def extract_code_elements(file_content):
    ...
    headers = [match.start() for match in re.finditer(header_pattern, file_content, re.MULTILINE)]
...
def chunk_function(c_code):
    ...
    function_signature_regex = r"^\s*(void|int|char|float|double|long|short|bool)\s+\w+\s*\(.*\)\s*\{"
    ...
    for line in lines:
        ...
        elif any(stripped_line.startswith(statement) for statement in control_flow_statements):
            current_chunk.append(line)
...
def main():
    ...
    parser.add_argument("--folder_path", type=str, default=r'data/RC04EX/Code/RC04EX_Io_Versa')
    parser.add_argument("--output_path", type=str, default=r'data/code_elements.json')
