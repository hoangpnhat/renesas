...
from app.database.schemas import LabelledNode, EntityNode
...
class CustomNeo4jPropertyGraphStore(PropertyGraphStore):
    r"""
    Custom Neo4j Property Graph Store.
    ...
    def get(
        ...
        WITH e
        RETURN e.id AS name,
               [l in labels(e) WHERE l <> '__Entity__' | l] AS type,
    ...
    def get_triplets(
        ...
        CALL {{
            ...
            RETURN e.name AS source_id, [l in labels(e) WHERE l <> '__Entity__' | l] AS source_type,
                   e{{.* , embedding: Null, name: Null}} AS source_properties,
                   type(r) AS type,
                   t.name AS target_id, [l in labels(t) WHERE l <> '__Entity__' | l] AS target_type,
                   t{{.* , embedding: Null, name: Null}} AS target_properties
            UNION ALL
            WITH e
            MATCH (e)<-[r{':`' + '`|`'.join(relation_names) + '`' if relation_names else ''}]-(t)
            RETURN t.name AS source_id, [l in labels(t) WHERE l <> '__Entity__' | l] AS source_type,
                   e{{.* , embedding: Null, name: Null}} AS source_properties,
                   type(r) AS type,
                   e.name AS target_id, [l in labels(e) WHERE l <> '__Entity__' | l] AS target_type,
                   t{{.* , embedding: Null, name: Null}} AS target_properties
        }}
    ...
    def get_rel_map(
        ...
        response = self.structured_query(
            ...
            RETURN source.id AS source_id, [l in labels(source) WHERE l <> '__Entity__' | l] AS source_type,
                    source{{.* , embedding: Null, id: Null}} AS source_properties,
                    type,
                    endNode.id AS target_id, [l in labels(endNode) WHERE l <> '__Entity__' | l] AS target_type,
