 // ****************************************************************************** FILENAME: RC04EXman.c  CONTENTS: This is a 128KB & 256KB flash application program for the V3300 test system.  COPYRIGHT 1997 HITACHI LTD. ALL RIGHTS RESERVED. CREATED BY T.Shimozato  2008/11/01  rev.00  1'st created by H.Kobori  *****************************************************************************
...
// -------------------------------------------------------------------------------- Function:       PrintTestProgramBanner  Description:    A general purpose function that Shell2 automatically executes during test program initialization only. --------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------- Function:       PrintTestNameFirst  Description:    A general purpose function that Shell2 automatically executes at the beginning of every test block. --------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------- Function:       PrintTestFunctionResult  Description:    A general purpose function that Shell2 automatically executes at the end of every test block. --------------------------------------------------------------------------------
            ...
            int status, char * tname)
{
    Do_For_Each_Alive_DUT
    {
        ...
        {
            printf ("T_%s= %d (uS)", tname, test_time);
        }
    }
}

// -------------------------------------------------------------------------------- Function:       PrintDieResult  Description:    A general purpose function that Shell2 automatically executes at the end of every die tested, just prior to binning. --------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------- Function:       CheckBoardConfig  Description:    A function that verifies the proper board config during test program initialization. --------------------------------------------------------------------------------
...
char  *val_devtype  = devicename;
// Device name
char  *program_name = PROGRAMNAME;
// Program revision
char  *Tester_name  = "V___________________";
// default 20char
// ------------------------
// ---- PROBE DIRECTRY ----
// ------------------------
...
int    REJ_BIN_LIST[] = {1,0,-1};
// if bin=1 pass else fail
////////////////////////////////////////////////////////////////////////////////
...
int glb_temp;
int glob_pmode_flp = 0;
// FLP PROGRAM MODE
int glob_pmode_e2p = 1;
// E2P PROGRAM MODE
...
char dataout_logfname[255] = "DATA.LOG";
...
// Please see an Exe_Menu_With_Var1 function in compat.c file
  ...
  GetLotName(lotname, sizeof(lotname));
  // Get Lot Name
  GetWaferName(wafername, sizeof(wafername));
  // Get Wafer Name
  strcpy( devicename, DEVICENAME );
  // Set Device Name
  strcpy( temp_comment, "Typ 25'C" );
  // Init temp_comment
  NameLen = 10;
  ...
  printf("********************************");
  printf( "%.30s", devicename );
  printf("RAM   : %dKB", RAM_SIZE/1024 );
  printf("********************************");
  ...
  if (test_flow == 1){
    if (NORMAL_FLOW || auto_ex_cmd_only){
      if(var1 == 0) {
        ...
        case BFT_FL1D:
        case BFT_FL2D:
          result_bin = WaferTestProgram( var1&0x00FF );
          // Wafer Test Versa Emulation
          break;
        // case BGR_WT1: result_bin = BGR_WT(interface,1); break; case BGR_WT2: tempo1 = bentime(); result_bin = BGR_WT_HT(interface); tempo2 = (bentime()-tempo1-init_bentime())/1000; printf(" BGR_WT2 RunTime = %d[ms]",tempo2); break; case BGR_WT4: result_bin = BGR_WT(interface,4); break; case BGR_WT5: result_bin = BGR_WT(interface,5); break;
        case AE_WT:
        ...
        break;
      	case QT_SNO:
            ...
            printf("+====================+");
            printf("| Sample No, ->%d",QT_SampleNo);
            printf("+====================+");
...
int UserExCommand(int argc,char *argv[])
{
  ...
  if( ExCommand( "fk" ) ||  ExCommand( "FK" ) ){
    ...
    if( argc>1 ){
      if( 0==strncmp( argv[1], "0x", 2 ) ){
        ...
        for( i=1; i<argc; i++ ){
          ...
          for( j=0; j<32; j++, fk_bit = fk_bit<<1 ){
            if( j==strtod( argv[i] ) ){
              if( fc_key&fk_bit )  fc_key &= ~fk_bit;
              // fc_key bit clear
              else                 fc_key |=  fk_bit;
              // fc_key bit set
            }
      ...
      var2 = fc_key; Exe_Menu_With_Var2( var2 );
      // set var2 no mean?
    }
    DisplayMenu();
    exit(PASS);
...
// ============================================================================
{
  ...
  get_system_option("__szDeviceID", device_id, &length);
  // Get Device ID
  printf("|   [ %s ]", device_id);
  printf("+==========================================");
...
{
  printf("Performing Partial Summary operation for %s.", program_name);

  return(PASS);
}

 // ****************************************************************************** Prober Alignment Function (F12) Use Test button or F12 to execute this flow *****************************************************************************
