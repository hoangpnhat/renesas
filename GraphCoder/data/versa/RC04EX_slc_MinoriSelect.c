// ****************************************************************************
// ! @file RC04EXslc_MonitorTrimingSelect.c @brief This is a 130nm(RC01SN) FLASH module user interface program for the 4000,5000 test system @author T.Morioka @date 2012.04.16 Rev.00
// ****************************************************************************
int MinoriSelect(int vcc[NUM_POWERSUPPLY],int waittime,interface_t* param)
{
  ...
  // debug add
  int etlr25,etlr26,etlr27,etlr28,etlr29,etlr30,etlr32,etlr33,etlr34,cond2;
  //hashi
  int logparam[20];
  ...
  while( test_cond != QUIT){
    ...
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Test    | Module   :%s(mdl)",module);
    printf("|          | RegSet   :%02X(reg)",GetValue(IN_REG,param) );
    printf("|          | WaitTime :%d *0x4000 cycle(wait)",waittime );
    printf("|          | Extal    :%d[ns] %d[MHz] (s)", GetValue(IN_EXTAL1,param),freq);
    printf("|          | [Other] CLK-Mode : %s (clk)",clk_mode);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Pattern | Read-BI    : %s(bi)",read_bi);
    printf("|  Option  | ");
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Power   | Vcc:%4dmV(vcc) Vddh:%4dmV(vddh) Vdd:%4dmV(vdd)", vcc[0],vcc[1],vcc[2]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  exit(e)");
    printf("+-----------------------------------------------------------------------------------------------------+");
    ...
    case SET_CVCC:
      ...
      DigitInput(&tempo);
      vcc[0] = tempo;
      printf("\tset vcc %dmV",vcc[0]);
      break;
    case SET_CVDD:
    //Add set vref 2018/0307
      printf("enter vdd(mV) -> ");
      DigitInput(&tempo);
      vcc[2] = tempo;
      printf("\tset VDD %dmV",vcc[2]);
      break;
    case SET_CVDDH:
    //Add set vrefh 2018/0307
      printf("enter vddh(mV) -> ");
      DigitInput(&tempo);
      vcc[1] = tempo;
      printf("\tset VDDH %dmV",vcc[1]);
      break;
    case SET_WAIT:
    // *** Set WaitTime ***
      printf("enter WaitTime (sec) ->");DigitInput(&tempo);
	  waittime = tempo;
      printf("WaitTime %ds",waittime);
      break;
    case SET_SPEED:
    // *** Set SPEED ***
...
//	    loop_max   = 1130;//org//
	  ...
	  break;
        ...
        fprintf(Fptdata," module,%s  patten,%s  vcc,%d,vddh,%d,vdd,%d,CLK-Mode,%s",module,wgl_name,vcc[0],vcc[1],vcc[2],clk_mode);
	    ...
	    fprintf(Fptdata,"%s,1,10,20,25,30,31,32,33,34,36,40,42,43,45,48,50",mesure_name);
...
for(j=1; j<=2; j++){
  	      if ( j == 1 ) {
  	      // Vmin //
			printf("VminVminVminVminVminVmin");
	      ...
	      // Vmax //
			printf("VmaxVmaxVmaxVmaxVmaxVmax");
...
// ************************************************************************************
      ...
      CpuModeFunc(MNORMAL,MSNORMAL,&vcc[0],W1SEC*waittime,param,CreatePatFileName("readdump_code_at"),"CPU Vth-READ ALL");
      // �t�@�[������ŕύX����\������
...
//	    loop_max   = 790;
	  ...
	  break;
          case 7:
          // ETLR26[7:0],ETLR27[3:0] T1 Precharge
	    loop_max   = 16;
	    loop_min   = 1;
	    loop_deltac = 1;
	    loop_delta = 1;
	  ...
	  break;
        ...
        fprintf(Fptdata,"*** etlr33<7:0>=0x%X (T4 saout FLI/FLP)",GetValue(IN_ETLR33,param));
        fprintf(Fptdata,"*** etlr34<7:0>=0x%X (T6 2nd-SA enable control time)",GetValue(IN_ETLR34,param));
        fprintf(Fptdata,"*** Extal=%dns ",GetValue(IN_EXTAL1,param));
        ...
        CloseDataOutFile();delay_timer(200);
          ...
          printf("*** T4 saout setting!! ETLR33 Select ?  ex) 0xEF input ->"); tempo11=0x22;
          //DigitInput(&tempo11);
          SetValue(IN_ETLR33,param,tempo11);
          // 2ns Delay
          printf("*** T6 (T4-T5) setting!! ETLR34 Select ?  ex) 0xEF input ->"); tempo12=0x22;
          //DigitInput(&tempo12);
          SetValue(IN_ETLR34,param,tempo12);
          // 2ns Delay 2.80ns Trimming Read
          printf("*** T5 saenb2/ T3 saenb setting!! ETLR32<7:0> Select ?  ex) 0xEF input ->"); tempo13=0x5E;
          //DigitInput(&tempo13);
          SetValue(IN_ETLR32,param,tempo13);
          // 2ns Delay 2.80ns Trimming Read
...
//          SetValue(IN_ETLR26,param,0x77),SetValue(IN_ETLR27,param,0x77 & 0xF0);    // 8ns Delay Setting Read
          printf("*** T1 Precharge setting!! ETLR26<7:0>,ETLR27<3:0> Select ?  ex) 0x77 input ->"); tempo16=0xEE;
          //DigitInput(&tempo16);
          SetValue(IN_ETLR26,param,tempo16),SetValue(IN_ETLR27,param,tempo16 & 0x0F);
          // 2ns Delay 2.80ns Trimming Read

          printf("*** and Discharge TapLoop enale?  Enable:2 / Disable:1 ? ->"); DigitInput(&taploopnum);
...
//	       SetValue(IN_ETLR32,param,0xF3); // T5 //
	     ...
	     break;
         case 6:
         //ETLR26[7:0],ETLR27[3:0] T3 saenb
////	       SetValue(IN_ETLR33,param,0xFF); // T4 //
////	       SetValue(IN_ETLR34,param,0xFF); // T6 //
////	       SetValue(IN_ETLR32,param,0xF3); // T5 //
...
//          SetValue(IN_ETLR26,param,0x77),SetValue(IN_ETLR27,param,0x77 & 0xF0);    // 8ns Delay Setting Read
	       SetValue(IN_ETLR28,param,0xF5);
	       // T2 Discharge target //
	       SetValue(IN_ETLR29,param,0xF5);
	       // T2 Discharge target //
	       SetValue(IN_ETLR30,param,0xF5);
	       // T2 Discharge target //
	       switch(loop){
	         case 1:tempo1 = 0x00 ;break;
	         case 2:tempo1 = 0x01 ;break;
	         case 3:tempo1 = 0x02 ;break;
	         case 4:tempo1 = 0x03 ;break;
	         //Def
	     ...
	     break;
         case 7:
         //ETLR26[7:0],ETLR27[3:0] T3 saenbPrecharge
//	       SetValue(IN_ETLR33,param,0xFF); // T4 //
//	       SetValue(IN_ETLR34,param,0xFF); // T6 //
//	       SetValue(IN_ETLR32,param,0xF3); // T5 //
	       SetValue(IN_ETLR32,param,((GetValue(IN_ETLR32,param) & 0xF0) | 0x0F));
	       // T3 ETLR32<3:0> saout Max //
//	       SetValue(IN_ETLR32,param,0xF3); // T1 Precharge //
	       switch(loop){
	         ...
	         case 4:tempo1 = 0xBB, tempo15 = 0xFB ;break;
	         //Def
...
// printf(" === EXTAL1 parameter === "); printf(" 20ns : 50.00MHz "); printf(" 21ns : 47.62MHz "); printf(" 22ns : 45.45MHz "); printf(" 23ns : 43.48MHz "); printf(" 24ns : 41.67MHz "); printf(" 25ns : 40.00MHz "); printf(" 28ns : 35.71MHz "); printf(" 29ns : 34.48MHz "); printf(" 30ns : 33.33MHz "); printf(" 31ns : 32.26MHz "); printf(" 32ns : 31.25MHz "); printf(" 33ns : 30.30MHz "); printf(" 40ns : 25.00MHz "); printf(" 50ns : 20.00MHz "); printf(" 100ns: 10.00MHz "); printf(" Select EXTAL1 T=%3dns /(ex: 20ns)  -> ",GetValue(IN_EXTAL1,param));
       if(taploopnum == 1){
    ...
    case 315:
	  ...
	  for(j=1;j<=Freq_num;j++){
		...
		for(i=1;i<=loop_max;i++){
		  ...
		  printf(" Freq = %03d MHz : Vdd = %04d mV : Result = %04X",dataout[j][0],dataout[0][i],dataout[j][i]);
		  Ffpt = 1;
		}
  	  }
...
int MinoriRead_Kani(int mode, int vs[NUM_POWERSUPPLY], int waittime, interface_t* param, int *current)
{
  ...
  stack1 = GetValue(IN_BDATA00,param);
  // �X�N�����u�� M0 or M1
  stack2 = GetValue(IN_BDATA01,param);
  // LowPowerRead - Off
  stack3 = GetValue(IN_BDATA02,param);
  // Back-Bias Off
  stack4 = GetValue(IN_BDATA03,param);
  // CLK Setting 1:�t�� 0:����
  stack5 = GetValue(IN_BDATA07,param);
  // Loop Times
  stack6 = GetValue(IN_BDATA08,param);
  // Micro-Code
  stack7 = GetValue(IN_BDATA09,param);
  // MINORI-Current Mode

	SetValue(IN_BDATA07,param,0xFFFFF);
	// Loop Times
  SetValue(IN_BDATA09,param,2);
  // MINORI-Current Mode
  ...
  switch(mode & 0xFFFFFFF0){
    ...
    break;
    default:printf("MinoriRead_Kani Func Error!!! C-Program");
  }
  ...
  printf("MinoriModeFunc Time = %d",(bentime()-start_time-init_bentime())/1000);
