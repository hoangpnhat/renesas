// ****************************************************************************
// ! @file RC04EX_slc_ReadShmooSelect.c @brief This is a 130nm(RC01SN) FLASH module user interface program for the V3300,4000,5000 test system @author S.Yamaoka @date 2010.07.09 Rev.00
// ****************************************************************************
...
int ReadShmooSelect( int vcc[NUM_POWERSUPPLY], int waittime, interface_t* param ){
  ...
  int extal_pllx8_table[]={
   ...
   //  0 Dummy
  5000000  ,
  //  1 if PLLx5  ICLK=  1MHz
  1000000  ,
  //  2 if PLLx5  ICLK=  5MHz
   500000  ,
   //  3 if PLLx5  ICLK= 10MHz
   250000  ,
   //  4 if PLLx5  ICLK= 20MHz
   227272  ,
   //  5 if PLLx5  ICLK= 22MHz
   208333  ,
   //  6 if PLLx5  ICLK= 24MHz
   192307  ,
   //  7 if PLLx5  ICLK= 26MHz
   178571  ,
   //  8 if PLLx5  ICLK= 28MHz
   166666  ,
   //  9 if PLLx5  ICLK= 30MHz
   156250  ,
   // 10 if PLLx5  ICLK= 32MHz
   147058  ,
   // 11 if PLLx5  ICLK= 34MHz
   138888  ,
   // 12 if PLLx5  ICLK= 36MHz
   131578  ,
   // 13 if PLLx5  ICLK= 38MHz
   125000  ,
   // 14 if PLLx5  ICLK= 40MHz
   119047  ,
   // 15 if PLLx5  ICLK= 42MHz
   113636  ,
   // 16 if PLLx5  ICLK= 44MHz
   108695  ,
   // 17 if PLLx5  ICLK= 46MHz
   104166  ,
   // 18 if PLLx5  ICLK= 48MHz
   100000  ,
   // 19 if PLLx5  ICLK= 50MHz
    96153  ,
    // 20 if PLLx5  ICLK= 52MHz
    92592  ,
    // 21 if PLLx5  ICLK= 54MHz
    89285  ,
    // 22 if PLLx5  ICLK= 56MHz
    86206  ,
    // 23 if PLLx5  ICLK= 58MHz
    83333  ,
    // 24 if PLLx5  ICLK= 60MHz
    80645  ,
    // 25 if PLLx5  ICLK= 62MHz
    78125  ,
    // 26 if PLLx5  ICLK= 64MHz
    75757  ,
    // 27 if PLLx5  ICLK= 66MHz
    73529  ,
    // 28 if PLLx5  ICLK= 68MHz
    71428  ,
    // 29 if PLLx5  ICLK= 70MHz
    69444  ,
    // 30 if PLLx5  ICLK= 72MHz
    67567  ,
    // 31 if PLLx5  ICLK= 74MHz
    65789  ,
    // 32 if PLLx5  ICLK= 76MHz
    64102  ,
    // 33 if PLLx5  ICLK= 78MHz
    62500  ,
    // 34 if PLLx5  ICLK= 80MHz
    60975  ,
    // 35 if PLLx5  ICLK= 82MHz
    59523  ,
    // 36 if PLLx5  ICLK= 84MHz
    58139  ,
    // 37 if PLLx5  ICLK= 86MHz
    56818  ,
    // 38 if PLLx5  ICLK= 88MHz
    55555  ,
    // 39 if PLLx5  ICLK= 90MHz
    54347  ,
    // 40 if PLLx5  ICLK= 92MHz
    53191  ,
    // 41 if PLLx5  ICLK= 94MHz
    52083  ,
    // 42 if PLLx5  ICLK= 96MHz
    51020  ,
    // 43 if PLLx5  ICLK= 98MHz
    50000  ,
    // 44 if PLLx5  ICLK=100MHz
    49019  ,
    // 45 if PLLx5  ICLK=102MHz
    48076  ,
    // 46 if PLLx5  ICLK=104MHz
    47169  ,
    // 47 if PLLx5  ICLK=106MHz
    46296  ,
    // 48 if PLLx5  ICLK=108MHz
    45454  ,
    // 49 if PLLx5  ICLK=110MHz
    44642  ,
    // 50 if PLLx5  ICLK=112MHz
    43859  ,
    // 51 if PLLx5  ICLK=114MHz
    43103  ,
    // 52 if PLLx5  ICLK=116MHz
    42372  ,
    // 53 if PLLx5  ICLK=118MHz
    41666  ,
    // 54 if PLLx5  ICLK=120MHz
    40983  ,
    // 55 if PLLx5  ICLK=122MHz
    40322  ,
    // 56 if PLLx5  ICLK=124MHz
    39682  ,
    // 57 if PLLx5  ICLK=126MHz
    39062  ,
    // 58 if PLLx5  ICLK=128MHz
    38461  ,
    // 59 if PLLx5  ICLK=130MHz
    37878  ,
    // 60 if PLLx5  ICLK=132MHz
    37313  ,
    // 61 if PLLx5  ICLK=134MHz
    36764  ,
    // 62 if PLLx5  ICLK=136MHz
    36231  ,
    // 63 if PLLx5  ICLK=138MHz
    35714  ,
    // 64 if PLLx5  ICLK=140MHz
    35211  ,
    // 65 if PLLx5  ICLK=142MHz
    34722  ,
    // 66 if PLLx5  ICLK=144MHz
    34246  ,
    // 67 if PLLx5  ICLK=146MHz
    33783  ,
    // 68 if PLLx5  ICLK=148MHz
    33333  ,
    // 69 if PLLx5  ICLK=150MHz
    32894  ,
    // 70 if PLLx5  ICLK=152MHz
    32467  ,
    // 71 if PLLx5  ICLK=154MHz
    32051  ,
    // 72 if PLLx5  ICLK=156MHz
    31645  ,
    // 73 if PLLx5  ICLK=158MHz
    31250  ,
    // 74 if PLLx5  ICLK=160MHz
   100000  ,
   // 75 if PLL OFF  ICLK= 10MHz
    50000  ,
    // 76 if PLL OFF  ICLK= 20MHz
    40000  ,
    // 77 if PLL OFF  ICLK= 25MHz
    33333  ,
    // 78 if PLL OFF  ICLK= 30MHz
    28571  ,
    // 79 if PLL OFF  ICLK= 35MHz
    25000  ,
    // 80 if PLL OFF  ICLK= 40MHz
    22222  ,
    // 81 if PLL OFF  ICLK= 45MHz
    20000  ,
    // 82 if PLL OFF  ICLK= 50MHz
  };
  ...
  v_memcpy( (void*)param_back, (void*)param, sizeof(interface_t)*TABLE_END );
  // input param store


  vs[0] = vcc[0]; vs[1] = vcc[1]; vs[2] = vcc[2];
  in_ldata[4] = 0x00000000;
  // UIArea
  in_ldata[5] = 0x00000000;
  // Top Address
  in_ldata[6] = 0x00180000;
  // End Address
  in_ldata[7] = 0x40050202;
  // bit0-7   expect 0=All0 1=All1 2=ChkA 3=ChkB
  ...
  in_ldata[8] = 0x00000000;
  // SCKDIVCR bit10-8=BCLK div  bit26-24=ICLK div  0=/1  1=/2  2=/4  3=/8  4=/16  5=/32  6=/64
  in_ldata[9] = 0x00000005;
  // CKSCR 0=HOCO 1=MOCO 2=LOCO 3=MAIN 4=SubOSC 5=PLL
  in_ldata[10]= 0x00004400;
  // PLLCCR bit0-1 0=/1 1=/2 2=/3 3=/4  bit4=0=main bit4=1=HOCO  b10-b8+1=xPll bit14=0=32-48MHz bit14=1=32-64MHz
  in_ldata[11]= 0x000411AA;
  // EXTAL ps
  in_ldata[12]= 0x00000000;
  // FLWT 0=0wait  1=1wait
  in_ldata[13]= 0x00000000;
  // HOCOMCR 0=24MHz 1=32MHz 2=48MHz 3=64MHz
  ...
  x_step = 1; y_step = 1;
  y_loop_min = 1; y_loop_max = 9;
  // vdd
  x_loop_min = 9; x_loop_max = 44;
  // freq
  if( CheckFK(7) ){
  // debug
    y_loop_min = 4; y_loop_max = 6;
    // vdd
    x_loop_min = 9; x_loop_max = 74;
    // freq
  }
  ...
  do{
    tempo = 1;
    while( tempo ){
      printf( " title   (change no)");
      printf( " EXPECT PATTERN (10) | " ); if( ctr_exp==0 )
      printf( " Read Type      (20) | " ); if( ctr_typ&0x01 )
      printf( " Clock Source   (30) | " ); if( ckscr==0 )
      printf( " PLLCCR         (40) | H'%04X PLL /%d x%d  FSEL0=%d ", in_ldata[10], pllccr_div + 1, pllccr_multiple + 1, pllccr_range ); if( pllccr_clk==0 )
                                                          if( pllccr_range==0 ) printf( "32-48MHz" ); else
      printf( " HOCOMCR        (50) | H'%02X ", hocomcr ); if( hocomcr==0 )
      printf( " FLWT           (60) | wait %d cyc", flwt );
      printf( " GREG           (70) | " ); if( ctr_trans&0x80 )
      printf( " Pattern        (80) | %s", use_pat_name );
      printf( " Sample Name    (1)  | %s", sample_name );
      printf( " Temperature    (2)  | %d'C", test_temperature );
      printf( " Target         (3)  | Area[%d]  Address[%08X]-[%08X]", in_ldata[4], in_ldata[5], in_ldata[6] );
      printf( " Range          (4)  | Exvref[ " ); if( exvref_flag )
      printf( " Voltage        (5)  | VCC=%dmV  ", vs[0] ); if( -1==vdd_const_flag )
      printf( " Option         (6)  | FailStop[ ", vs[0] ); if( stop_flag )
      printf( " Break (99)  Test Start(0)   input no ==> " );
      DigitInput( &tempo );
      if( tempo==99 ) return( FAIL );
      if( tempo==1 ){ printf( "sample name[max 30 char] => " ); StringInput( sample_name ); } if( tempo==2 ){
      if( tempo==3 ){
        printf( "                AreaSelect  Top Addr   End Addr+1" );
        printf( " UserArea         ( 0 )     0x00000000-0x00180000" );
        printf( " Extra1=Table1    ( 5 )     0x01010000-0x01010400" );
        printf( " Extra2=Info1     ( 1 )     0x01008000-0x01008400" );
        printf( " Extra3=Info2     ( 2 )     0x0100A000-0x0100A400" );
        printf( " Extra4=Info3     ( 3 )     0x0100C000-0x0100C400" );
        printf( " Extra5=Table2    ( 6 )     0x01012000-0x01012400" );
        printf( " Extra6=Info4     ( 4 )     0x01010000-0x01003000 or 0x01004000" );
      ...
      if( tempo==4 ){
        ...
        if( exvref_flag ){
          ...
          printf( " Ystart => " ); DigitInput( &y_loop_min );
        }
        ...
        printf( " Xstart => " ); DigitInput( &x_loop_min );
      ...
      if( tempo==10 ){ printf( "0=All0  1=All1  2=ChkA  3=ChkB  => " ); DigitInput( &ctr_exp ); } if( tempo==20 ){
      if( tempo==30 ){ printf( "0=HOCO  1=MOCO  2=LOCO  3=MAIN  4=SubOSC  5=PLL   => " ); DigitInput( &ckscr ); } if( tempo==40 ){
      if( tempo==50 ){ printf( "0=24MHz  1=32MHz  2=48MHz  3=64MHz  => " ); DigitInput( &hocomcr ); } if( tempo==60 ){
      if( tempo==70 ){ printf( "bit7=1=FACI Trans Exe  bit6=1=Info1 data set  bit5=Table2 data set  bit4=Init data set  0xYY => " ); DigitInput( &ctr_trans ); } if( tempo==80 ){
    ...
    if( ( Fptdata = fopen( baselog, "at" ) )==NULL ){ printf( " -- FAIL -- Not Open file:%s !", baselog ); return( FAIL ); }  if( CheckFK(7) ){
    // debug
    ...
    DataOut2( 1, Fptdata, "\n-------------------------------------------\n" );
    sprintf( outbuf, "%s", sample_name ); DataOut2( 1, Fptdata, outbuf );
    DataOut2( 1, Fptdata, "-------------------------------------------\n" );
    sprintf( outbuf, "Ta = %d 'C", test_temperature ); DataOut2( 1, Fptdata, outbuf );
    ...
    sprintf( outbuf, " PLLCCR         (40) | H'%04X PLL /%d x%d  FSEL0=%d ", in_ldata[10], pllccr_div + 1, pllccr_multiple + 1, pllccr_range ); DataOut2( 1, Fptdata, outbuf ); if( pllccr_clk==0 ) DataOut2( 1, Fptdata, "MAIN" ); else if( pllccr_clk==1 ) DataOut2( 1, Fptdata, "HOCO" ); else;
    if( pllccr_range==0 ) DataOut2( 1, Fptdata, "32-48MHz" ); else DataOut2( 1, Fptdata, "48-64MHz" );
    sprintf( outbuf, " HOCOMCR        (50) | H'%02X ", hocomcr ); DataOut2( 1, Fptdata, outbuf ); if( hocomcr==0 ) DataOut2( 1, Fptdata, "24MHz" ); else if( hocomcr==1 ) DataOut2( 1, Fptdata, "32MHz" ); else if( hocomcr==2 ) DataOut2( 1, Fptdata, "48MHz" ); else if( hocomcr==3 ) DataOut2( 1, Fptdata, "64MHz" ); else DataOut2( 1, Fptdata, "None" );
    sprintf( outbuf, " FLWT           (60) | wait %d cyc", flwt ); DataOut2( 1, Fptdata, outbuf );
    ...
    sprintf( outbuf, " Pattern             | %s", use_pat_name ); DataOut2( 1, Fptdata, outbuf );
    sprintf( outbuf, " Sample Name    (1)  | %s", sample_name ); DataOut2( 1, Fptdata, outbuf );
    sprintf( outbuf, " Temperature    (2)  | %d'C", test_temperature ); DataOut2( 1, Fptdata, outbuf );
    sprintf( outbuf, " Target         (3)  | Area[%d]  Address[%08X]-[%08X]", in_ldata[4], in_ldata[5], in_ldata[6] ); DataOut2( 1, Fptdata, outbuf );
    ...
    sprintf( outbuf, " Voltage        (5)  | VCC=%dmV  ", vs[0] ); DataOut2( 1, Fptdata, outbuf ); if( -1!=vdd_const_flag ){ s
    DataOut2( 1, Fptdata, "\n\n" );
    ...
    for(  x = x_loop_min; x<=x_loop_max; x+=x_step ){ sprintf( outbuf, ",%3d.%03d", x_label[x]/1000, x_label[x]%1000 ); DataOut2( 1, Fptdata, outbuf ); } DataOut2( 1, Fptdata, ",,,contents,base,point,read1,read2,read3,read4,read5,read6,read7,read8" );
    ...
    for( y = y_loop_min; y<=y_loop_max; y+=y_step ){
      ...
      sprintf( outbuf, "%3d.%03d", y_label[y]/1000, y_label[y]%1000 ); DataOut2( 1, Fptdata, outbuf );

            // *********************************************************************
            // ******  freq X loop start  *******************************************
      for( x = x_loop_start; x<=x_loop_max; x+=x_step ){
        if( CheckHitKey() ){ printf( " Escape(1) / Next Y-vdd(2) / Next X-freq(0) --> " ); DigitInput( &break_flag ); } if( break_flag ){ if( 2==break_flag ) break_flag = 0;
          break;
        }
        ...
        else if( x_label[x]<=30000 ) time_out = W1SEC*3;
        //Versa 2.02s@25MHz  1.06@50MHz  Simulation 2.85s@10MHz
    ...
    DataOut2( 1, Fptdata, "\n=================================================================================\n" );
    sprintf( outbuf, "%s    Ta = %d", sample_name, test_temperature ); DataOut2( 1, Fptdata, outbuf );
    GetDateTime( &stime_list, stime_buf, 0 ); time( &etime_list ); GetDateTime( &etime_list, etime_buf, 0 );
    sprintf( outbuf, "Date %s - %s", stime_buf, etime_buf ); DataOut2( 1, Fptdata, outbuf );
//    sprintf( outbuf, "lot=%-10.10s , wafer=%-2.2s , X=%3d , Y=%3d ", lotname, wafername, xy_location[0], xy_location[1] ); DataOut2( 3, Fptdata, outbuf );
    sprintf( outbuf, "wafer=%-2.2s , X=%3d , Y=%3d ", wafername, xy_location[0], xy_location[1] ); DataOut2( 1, Fptdata, outbuf );
    sprintf( outbuf, " PAT = [%s] VCC=%dmV", use_pat_name, vs[0] ); DataOut2( 1, Fptdata, outbuf ); if( -1!=vdd_const_flag ){ s
    sprintf( outbuf, " Last Pass PWSTF=%02X MOSCCR=%02X PLLCCR=%04X FLWT=%02X SMDER=%04X SCKSCR=%02X", dbg_pwstf, dbg_mosccr, dbg_pllccr, dbg_flwt, dbg_smder, dbg_sckscr ); DataOut2( 3, Fptdata, outbuf );
    DataOut2( 1, Fptdata, "\nFreq=" );
    for( x = x_loop_min; x<=x_loop_max; x+=x_step ){ sprintf( outbuf, ",%3d.%03d", x_label[x]/1000, x_label[x]%1000 ); DataOut2( 1, Fptdata, outbuf ); } DataOut2( 1, Fptdata, ",,pass freq,,contents,base,point,read1,read2,read3,read4,read5,read6,read7,read8" );
    if( -1!=vdd_const_flag ){ sprintf( outbuf, "vdd=[%dmV]", vdd_const_flag ); DataOut2( 1, Fptdata, outbuf ); }else{ DataOut2( 1, Fptdata, "vdd" ); } if( -1!=vddh_const_flag ){ s
    ...
    for( y = y_loop_min; y<=y_loop_max; y+=y_step ){
      ...
      if( tempo_pass == -1 ) { sprintf( outbuf, ",,%3d.%03d", 0, 0 ); DataOut2( 1, Fptdata, outbuf ); } else                   { s
      sprintf( outbuf, ",,0x%08X,0x%08X,0x%08X", fail_contents[y], fail_base[y], fail_point[y] ); DataOut2( 1, Fptdata, outbuf );
      for( tempo = 0; tempo<8; tempo++ ){ sprintf( outbuf, ",0x%08X", fail_data[y][tempo] ); DataOut2( 1, Fptdata, outbuf ); } } DataOut2( 1, Fptdata, "=================================================================================" );
    fclose( Fptdata );
  }while( repeat_flag );

  v_memcpy( (void*)param, (void*)param_back, sizeof(interface_t)*TABLE_END );
  // input param restore
...
int ReadShmooSelect( int vcc[NUM_POWERSUPPLY], int waittime, interface_t* param ){
  // int vdd_500_table[]={ 0, 600, 575, 550, 525, 500, 475, 450, 425, 400, };
  int vdd_500_table[]={
  ...
  in_ldata[5] = 0x00000000;
  // Top Address
  in_ldata[6] = 0x00040000;
  // End Address
  in_ldata[7] = 0x00000202;
  // bit0-7   expect 0=All0 1=All1 2=ChkA 3=ChkB
                                                        // bit8-15  bit8=1=Normal bit9=1=X-PingPong bit10=1=X-Complemental
  in_ldata[8] = 0x00000000;
  // 0  start n
  in_ldata[9] = 0x00000027;
  // 39   end n
  in_ldata[12]= 0x00000000;
  // FLWT 0=0wait  1=1wait
  ...
  do{
    ...
    while( tempo ){
      ...
      printf( " Read Type      (20) | " ); if( ctr_typ&0x01 )
      printf( " FLWT           (60) | wait %d cyc", flwt );
      printf( " Extal          (70) | %d ps", extal_ps );
      printf( " Sample Name    (1)  | %s", sample_name );
      printf( " Temperature    (2)  | %d'C", test_temperature );
      printf( " Target         (3)  | Area[%d]  Address[%08X]-[%08X]", in_ldata[4], in_ldata[5], in_ldata[6] );
      printf( " Range          (4)  | Exvref[ " ); if( exvref_flag )
      printf( " Voltage        (5)  | VCC=%dmV  ", vs[0] ); if( -1==vdd_const_flag )
      printf( " Freq n max39   (6)  | %d - %d", in_ldata[8], in_ldata[9] );
      printf( " flash_boot          (77)  UI=0-1000 Program Func  1000-3FFFF expect ChkA" );
      printf( " flash_boot_port     (772) UI=0-1000 Program Func" );
      printf( " flash_boot_port23s  (773) UI=0-1000 Program Func" );
      printf( " flash_boot_dijkstra (774) UI=0-1000 Program Func" );
      printf( " flash_boot_jmp      (78)  7s time jmp UI 101 address" );
      printf( " flash_boot_jmp2     (782) 30s time out jmp UI 101 address" );
      printf( " flash_boot_jmp3     (783) ETCR0-11 ETLR0-39 Change Jmp" );
      printf( " flash_boot_jmp4     (784) ETCR0-11 ETLR0-39 Change void jmp" );
      printf( " User Boot TM18      (79)" );
      printf( " Break (99)  Test Start(0)   input no ==> " );
      DigitInput( &tempo );
      if( tempo==99 ) return( FAIL );
      if( tempo==1 ){ printf( "sample name[max 30 char] => " ); StringInput( sample_name ); } if( tempo==2 ){
      if( tempo==3 ){
        printf( "                AreaSelect  Top Addr   End Addr+1" );
        printf( " UserArea         ( 0 )     0x00000000-0x00040000" );
        printf( " Extra1=Table1    ( 5 )     0x01010000-0x01010400" );
        printf( " Extra2=Info1     ( 1 )     0x01008000-0x01008400" );
        printf( " Extra3=Info2     ( 2 )     0x0100A000-0x0100A400" );
        printf( " Extra4=Info3     ( 3 )     0x0100C000-0x0100C400" );
      ...
      if( tempo==10 ){ printf( "0=All0  1=All1  2=ChkA  3=ChkB  => " ); DigitInput( &ctr_exp ); } if( tempo==20 ){
      if( tempo==60 ){ printf( "0=0 wait  1=1 wait  => " ); DigitInput( &flwt ); } if( tempo==70 ){
    ...
    time( &stime_list );
    strcat( baselog, filenamebuf );
    if( ( Fptdata = fopen( baselog, "at" ) )==NULL ){ printf( " -- FAIL -- Not Open file:%s !", baselog ); return( FAIL ); }  DataOut2( 2, Fptdata, "-------------------------------------------" );
    sprintf( outbuf, "%s", sample_name ); DataOut2( 2, Fptdata, outbuf );
    DataOut2( 2, Fptdata, "-------------------------------------------\n" );
    sprintf( outbuf, "Ta = %d 'C", test_temperature ); DataOut2( 2, Fptdata, outbuf );

    sprintf( outbuf, " title   (change no) %d", tempo0 ); DataOut2( 2, Fptdata, outbuf );
    ...
    sprintf( outbuf, " FLWT           (60) | wait %d cyc", flwt ); DataOut2( 2, Fptdata, outbuf );
    sprintf( outbuf, " Extal          (70) | %d ps", extal_ps ); DataOut2( 2, Fptdata, outbuf );
    sprintf( outbuf, " Pattern             | %s", use_pat_name ); DataOut2( 2, Fptdata, outbuf );
    sprintf( outbuf, " Sample Name    (1)  | %s", sample_name ); DataOut2( 2, Fptdata, outbuf );
    sprintf( outbuf, " Temperature    (2)  | %d'C", test_temperature ); DataOut2( 2, Fptdata, outbuf );
    sprintf( outbuf, " Target         (3)  | Area[%d]  Address[%08X]-[%08X]", in_ldata[4], in_ldata[5], in_ldata[6] ); DataOut2( 2, Fptdata, outbuf );
    ...
    sprintf( outbuf, " Voltage        (5)  | VCC=%dmV  ", vs[0] ); DataOut2( 2, Fptdata, outbuf ); if( -1!=vdd_const_flag ){ s
    DataOut2( 2, Fptdata, "\n\n" );
    ...
    SetValue( IN_EXTAL_PS, param, extal_ps );
    // def 100ns

        // *********************************************************************
        // ******  vdd/vddh Y loop start  **************************************
    for( y = y_loop_min; y<=y_loop_max; y+=y_step ){
      if( CheckHitKey() ){ printf( " Escape(1) / Next(0) --> " ); DigitInput( &break_flag ); } if( break_flag )  break;

      if( -1==vddh_const_flag ) vs[1] = y_label[ y ]; else vs[1] = vddh_const_flag;
      if( -1==vdd_const_flag )  vs[2] =
      // y_label[ y ]
      sprintf( outbuf, "%3d.%03d", y_label[y]/1000, y_label[y]%1000 ); DataOut2( 2, Fptdata, outbuf );
      ...
      for( n = in_ldata[8]; n<=in_ldata[9]; n++ ){
        ...
        if( y==y_loop_min ){ sprintf( outbuf, ",%3d.%03d", (freq_t[n]/1000)/1000, (freq_t[n]/1000)%1000 ); DataOut2( 2, Fptdata, outbuf ); x_label[n] = freq_t[n]/1000; } } if( y==y_loop_min ){ dbg_mosccr = ReadEcrMode8( 0xE4, SIZE_L ); dbg_pwstf = ReadEcrMode8( 0xE8, SIZE_L );
        dbg_flwt = ReadEcrMode8( 0xF4, SIZE_L ); dbg_smder = ReadEcrMode8( 0xD0, SIZE_L ); dbg_sckscr = ReadEcrMode8( 0xD8, SIZE_L );
      ...
      if( dbg_flag ){
        ...
        for( n = 0x3D10; n<0x3D50; n+=4 ){ sprintf( outbuf, ",%08X", ReadEcrMode8( n, SIZE_L ) ); DataOut2( 2, Fptdata, outbuf ); } DataOut2( 2, Fptdata, ",ChangeReg ETCR0-17 ETLR0-39,0x3D50-->" );
        for( n = 0x3D50; n<0x3D90; n+=4 ){ sprintf( outbuf, ",%08X", ReadEcrMode8( n, SIZE_L ) ); DataOut2( 2, Fptdata, outbuf ); } } s
    }
        // ******  vdd Y loop end  *********************************************
        // *********************************************************************
    ...
    DataOut2( 3, Fptdata, "\n=================================================================================\n" );
    sprintf( outbuf, "%s    Ta = %d", sample_name, test_temperature ); DataOut2( 3, Fptdata, outbuf );
    GetDateTime( &stime_list, stime_buf, 0 ); time( &etime_list ); GetDateTime( &etime_list, etime_buf, 0 );
    sprintf( outbuf, "Date %s - %s", stime_buf, etime_buf ); DataOut2( 3, Fptdata, outbuf );
//    sprintf( outbuf, "lot=%-10.10s , wafer=%-2.2s , X=%3d , Y=%3d ", lotname, wafername, xy_location[0], xy_location[1] ); DataOut2( 3, Fptdata, outbuf );
    sprintf( outbuf, "wafer=%-2.2s , X=%3d , Y=%3d ", wafername, xy_location[0], xy_location[1] ); DataOut2( 3, Fptdata, outbuf );
    sprintf( outbuf, " PAT = [%s] Extal=%dps VCC=%dmV", use_pat_name, extal_ps, vs[0] ); DataOut2( 3, Fptdata, outbuf ); if( -1!=vdd_const_flag ){ s
    sprintf( outbuf, " 1st Test PWSTF=%02X MOSCCR=%02X  FLWT=%02X SMDER=%04X SCKSCR=%02X", dbg_pwstf, dbg_mosccr, dbg_flwt, dbg_smder, dbg_sckscr ); DataOut2( 3, Fptdata, outbuf );
    sprintf( outbuf, " Trim 64MHz = %08X = %3d.%03d MHz", x_label[41], ( x_label[40]/1000 )/1000, ( x_label[40]/1000 )%1000 ); DataOut2( 3, Fptdata, outbuf );
    ...
    if( -1!=vdd_const_flag ){ sprintf( outbuf, "vdd=[%dmV]", vdd_const_flag ); DataOut2( 3, Fptdata, outbuf ); }else{ DataOut2( 3, Fptdata, "vdd" ); } if( -1!=vddh_const_flag ){ s

    for( y = y_loop_min; y<=y_loop_max; y+=y_step ){
      sprintf( outbuf, "%3d.%03d", y_label[y]/1000, y_label[y]%1000 ); DataOut2( 3, Fptdata, outbuf );
      ...
      for( tempo = 0; tempo<8; tempo++ ){ sprintf( outbuf, ",0x%08X", fail_data[y][tempo] ); DataOut2( 3, Fptdata, outbuf ); } } DataOut2( 3, Fptdata, "=================================================================================" );
    fclose( Fptdata );
  }while( 1 );
...
int RomBoot_FirstSetting(int mode ,int vcc[NUM_POWERSUPPLY], int waittime, interface_t* param ){
  ...
  flwt = in_ldata[12];
    ...
    if( tempo0==783 || tempo0==784 ){
      ...
      n = 0;
      do{
        printf( "No Change=0  ETLR Change=1  ETCR Change=2   => " );
        //DigitInput( &tempo2 );
        tempo2 = 0;
        if( tempo2==1 || tempo2==2 ){
          tempo2 = ( tempo2<<24 ) & 0xFF000000;
