// ***************************************************************************
// FILENAME    RX200tst.c CONTENTS: This is a 130nm(RC01SN) FLASH module voltage monitor subroutine program for the V3300,4000,5000 test system COPYRIGHT 2007 Renesas Technology ALL RIGHTS RESERVED 2010/07/22  rev.00  1'st created by S.Yamaoka
// ****************************************************************************
...
//  double k;
  ...
  selvol = 0;
  //Initialize selvol
  tempo0 = SetValue(IN_BDATA15,param,UA250);
  //PMU range = 250[uA]
  ...
  switch(mode){
    ...
    case V_VRSG:
      ...
      change_reg = IN_ETLR18;
            //for ( i=0 ; i<=7 ; i++ ) reg_code[i] = (0x0 + i) & 0x07;    // Oo͏ύẌׁARgAEg
      reg_code[0] = 0x1 & 0x07;
      reg_code[1] = 0x0 & 0x07;
      reg_code[2] = 0x3 & 0x07;
      reg_code[3] = 0x2 & 0x07;
    ...
    case I_IMONI_FLASH_NOTEMP:
      if( mode == I_IMONI_I01U_NOTEMP )  {sprintf(filename,"imoni-i01u(no-temp)_vsTAP");  testsel_mode = MONITOR_IMONI_I01U_NOTEMP;} if( mode == I_IMONI_FLASH_NOTEMP ) {s
      terminal = VSSMON_MON;
    ...
    case I_IMONI_FLASH_TEMP:
      if( mode == I_IMONI_I01U_TEMP )  {sprintf(filename,"imoni-i01u(temp)_vsTAP");  testsel_mode = MONITOR_IMONI_I01U_TEMP;} if( mode == I_IMONI_FLASH_TEMP ) {s
      terminal = VSSMON_MON;
    ...
    case T_READDELAY10_FLI2:
      ...
      selvol = 40;
  	   break;
    default:  printf("!!!!!!!!!!! No select !!!!!!!!!!!!!");  return;  break;
  }
  // switch
  ...
  if((fpt=fopen(datapat,"at"))==NULL){
    ...
    // -----------------------------------------------------
  if( mode<40 ){
    fprintf(fpt,"ETLR%02d,ETLR_Tap,Voltage[mV] (Vcc:%d Vddh:%d Vdd:%d ETLR%02d)",change_reg,vs[0],vs[1],vs[2],change_reg);
  }else if( mode==V_VHH_E_TEMP || mode==V_VHH_P_TEMP ){
  // d(Vhh)x⏞
    if( mode==V_VHH_E_TEMP ) fprintf(fpt,"ETLR16(Vhh_E):0x%02X",GetValue(IN_ETLR16,param));
    if( mode==V_VHH_P_TEMP ) fprintf(fpt,"ETLR17(Vhh_P):0x%02X",GetValue(IN_ETLR17,param));
    fprintf(fpt,"ETLR%02d,ETLR_Tap,Voltage[mV] (Vcc:%d Vddh:%d Vdd:%d ETLR%02d)",change_reg,vs[0],vs[1],vs[2],change_reg);
  }else if(mode <= 43 && 40 <= mode){
  // dj^
    if( mode==I_IMONI_TEMP ) fprintf(fpt,"ETLR09(Imoni):0x%02X",GetValue(IN_ETLR09,param));
    fprintf(fpt,"ETLR%02d,ETLR_Tap,Current[nA] (Vcc:%d Vddh:%d Vdd:%d ETLR%02d)",change_reg,vs[0],vs[1],vs[2],change_reg);
  }else if(mode == 50 || mode == 51){
    fprintf(fpt,"ETLR%02d,ETLR_Tap,Current[cyc] (Vcc:%d Vddh:%d Vdd:%d ETLR%02d)",change_reg,vs[0],vs[1],vs[2],change_reg);
  }else{
    fprintf(fpt,"ETLR%02d,ETLR_Tap,Current[ns] (Vcc:%d Vddh:%d Vdd:%d ETLR%02d)",change_reg,vs[0],vs[1],vs[2],change_reg);
  }
  ...
  // <=61
    for ( i=loop_st ; i<=loop_ed ; i++ ){
      ...
      printf("### ETLR%02d  Code:0x%02X",change_reg,reg_code[i]);
      if( mode<I_IMONI_I01U_NOTEMP
      // <40
        ...
        fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d,[mV]",change_reg,GetValue(change_reg,param),reg_code[i],selvol);
      }else if(I_IMONI_I01U_NOTEMP <= mode && mode <= I_IMONI_FLASH_TEMP){
      // 40`43
        ...
        if( selvol>=24000 ){
          ...
          //set PMU range 250[uA]
          CpuModeMonitor_Repeat(repeat_mode,terminal,&vs[0],param,W1SEC*waittime,CreatePatFileName("monitorcurrent_at"),"MONITOR_Tap_dependency",&selvol);
                    //CpuModeMonitor(terminal,&vs[0],param,waittime,CreatePatFileName("monitorcurrent_at"),"MONITOR_Tap_dependency",&selvol);
        }
        fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d,[nA]",change_reg,GetValue(change_reg,param),reg_code[i],selvol);
        ...
        if(mode == T_OSC32){
          ...
          printf( " OSC(ETCR15&14) = 0x%04X ->> %d[cyc]", GetValue(OUT_LDATA0,param), result );
          fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d",change_reg,GetValue(change_reg,param),reg_code[i],result);
        }else{
          result = 0xFFFF-GetValue(OUT_LDATA1,param);
          printf( " OSC(ETCR15&14) = 0x%04X ->> %d[cyc]", GetValue(OUT_LDATA1,param), result );
          fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d",change_reg,GetValue(change_reg,param),reg_code[i],result);
        }
      }
    }
...
//      vs[1] = i; //Vddh=0
      ...
      vol[V_VDD] = selvol;
      fprintf(fpt,"V_EXVREF_VDD =%.1f, V_VDD=%d,[mV]",k,selvol);
      }
  }else if( mode == 4
  // VDDH
...
//      vs[2] = i; //Vdd=0
      ...
      vol[V_VDDH] = selvol;
      fprintf(fpt,"V_EXVREF_VDDH=%d, V_VDDH=%d,[mV]",i,selvol);
      }
  }else if(T_READDELAY8_FLP0 <= mode&&mode <= T_READDELAY10_FLI2){
  // DELAY
    ...
    for ( i=loop_st ; i<=loop_ed ; i++ ){
      set_reg = (init_reg & mask) | ((reg_code[i] << shift) & 0xFF);
      SetValue(change_reg,param,set_reg);
	     printf(" ETLR%02d  Code:0x%02X",change_reg,GetValue(change_reg,param));
      first_flag =1;
    ...
    // prbg̃[v
      ...
      // Βlprbg̃[v
        ...
        if( mode==I_IMONI_TEMP ){
          ...
          printf("Monitor : %d[nA]",selvol);
          fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d,[nA]",change_reg,GetValue(change_reg,param),reg_code[i],selvol);
        }else{
          CpuModeMonitor_Repeat(repeat_mode,terminal,&vs[0],param,W1SEC*waittime,CreatePatFileName("monitorvoltage_at"),"MONITOR_Tap_dependency",&selvol);
                    //CpuModeMonitor(terminal,&vs[0],param,waittime,CreatePatFileName("monitorvoltage_at"),"MONITOR_Tap_dependency",&selvol);
          printf("Monitor : %d[mV]",selvol);
          fprintf(fpt,"ETLR%02d:%02X, H'%02X, %d,[mV]",change_reg,GetValue(change_reg,param),reg_code[i],selvol);
        }
      }
    }
...
int CurrentMenu(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param){
  ...
  while( test_cond != EXIT){
    ...
    case QUIT:
      ...
      //-- Flash : Power Off --//
      case 5:
      //-- Flash : Read-Idle (Chip : SLEEP) --//
      case 6:
      //-- Flash : Power Off (Chip : SLEEP) --//
        stack0 = SetValue(IN_BDATA00,param,test_cond - 1);
        stack1 = SetValue(IN_BDATA01,param,1);
        //d]̂ߖ[v
      ...
      break;
      case 7:
      //-- Flash : LP-Mode(Chip:Sub-OSC) --//
      SetValue(IN_BDATA00,param,2);
      // Flash ON
      SetValue(IN_BDATA01,param,1);
      // Loop Mode ON
      SetValue(IN_EXTAL1,param,31*1000);
      // 32KHz
      ...
      case 50:
        ...
        result = CpuModeMonitor_current2((ADC_AVE
        // | PIN_JUMP
        if(result != PASS)break;
		printf("PE Current %d[nA]",selvol);
      ...
      case 250:
        ...
        result = CpuModeMonitor_current2_First(0,MVCCMON,&vs[0],param);
        if(result != PASS)break;
        result = CpuModeMonitor_current2((ADC_DIGI | CRY_CLK
        // | PIN_JUMP
        if(result != PASS)break;
		printf("PE Current %d[nA]",selvol);
        SetValue(IN_TESTSEL,param,stack0);
	...
	stack3 = SetValue(IN_ETCR11,param,GetValue(IN_ETCR11,param) | 0x02);
        ...
        for(k=1;k<=3;k++){
          ...
          if( NULL==( Fptdata = fopen( buff, "at" ) ) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", buff ); exit( -99 ); } delay_timer(5*1000);
...
int BGRMenu(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param){
  ...
  while( test_cond != EXIT){
    ...
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|BGR Status| Extra5 BGR Code %s(bgr)",bgr_status);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("| Sample   | L# : %s  W# : %3d",lotname,atoi(wafername));
    printf("| Info     |  X : %3d      Y : %3d",xy_location[0],xy_location[1]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("| Rext WT  | WT1 Pass Code : %2X",info[13]);
    printf("|          | WT2 Pass Code : %2X",info[14]);
    printf("|          | WT3 Pass Code : %2X",info[15]);
    printf("|          | WT4 Pass Code : %2X",info[16]);
    printf("|          | WT5 Pass Code : %2X",info[17]);
    printf("|          | WT6 Pass Code : %2X",info[18]);
    printf("|          | WT8 Pass Code : %2X",info[19]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Extra1  | ETLR00 : 0x%X   ETLR01 : 0x%X ",extra1_etlr00,extra1_etlr01);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Extra5  |  Vcc    ETLR00  ETLR01");
    printf("| BGR Trim | %4dmV   0x%X    0x%X ",VCC_ARRAY0,Extra5_ETLR00[0],Extra5_ETLR01[0]);
    printf("|(Temp=90C)| %4dmV   0x%X    0x%X ",VCC_ARRAY1,Extra5_ETLR00[1],Extra5_ETLR01[1]);
    printf("|          | %4dmV   0x%X    0x%X ",VCC_ARRAY2,Extra5_ETLR00[2],Extra5_ETLR01[2]);
    printf("|          | %4dmV   0x%X    0x%X ",VCC_ARRAY3,Extra5_ETLR00[3],Extra5_ETLR01[3]);
    printf("|          | %4dmV   0x%X    0x%X ",VCC_ARRAY4,Extra5_ETLR00[4],Extra5_ETLR01[4]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    if( (vs[0] != VCC_ARRAY0) && (vs[0] != VCC_ARRAY1) && (vs[0] != VCC_ARRAY2) && (vs[0] != VCC_ARRAY3) && (vs[0] != VCC_ARRAY4) ){
      printf("|   VCC    | VCC Not Correct!! (vcc)");
    }else{
      printf("|   VCC    | VCC = %d mV(vcc) ",vs[0]);
    }
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("| ETLR00-01| ETLR00 : 0x%X (0)  ETLR01 : 0x%X (1)",GetValue(IN_ETLR00,param),GetValue(IN_ETLR01,param));
    printf("+----------+------------------------------------------------------------------------------------------+");
    ...
    switch(test_cond){
      ...
      case QUIT:
        ...
        break;
	  case 0:
		printf("ETLR00 = 0x%X",GetValue(IN_ETLR00,param));
		printf(" Enter New Code -> ");DigitInput(&tempo1);
		SetValue(IN_ETLR00,param,tempo1);
	  break;
	  case 1:
		printf("ETLR01 = 0x%X",GetValue(IN_ETLR01,param));
		printf(" Enter New Code -> ");DigitInput(&tempo1);
	...
	switch(vs[0]){
	  ...
	  default:
	    printf(" Vcc Not Correct!!! Please Change Vcc Level!!!");
	    result = FAIL;
	  break;
	}
	if(result == PASS){
	  ...
	  printf("");
          ...
          printf("    *** Vcc = %4d mV  ***      ",vs[0]);
          printf("    Before         After       ");
	  printf("  ETLR00=0x%X -> ETLR00=0x%X   ",Extra5_ETLR00[tempo1],GetValue(IN_ETLR00,param));
	  printf("  ETLR01=0x%X -> ETLR01=0x%X   ",Extra5_ETLR01[tempo1],GetValue(IN_ETLR01,param));
	  printf("===============================");
	...
	if(result == PASS){
      ...
      break;
	  // case 200: ParamInitialize(param_ini); BGR_WT_HT(param_ini); break;
      case 300:
	...
	printf("Result = 0x%X",(GetValue(OUT_ETLR00,param) & 0x7F));
	...
	SetValue(IN_TESTSEL,param,0);
	 ...
	 for(k=1;k<=2;k++){
	    ...
	    monitor_pin = VRSGMON_APP;
          break;
	  // case 3: sprintf(testname,"Vreg20_Mode"); tempo1=2; loop_max = 2300; loop_min = 1800; loop_delta = 2; monitor_pin = VSSMON_APP; break;
	}
	...
	repeat_mode = OUTIF_READ | RAMBOOT_CHANGE;
	  ...
	  tempo3 = (GetValue(OUT_ETLR00,param) & 0x7F);
	  // tempo3 =result
	  printf("%s  Loop = %d  Result = 0x%X",testname,loop,tempo3);
	  ...
	  if(result == PASS){
	    fprintf(Fptdata,"%d,%X,%X",loop,GetValue(OUT_ETLR00,param),GetValue(OUT_ETLR01,param));
          }else{
	...
	SetValue(IN_LDATA0,param,100);
	    ...
	    monitor_pin = VRSGMON_APP;
          break;
	  // case 2: sprintf(testname,"Vreg20_Mode"); monitor_pin = VSSMON_APP; break;
	}
...
int CurrentAllDataOut(int mode, int vs[NUM_POWERSUPPLY], int Temp, interface_t* param, char *file_name_part)
{
  ...
  switch(get_site_number()){
    case 0: freq_num =  136; break;
    // 1.36MHz
    case 1: freq_num =  800; break;
    // 8.00MHz
    case 2: freq_num = 1300; break;
    //13.00MHz
    case 3: freq_num = 2000; break;
    //20.00MHz
  }
  ...
  switch(mode){
     case 0:
                // ********  Current Mesure  ******* //
     // Readd̎擾RgAEg ChangeAddress(FLP0,0x00000,0xFFF,param);  SetValue(IN_TESTSEL,param,0); DeviceSpecificPowerUp(); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*20,param,CreatePatFileName("programall_code_at"),"Program ALL"); if(result !=PASS )return(FAIL);  MinoriRead_Kani(WHILE1_ALL0 | READ_ON , &vs[0], param, &current); current_data[2] = current; MinoriRead_Kani(WHILE1_ALL0 | READ_OFF, &vs[0], param, &current); current_data[3] = current;  ChangeAddress(FLP0,0x00000,0x7FFFF,param);  SetValue(IN_BDATA00,param,0); DeviceSpecificPowerUp(); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*10,param,CreatePatFileName("erase_code_at"),"Erase"); if(result !=PASS )return(FAIL);  MinoriRead_Kani(WHILE1_ALL1 | READ_ON , &vs[0], param, &current); current_data[0] = current; MinoriRead_Kani(WHILE1_ALL1 | READ_OFF, &vs[0], param, &current); current_data[1] = current;  ChangeAddress(FLP0,0x00000,0x7FFFF,param);  SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0); DeviceSpecificPowerUp(); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*20,param,CreatePatFileName("programchecker_code_at"),"Program CHECKER"); if(result !=PASS )return(FAIL);  MinoriRead_Kani(NORMAL_READ | READ_ON , &vs[0], param, &current); current_data[4] = current; MinoriRead_Kani(NORMAL_READ | READ_OFF, &vs[0], param, &current); current_data[5] = current;      // ********  File Out  ******* // if( NULL==( Fptdata = fopen( buff, "at" ) ) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", buff ); exit( -99 ); } delay_timer(5*1000); fprintf(Fptdata,"Vcc[mV],%d,Vddh[mV],%d,Vdd[mV],%d",vs[0],vs[1]*25/10,vs[2]*79/50); fprintf(Fptdata,"Temp,%d,Freq[MHz],%d.%d",Temp,freq_num/100,freq_num%100); fprintf(Fptdata,",While1_ALL0_ON ,%d",current_data[0]); fprintf(Fptdata,",While1_ALL0_OFF,%d",current_data[1]); fprintf(Fptdata,",While1_ALL1_ON ,%d",current_data[2]); fprintf(Fptdata,",While1_ALL1_OFF,%d",current_data[3]); fprintf(Fptdata,",Normal_CHKA_ON ,%d",current_data[4]); fprintf(Fptdata,",Normal_CHKA_OFF,%d",current_data[5]); fprintf(Fptdata,",Ramdom_CHKA_ON ,%d",current_data[6]); fprintf(Fptdata,",Ramdom_CHKA_OFF,%d",current_data[7]); fprintf(Fptdata,","); delay_timer(5*1000); if( fclose( Fptdata ) ){ printf( "CloseDataOutFile Fptdata Close Err" ); exit( -99 ); } delay_timer(200); break;
     case 1:
        ...
        // 0x00 pass Only
        SetValue(IN_BDATA00,param,0);
        CpuModeMonitor_current2((ADC_AVE | CRY_CLK),MNORMAL,&vs[0],200,param,CreatePatFileName("dc_current_at"),"Read-Idle-Current",&current);
        // t@[ŕύX\
        ...
        SetValue(IN_BDATA00,param,1);
        DeviceSpecificPowerUp();
	CpuModeMonitor_current2((ADC_AVE | CRY_CLK),MNORMAL,&vs[0],200,param,CreatePatFileName("dc_current_at"),"PowerOff-Current",&current);
	// t@[ŕύX\
     ...
     case 2:
        ...
        // 0x00 pass Only
        SetValue(IN_BDATA00,param,2);
        CpuModeMonitor_current2((ADC_AVE),MNORMAL,&vs[0],200,param,CreatePatFileName("dc_current_at"),"Flash-STBY-Current",&current);
        // t@[ŕύX\
        current_data[0]=current;
        ...
        SetValue(IN_BDATA00,param,3);
        DeviceSpecificPowerUp();
	CpuModeMonitor_current2((ADC_AVE),MNORMAL,&vs[0],200,param,CreatePatFileName("dc_current_at"),"SSTBY2-PowerOFF-Current",&current);
	// t@[ŕύX\
        current_data[1]=current;
     ...
     case 3:
        ...
        result = CpuModeMonitor_current2(0,MVCCMON,&vs[0],param,250,CreatePatFileName("programall_code_at_current"),"PE_Bottom_Dummy",&current);
        // t@[ŕύX\
        if(result != PASS)return(FAIL);
        ...
        ffpt_bak=Ffpt;
        Ffpt=0;
          result = CpuModeMonitor_current2(0,MVCCMON,&vs[0],param,300,CreatePatFileName(pat_name),"PE_Bottom",&current);
          // t@[ŕύX\
        Ffpt = ffpt_bak;
          if(result != PASS)break;
...
sprintf(filename2,"%s_W%d_X%dY%d_",lotname,atoi(wafername),xy_location[0],xy_location[1]);
  ...
  fprintf(fpt,"MaxVol=%d[mV],MinVol=%d[mV]",max_vol,min_vol);
  fprintf(fpt,"ETLR%02d:%02X,chargepump[uA] (Vcc:%d Vddh:%d Vdd:%d)",change_reg,GetValue(change_reg,param),vs[0],vs[1],vs[2]);
    ...
    CpuModeMonitor(terminal,&vs[0],param,W1SEC*waittime,CreatePatFileName("monitorvoltage_at"),"CP_Voltage_dependency",&selvol);
  	 ...
  	 //UA25
  	 	 if(-24000 < selvol && selvol <= 24000){
  	 	   fprintf(fpt,"TargetVoltage:%d[mV], %d,[nA]",target_vol[j],selvol);
...
int CPvsVol(int mode,int vs[NUM_POWERSUPPLY], int waittime, interface_t* param)
{
  ...
  printf("xxxxxxxxxxxxxxxxxxxxx  BDATA00:%08X, LDATA0:%08X",GetValue(IN_BDATA00,param),GetValue(IN_LDATA0,param));
  pmu_range = GetValue(IN_BDATA15,param);
...
// -----------------------------------------------------
  ...
  fprintf(fpt,"Start_volt=%d[mV],StepVoltage=%d[mV],MesureNumber=%d",start_volt,step_volt,meascon);
  fprintf(fpt,"ETLR%02d:%02X,chargepump[uA] (Vcc:%d Vddh:%d Vdd:%d)",change_reg,GetValue(change_reg,param),vs[0],vs[1],vs[2]);

      next_volt =  start_volt;
      //Initial force voltage//
	
  for ( j = 0 ; j<meascon+1 ; j++ ){
    if(next_volt > 10000) break;
    //10V over Exit//
    if(next_volt < -5000) break;
    //-5V over Exit//
    selvol =  next_volt;
    //Extarnal force voltage//

    DeviceSpecificPowerUp();
    CpuModeMonitor(terminal,&vs[0],param,W1SEC*waittime,CreatePatFileName(pat_name),"CP_Voltage_dependency",&selvol);
  	 
  	if(pmu_range == UA2500){
  	  fprintf(fpt,"TargetVoltage:%d[mV], %d,[nA]",next_volt,selvol);
	  SetValue(IN_BDATA15,param,UA250);
	  pmu_range = GetValue(IN_BDATA15,param);
  	}else if(pmu_range == UA250){
...
int VrsgMonitor_Repeat(interface_t* param, int Vresult[BGR_ARRAY_VCC])
{
  ...
  char moni_pin[20];
	...
	MinoriMonitor_Vrsg_First(MVRSGMON,&vs[0],waittime,param,"USER_ALL0_M0_MULTISCANI_XS","MINORI Vrsg Monitor",&selvol);
	  ...
	  Ffpt = ffpt_bak;
	  	  //if(result != PASS) break;  //Pass or Fail
		//printf("VrsgMonitor_Repeat : selvol = %dmV",selvol);
	  vrsg_buff[i]=selvol;
        }
        result = VrsgAnalyze(&vrsg_buff[0], &Vanalyze);
				//printf("VrsgMonitor_Repeat : Vanalyze = %dmV",Vanalyze);
  ...
  int vcc_array[BGR_ARRAY_VCC],vs[NUM_POWERSUPPLY],vrsg_buff[BGR_ARRAY_REPEAT],Vanalyze;
	...
	if(result == PASS){
	  ...
	  *Vresult = 0;
      ...
      printf("VrsgMoni Vcc:%dmV - Result %dmV",vs[0],Vanalyze);
      DeviceSpecificPowerDown();
      DeviceLevelsPowerDown();
  ...
  char moni_pin[20];
    ...
    stack3 = SetValue(IN_BDATA01,param,0);
      ...
      vs[0] = vcc_array[vcc_num];
		...
		//  ************** Auto Trim **************  //
	printf("BgrTrim Vcc:%dmV - Target %dmV",vs[0],Target);
        ffpt_bak = Ffpt;
	Ffpt = 0;
        DeviceSpecificPowerUp();
...
int Trim_VddVddh(int mode, interface_t* param)
{
  ...
  int first_flag,repeat_mode,inif_flag;
    ...
    for(i=0;i<=14;i++){
	  ...
	  printf("  ETLR01:0x%X, ETLR02:0x%X, ETLR03:0x%X, selvol=%dmV",GetValue(IN_ETLR01,param),GetValue(IN_ETLR02,param),GetValue(IN_ETLR03,param),selvol);
      
      if( ((target - delta) <= selvol) && (selvol <= (target + delta))){
		printf("Vdd/Vddh Trimming ***PASS***");break;
      }else if(((target - delta*3) > selvol)){
		i++;
      }else if(((target + delta) < selvol)){
		printf("Vdd/Vddh Trimming ---FAIL---");break;
		result = FAIL;
      }
	}
    Ffpt=1;
...
int Iref_Trim_ALL(int mode, interface_t* param)
{
  ...
  for(etlr12_74 = 0; etlr12_74 <= 15; etlr12_74++){
    for(etlr12_30 = 0; etlr12_30 <= 15; etlr12_30 = etlr12_30 +15){
      ...
      printf("Iref Monitor = %dnA",selvol);
  ...
  fprintf(Fptdata,"ETLR12[7:4],%d",etlr12_74);
  fprintf(Fptdata,"ETLR09[7:0],0x%X,%d,",GetValue(IN_ETLR09,param),GetValue(IN_ETLR09,param)&0x7F);
  fprintf(Fptdata,"Iref(temp)[nA],0");
  fprintf(Fptdata," ,ETLR12_30 = 0,ETLR12_30 = 15,");
  for(i=0;i<=15;i++){
    fprintf(Fptdata,"ETLR12_74 = %d ,%d,%d,",i,Iref_moni[i][0],Iref_moni[i][15]);
  ...
  etlr12_74 = etlr12_74 -1;
  //TapɈړĂ̂Ŗ߂ĂB
  SetValue(IN_ETLR12,param,((stack1 & 0x0F) | (etlr12_74 << 4)));
  printf("*** Trimming PASS ***  ETLR12[7:4] = 0x%X",etlr12_74);
  return(result);
}
...
int Iref_Temp_moni_ALL(int mode, interface_t* param)
{
  ...
  for(etlr09_60 = 0; etlr09_60 <= 127; etlr09_60++){
    for(etlr12_74 = 0; etlr12_74 <= 15; etlr12_74++){
      for(etlr12_30 = 0; etlr12_30 <= 15; etlr12_30++){
        ...
        printf("etlr09_60 %2X, etlr12_74 %X, etlr12_30 %X,Iref Monitor = %dnA",etlr09_60,etlr12_74,etlr12_30,selvol);

        Iref_moni[etlr09_60][etlr12_74][etlr12_30] = selvol;
        Iref_pass[etlr09_60][etlr12_74][etlr12_30] = 0;
        //ʓ|Ȃ̂ŁA0PassƂB

        if(result != PASS){
          Iref_pass[etlr09_60][etlr12_74][etlr12_30] = 1;
          //ʓ|Ȃ̂ŁA1FAILƂB
        }
  ...
  for(etlr09_60 = 0; etlr09_60 <= 127; etlr09_60++){
    for(etlr12_74 = 0; etlr12_74 <= 15; etlr12_74++){
      for(etlr12_30 = 0; etlr12_30 <= 15; etlr12_30++){
        fprintf(Fptdata,"%d,%d,%d,%d,%d,",etlr09_60,etlr12_74,etlr12_30,Iref_moni[etlr09_60][etlr12_74][etlr12_30],Iref_pass[etlr09_60][etlr12_74][etlr12_30]);
      }
    }
  }
...
int Iref_Trim_forRext(int mode, interface_t* param)
{
  ...
  for(etlr12_74 = 15; etlr12_74 >= 0; etlr12_74-- ){
    printf("Result = %d nA",Iref_moni[etlr12_74][etlr12_30]);
  ...
  fprintf(Fptdata,"ETLR12[7:4],%d",etlr12_74);
  fprintf(Fptdata,"ETLR09[7:0],0x%X,%d,",GetValue(IN_ETLR09,param),GetValue(IN_ETLR09,param)&0x7F);
  fprintf(Fptdata,"Iref(temp)[nA],%d",Iref_result);
  fprintf(Fptdata,"monitor_Data,ETLR12_30 = 15,");
  for(i=0;i<=15;i++){
    fprintf(Fptdata,"ETLR12_74 = %d ,%d,",i,Iref_moni[i][etlr12_30]);
  }
  CloseDataOutFile();
  ...
  SetValue(IN_ETLR12,param,((stack1 & 0x0F) | (etlr12_74 << 4)));
  printf("*** Trimming PASS ***  ETLR12[7:4] = 0x%X  ETLR09 = %X",etlr12_74,GetValue(IN_ETLR09,param));
  return(PASS);
}
...
int Shmoo_LPRead(int mode, interface_t* param)
{
  ...
  default:
    ...
    for(loop_mode = 1; loop_mode <= 4; loop_mode++){
      ...
      fprintf(Fptdata,"Initial patten,%s vcc,%d,vddh,%d,vdd,%d",wgl_name,vs[0],vs[1],vs[2]);
    ...
    if( NULL==(file_log = fopen( file_name, "at" )) ){
      ...
      fclose( file_log );
        ...
        for ( j=0 ; j<loop_num ; j++ ) {
          CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Program ALL");
          // t@[ŕύX\
        }
        SetValue(IN_TESTSEL,param,0);
        SetValue(IN_BDATA00,param,0);
...
// 1=ALL Pattern vth_mode = 1; result = VthReadDist(vth_mode, bit_count, min_vol, tempo2, &vs[0],W1SEC*waittime, param, item_name); SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA03,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  if(PASS != result){ sprintf(outbuf," Make Vth Bunpu Error!!!"); DataOut(Ffpt,outbuf); return(PASS); } } SetValue(IN_ETCR08,param,etcr8);  time( &time_list2 ); // time_list += V4V5K_TIME_OFS; printf("Test-Time:%d[s]",time_list2-time_list); printf("");   SetValue(IN_AREA,param,stack0); SetValue(IN_TOPADDR0 ,param,stack1); SetValue(IN_TESTSIZE0,param,stack2); SetValue(IN_TOPADDR1 ,param,stack3); SetValue(IN_TESTSIZE1,param,stack4); SetValue(IN_TOPADDR2 ,param,stack5); SetValue(IN_TESTSIZE2,param,stack6); SetValue(IN_EXTAL1,param,stack7); SetValue(IN_REG,param,stack8); SetValue(IN_TESTSEL,param,0); SetValue(IN_WRITESIZE,param,0);   }  int Erase_VthvsIref(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param) { int tempo,tempo0,tempo1,tempo2,tempo3,tempo4,tempo5,tempo6; int stack0,stack1,stack2,stack3,stack4,stack5,stack6,stack7,stack8; int etcr8; int min_vol,result,vth_mode,bit_count,selvol; int i,j,l,mat,cond1,cond2,cond3,adr_mode,adr_st,adr_end; int loop_num; char file_name[128]; char item_name[128]; char sample_name[128]; char test_name[128]; char test_item[128]; FILE_T *file_log;  cond1 = cond2 = cond3 = 0;  stack0 = GetValue(IN_AREA,param); stack1 = GetValue(IN_TOPADDR0,param); stack2 = GetValue(IN_TESTSIZE0,param); stack3 = GetValue(IN_TOPADDR1,param); stack4 = GetValue(IN_TESTSIZE1,param); stack5 = GetValue(IN_TOPADDR2,param); stack6 = GetValue(IN_TESTSIZE2,param); stack7 = GetValue(IN_EXTAL1,param); stack8 = GetValue(IN_REG,param);     printf("Input Tempurature -40C(-40) / 25C(25) / 95C(95) ->");DigitInput(&tempo2); sprintf(sample_name,"%s_W%d_S%d",lotname,atoi(wafername),SampleNo); sprintf(test_name,"Iref_VthRead_%dC",tempo2); sprintf(item_name,"%s_%s",test_name,sample_name);  sprintf(file_name,"%s%s%s_%d.csv", glob_cdp, glob_datalog, item_name, get_site_number());  time( &time_list ); // time_list += V4V5K_TIME_OFS;     SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,0); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0x20000);  if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } fprintf(file_log,"<<Iref vs Vthread>> ETLR25=0x%X,Vcc=%d,Distribution:0V~0.05VStep,Mat=%d(0x%X~0x%X)",GetValue(IN_ETLR25,param),vs[0],mat,GetValue(IN_TOPADDR0 ,param),GetValue(IN_TESTSIZE0 ,param)-1); fprintf(file_log,""); fclose( file_log ); SetValue(IN_ETLR09,param,( GetValue(IN_ETLR09,param) & 0x7F ) | 0x80); //Off for ( i=1 ; i<=5 ; i++ ) { if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } switch (i){ case 1: fprintf(file_log,"Iref_5.0uA"); SetValue(IN_ETLR25,param,( GetValue(IN_ETLR25,param) & 0xF0 ) | 0x00); break; case 2: fprintf(file_log,"Iref_3.5uA"); SetValue(IN_ETLR25,param,( GetValue(IN_ETLR25,param) & 0xF0 ) | 0x0F); break; case 3: fprintf(file_log,"Iref_2.5uA"); SetValue(IN_ETLR25,param,( GetValue(IN_ETLR25,param) & 0xF0 ) | 0x0D); break; case 4: fprintf(file_log,"Iref_2.0uA"); SetValue(IN_ETLR25,param,( GetValue(IN_ETLR25,param) & 0xF0 ) | 0x0A); break; case 5: fprintf(file_log,"Iref_1.5uA"); SetValue(IN_ETLR25,param,( GetValue(IN_ETLR25,param) & 0xF0 ) | 0x0B); break; }  SetValue(IN_EXTAL1,param,31); SetValue(IN_TESTSEL,param,MONITOR_IMONI_FLASH_NOTEMP); SetValue(IN_BDATA15,param,UA25); result = CpuModeMonitor(VSSMON_MON,&vs[0],param,W1SEC*waittime,CreatePatFileName("monitorcurrent_at"),"MONITOR_I_IMONI_FLASH_NOTEMP",&selvol); if( selvol>=24000 ){ SetValue(IN_BDATA15,param,UA250); result = CpuModeMonitor(VSSMON_MON,&vs[0],param,W1SEC*waittime,CreatePatFileName("monitorcurrent_at"),"MONITOR_I_IMONI_FLASH_NOTEMP",&selvol); } fprintf(file_log,",%d",selvol/3);  SetValue(IN_BDATA15,param,0); fclose( file_log );  SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0);  SetValue(IN_BDATA01,param,0); // ALL Count SetValue(IN_BDATA02,param,99); // Log Headder SetValue(IN_BDATA04,param,1); // Log Output -> DATALOG SetValue(IN_BDATA05,param,1); // Vth-Level-Log:OFF SetValue(IN_EXTAL1,param,VTH_FREQ); min_vol = -2000; tempo2 = 50; bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern vth_mode = 1; result = VthReadDist(vth_mode, bit_count, min_vol, tempo2, &vs[0],W1SEC*waittime, param, item_name); SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA03,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  if(PASS != result){ sprintf(outbuf," Make Vth Bunpu Error!!!"); DataOut(Ffpt,outbuf); return(PASS); } }  time( &time_list2 ); // time_list += V4V5K_TIME_OFS; printf("Test-Time:%d[s]",time_list2-time_list); printf("");   SetValue(IN_AREA,param,stack0); SetValue(IN_TOPADDR0 ,param,stack1); SetValue(IN_TESTSIZE0,param,stack2); SetValue(IN_TOPADDR1 ,param,stack3); SetValue(IN_TESTSIZE1,param,stack4); SetValue(IN_TOPADDR2 ,param,stack5); SetValue(IN_TESTSIZE2,param,stack6); SetValue(IN_EXTAL1,param,stack7); SetValue(IN_REG,param,stack8); SetValue(IN_TESTSEL,param,0); SetValue(IN_WRITESIZE,param,0);   }  int Extra_VthRead(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param,int mode){ int tempo,tempo0,tempo1,tempo2,tempo3,tempo4,tempo5,tempo6; int stack0,stack1,stack2,stack3,stack4,stack5,stack6,stack7,stack8; int etcr8; int min_vol,result,vth_mode,bit_count,selvol; int i,j,l,mat,cond1,cond2,cond3,adr_mode,adr_st,adr_end; int loop_num; char file_name[128]; char item_name[128]; char sample_name[128]; char test_name[128]; char test_item[128]; FILE_T *file_log;  cond1 = cond2 = cond3 = 0;  stack0 = GetValue(IN_AREA,param); stack1 = GetValue(IN_TOPADDR0,param); stack2 = GetValue(IN_TESTSIZE0,param); stack3 = GetValue(IN_TOPADDR1,param); stack4 = GetValue(IN_TESTSIZE1,param); stack5 = GetValue(IN_TOPADDR2,param); stack6 = GetValue(IN_TESTSIZE2,param); stack7 = GetValue(IN_EXTAL1,param); stack8 = GetValue(IN_REG,param);     printf("Input Retention Time [hr] (ex.100hr [100]) ->");DigitInput(&tempo2); sprintf(sample_name,"%s_W%d_S%d",lotname,atoi(wafername),SampleNo); sprintf(test_name,"%dhr",tempo2); sprintf(item_name,"%s_VthRead",sample_name);  sprintf(file_name,"%s%s%s%s_%d.csv", glob_cdp, glob_bunpu, glob_Extralog, item_name, get_site_number());  time( &time_list ); // time_list += V4V5K_TIME_OFS;     SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,3); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0x400); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0);   //File Hedder & File search if( NULL==(file_log = fopen( file_name, "rt" )) ){ if(mode != 0){ printf(" File Open Error!!! [%s] ",file_name); return(FAIL); } if( NULL==(file_log = fopen( file_name, "at" )) ){ fprintf(file_log,"<<Extra3 Vthread>> ETLR25=0x%X,Vcc=%d,Distribution:0V~0.05VStep,Mat=%d(0x%X~0x%X)",GetValue(IN_ETLR25,param),vs[0],mat,GetValue(IN_TOPADDR0 ,param),GetValue(IN_TESTSIZE0 ,param)-1); fprintf(file_log,""); fclose( file_log ); } }   // Retention Time Out if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } fprintf(file_log,"%s",test_name); fclose( file_log );   //Mode:0 => Program-Erase_x2 if(mode == 0){ SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0); SetValue(IN_EXTAL1,param,31); CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("programall_code_at"),"Program ALL"); for(i=0;i<2;i++){ CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Erase"); SetValue(IN_BDATA00,param,1); //PewWrite Skip } }   SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0); SetValue(IN_BDATA01,param,0); // ALL Count SetValue(IN_BDATA02,param,99); // Log Headder SetValue(IN_BDATA04,param,3); // Log Output -> Bunpu/Extra SetValue(IN_BDATA05,param,1); // Vth-Level-Log:OFF SetValue(IN_EXTAL1,param,VTH_FREQ); min_vol = -2000; tempo2 = 50; bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern vth_mode = 1; result = VthReadDist(vth_mode, bit_count, min_vol, tempo2, &vs[0],W1SEC*waittime, param, item_name); SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA03,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  if(PASS != result){ sprintf(outbuf," Make Vth Bunpu Error!!!"); DataOut(Ffpt,outbuf); return(PASS); }  time( &time_list2 ); // time_list += V4V5K_TIME_OFS; printf("Test-Time:%d[s]",time_list2-time_list);   SetValue(IN_AREA,param,stack0); SetValue(IN_TOPADDR0 ,param,stack1); SetValue(IN_TESTSIZE0,param,stack2); SetValue(IN_TOPADDR1 ,param,stack3); SetValue(IN_TESTSIZE1,param,stack4); SetValue(IN_TOPADDR2 ,param,stack5); SetValue(IN_TESTSIZE2,param,stack6); SetValue(IN_EXTAL1,param,stack7); SetValue(IN_REG,param,stack8); SetValue(IN_TESTSEL,param,0); SetValue(IN_WRITESIZE,param,0);  return(PASS);  }  int VppTf_vs_Tap(int vs[NUM_POWERSUPPLY],interface_t* param,int mode,int Temprature){ int tempo,tempo0,tempo1,tempo2,tempo3,tempo4,tempo5,tempo6; int stack0,stack1,stack2,stack3,stack4,stack5,stack6,stack7,stack8; int selvol,etlr15_20,trtf,etlr20_30,vpp; int result; int i,j,l,mat,cond1,cond2,cond3,adr_mode,adr_st,adr_end; int loop_num; char file_name[128]; char item_name[128]; char sample_name[128]; char test_name[128]; char test_item[128]; char trtf_name[30]; FILE_T *file_log;  cond1 = cond2 = cond3 = 0;  SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,0); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0x100); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0);       sprintf(sample_name,"%s_W%d_S%d",lotname,atoi(wafername),SampleNo); sprintf(item_name,"%s_TrTfMonitor_%dC",sample_name,Temprature);  sprintf(file_name,"%s%s%s_%d.csv", glob_cdp, glob_datalog, item_name, get_site_number());  time( &time_list ); // time_list += V4V5K_TIME_OFS;    //Monitor Log Out if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } fprintf(file_log,"Vpp-Tf vs Tap Result"); fprintf(file_log,"Vcc = %d , Vddh = %d , Vdd = %d ,",vs[0],vs[1],vs[2]); fprintf(file_log,"Vpp,"); fclose( file_log );   Ffpt = 0; for(trtf = 0; trtf<=8; trtf++){ for(vpp = 1; vpp<=5;vpp++){ printf("******* TrTf = %d , Vpp = %d ********",trtf,vpp); switch (vpp){ case 1 : etlr20_30 = 0;break; case 2 : etlr20_30 = 4;break; case 3 : etlr20_30 = 9;break; case 4 : etlr20_30 = 12;break; case 5 : etlr20_30 = 15;break; } SetValue(IN_ETLR20,param, (GetValue(IN_ETLR20,param) & 0xF0 ) | etlr20_30 );  if(trtf == 0 ){ SetValue(IN_TESTSEL,param,MONITOR_VPP_P); SetValue(IN_BDATA00,param,0); CpuModeMonitor(VSSMON_MON,&vs[0],param,W1SEC*5,CreatePatFileName("monitorvoltage_at"),"MONITOR_V_VPP_P",&selvol); if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  fprintf(file_log,"%d,,,",selvol); fclose( file_log );  }else{   switch (trtf){ case 1 : etlr15_20 = 4; sprintf(trtf_name," 25,");break; case 2 : etlr15_20 = 5; sprintf(trtf_name," 50,");break; case 3 : etlr15_20 = 6; sprintf(trtf_name," 75,");break; case 4 : etlr15_20 = 7; sprintf(trtf_name,"100,");break; case 5 : etlr15_20 = 0; sprintf(trtf_name,"120,");break; case 6 : etlr15_20 = 1; sprintf(trtf_name,"150,");break; case 7 : etlr15_20 = 2; sprintf(trtf_name,"200,");break; case 8 : etlr15_20 = 3; sprintf(trtf_name,"300,");break; } SetValue(IN_ETLR15,param, (GetValue(IN_ETLR15,param) & 0xF8 ) | etlr15_20 ); SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0); SetValue(IN_ETCR11,param,0);  CpuModeMonitor_VppTf(MNORMAL,&vs[0],param,W1SEC*3,CreatePatFileName("VppTf_Check"),"VppTf Monitor",&selvol); selvol = selvol * GetValue(IN_EXTAL1,param); if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } if(vpp==1)fprintf(file_log,"%s",trtf_name); fprintf(file_log,"%d.%d,",selvol/1000,(selvol/100)%10); if((GetValue(OUT_ETCR16,param) & 0x40) == 0x40){ fprintf(file_log,"FAIL,"); }else{ fprintf(file_log,"PASS,"); } fclose( file_log );  SetValue(IN_ETCR11,param,1); CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*5,param,CreatePatFileName("programall_code_at"),"Program ALL"); if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  if((GetValue(OUT_ETCR16,param) & 0x40) == 0x40){ fprintf(file_log,"FAIL,"); }else{ fprintf(file_log,"PASS,"); } fclose( file_log ); }  } if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); } fprintf(file_log,""); fclose( file_log ); } Ffpt =1; time( &time_list2 ); // time_list += V4V5K_TIME_OFS; printf("Test-Time:%d[s]",time_list2-time_list);  return(PASS);  }  int MonitorALL_Tap(int vs[NUM_POWERSUPPLY],interface_t* param,int mode,int Temprature){ int tempo,tempo0,tempo1,tempo2,tempo3,tempo4,tempo5,tempo6; int stack0,stack1,stack2,stack3,stack4,stack5,stack6,stack7,stack8; int selvol[260],etlr15_20,trtf,etlr20_30,vpp; int result; int i,j,l,mat,cond1,cond2,cond3,adr_mode,adr_st,adr_end; int loop_num; char file_name[128]; char item_name[128]; char sample_name[128]; char test_name[30]; char test_item[128]; char trtf_name[30]; FILE_T *file_log;  cond1 = cond2 = cond3 = 0; /* #define  MONITOR_VHH_W    1 #define  MONITOR_VHH_E    2 #define  MONITOR_VNOEMI   3 #define  MONITOR_VREG20   4 #define  MONITOR_VWI      5 #define  MONITOR_V33R     6 #define  MONITOR_VRSG     7 #define  MONITOR_VPP_W    8 #define  MONITOR_VPP_E    9 #define  MONITOR_VPP_PW   10 #define  MONITOR_VDEMG    11 #define  MONITOR_VREF10   12 #define  MONITOR_IREF     13 #define  MONITOR_IREF_T   14
  ...
  switch(mode){
    case V_VHH_P:
      ...
      SetValue(IN_TESTSEL,param,1);
      //Vhh_P̃j^[
      SetValue(IN_BDATA00,param,17);
      //ETLR17CNg
      SetValue(IN_BDATA01,param,0);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x3F);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
      //0x100ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0x100);
      //0x100ڂVSSMONɈڍs
    break;
    case V_VHH_E:
      sprintf(test_name,"Vhh_E");
      SetValue(IN_TESTSEL,param,2);
      //Vhh_Ẽj^[
      SetValue(IN_BDATA00,param,16);
      //ETLR16CNg
      SetValue(IN_BDATA01,param,0);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
    ...
    case V_VNOEMI_PE:
      ...
      SetValue(IN_TESTSEL,param,3);
      //Vnoemĩj^[
      SetValue(IN_BDATA00,param,19);
      //ETLR19CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0xF0);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x0F);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
      //0x100ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0x100);
    ...
    case V_VREG20:
      ...
      //Vreg20̃j^[
      SetValue(IN_BDATA00,param,23);
      //ETLR23CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0xFC);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x03);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
      //0x100ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0x100);
    ...
    case V_VWI:
      ...
      SetValue(IN_TESTSEL,param,5);
      //Vwĩj^[
      SetValue(IN_BDATA00,param,19);
      //ETLR19CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0x0F);
      //WX^l̏l
      SetValue(IN_BDATA03,param,0x10);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x0F);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
    ...
    case V_V33R:
      ...
      SetValue(IN_TESTSEL,param,6);
      //V33r̃j^[
      SetValue(IN_BDATA00,param,24);
      //ETLR24CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0xF8);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x07);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
      //0x100ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0x100);
    ...
    case V_VPP_P:
      ...
      SetValue(IN_TESTSEL,param,8);
      //Vpp_P̃j^[
      SetValue(IN_BDATA00,param,20);
      //ETLR20CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0xF0);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
    ...
    case V_VPP_PW:
      ...
      SetValue(IN_TESTSEL,param,10);
      //Vpp_PW̃j^[
      SetValue(IN_BDATA00,param,21);
      //ETLR21CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0x0F);
      //WX^l̏l
      SetValue(IN_BDATA03,param,0x10);
    ...
    break;
    case V_VDEMG:
      stack1 = SetValue(IN_ETLR20,param,0xFF);
      //VdemgVpp琶̂ߍőTapɂĂB
      sprintf(test_name,"Vdemg");
      SetValue(IN_TESTSEL,param,11);
      //Vdemg̃j^[
      SetValue(IN_BDATA00,param,21);
      //ETLR21CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0xF0);
    ...
    case V_VREF10:
      ...
      SetValue(IN_TESTSEL,param,12);
      //Vref10̃j^[
      SetValue(IN_BDATA00,param,11);
      //ETLR11CNg
      SetValue(IN_BDATA01,param,GetValue(IN_ETLR00 + GetValue(IN_BDATA00,param),param) & 0x0F);
      //WX^l̏l
      SetValue(IN_BDATA03,param,0x10);
    ...
    case I_IMONI_FLASH_NOTEMP:
      ...
      SetValue(IN_TESTSEL,param,13);
      //Iref̃j^[
      SetValue(IN_BDATA00,param,8);
      //ETLR8CNg
      SetValue(IN_BDATA01,param,0x80);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x7F);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0);
      //0ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0);
      //0ڂVSSMONɈڍs
      SetValue(IN_ETLR09,param,GetValue(IN_ETLR09,param)|0x80);
      //WX^l̏l
    break;
    case I_IMONI_FLASH_TEMP:
      sprintf(test_name,"IrefT");
      SetValue(IN_TESTSEL,param,14);
      //IrefT̃j^[
      SetValue(IN_BDATA00,param,9);
      //ETLR9CNg
      SetValue(IN_BDATA01,param,0x00);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0x7F);
      ...
      SetValue(IN_LDATA1,param,0);
      //0ڂVSSMONɈڍs
      SetValue(IN_ETLR08,param,GetValue(IN_ETLR08,param)|0x80);
      //WX^l̏l
    break;
    case V_VHH_E_TEMP:
      ...
      SetValue(IN_TESTSEL,param,2);
      //Vhh_Ẽj^[
      SetValue(IN_BDATA00,param,13);
      //ETLR13CNg
      SetValue(IN_BDATA01,param,0);
      //WX^l̏l
      SetValue(IN_BDATA03,param,1);
      //WX^l̃CNg
      SetValue(IN_BDATA05,param,0xFF);
      //WX^l̃CNg
      SetValue(IN_LDATA0,param,0x100);
      //0x100ڂœdj^[Ɉڍs
      SetValue(IN_LDATA1,param,0x100);
...
// ************************************************************************************
  ...
  fprintf(file_log,"%s Monitor vs Tap Result",test_name);
  fprintf(file_log,"Vcc = %d , Vddh = %d , Vdd = %d ,",vs[0],vs[1],vs[2]);
  fprintf(file_log,"ETLR%02d,%s",GetValue(IN_BDATA00,param),test_name);
  switch(mode){
  //WX^̕тԂłȂCaseɍ쐬B
    case V_VRSG:
      for(j=0;j<=GetValue(IN_BDATA05,param);j++){
        ...
        fprintf(file_log,"0x%02X,%d",GetValue(IN_BDATA01,param) + (GetValue(IN_BDATA03,param) * i) ,selvol[i]);
      }
    break;
    case V_VREF10:
      for(j=0;j<=GetValue(IN_BDATA05,param);j++){
...
int SinraiMenu(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param){
  ...
  while( test_cond != EXIT){
    ...
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("| Sample   | L# : %s  W# : %3d  S# : %05d",lotname,atoi(wafername),SampleNo);
    printf("| Info     |  X : %3d      Y : %3d",xy_location[0],xy_location[1]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("+----------+------------------------------------------------------------------------------------------+");
    ...
    case 9:
      ...
      break;
    case 10:
		Ret_Distribution_256KB(&vs[0],W1SEC*waittime,param);
		//Row_st
      break;
    case 11:
      ...
      break;
    case 12:
		Ret_Distribution_multi_area_rowst(&vs[0],W1SEC*waittime,param);
		//Row_st
      break;
    case 13:
		Ret_Distribution_multi_area_all01(&vs[0],W1SEC*waittime,param);
		//Row_st
      break;
...
void CPcurrent_ALL(int mode, int vs[NUM_POWERSUPPLY],int waittime,interface_t* param,int temprature){
  ...
  fprintf(file_log,"%s CPcurrent vs TestVoltage Result",mode_name);
  fprintf(file_log,"Vcc = %d , Vddh = %d , Vdd = %d ,",vs[0],vs[1],vs[2]);
  fprintf(file_log,"Voltage,%s",mode_name);
  for(i=0;i<voltage_step;i++){
    fprintf(file_log,"%d,%d",voltage_start + (voltage_delta * i),CpCurrent_Data[i]*voltage_minus);
  }
  fclose( file_log );
...
int ReadWindow_short(int mode, int vs[NUM_POWERSUPPLY],int waittime,interface_t* param,int temprature){
  ...
  vcc2_path = vcc2;
    ...
    etlr09 = GetValue(IN_ETLR09,param);
    // ireftmpI
    etlr12 = GetValue(IN_ETLR12,param);
    // ireftmp
	...
	SetValue(IN_TESTSIZE0,param,0x19000);
	// 4kBx25=100kB
  ...
  SetValue(IN_BDATA02,param,(GetValue(IN_ETLR12,param) & 0xF0) | 0x8);
  //0x8-0xF

  SetValue(IN_ETLR09,param,(etlr09 & 0x7F));
  // ireftemp On etlr09<7>=0 //

  printf("*** temparature? (ex:-40->40 /, 25-> 25/, 95-> 95) ->");DigitInput(&tempo2);
  temprature=tempo2;
  ...
  if(addr > (FBM_READTOP + 0x1000)){
    printf("Error!!! ReadCountData Over FBMSIZE 0x1000 < 0x%X",addr);
    return(FAIL);
  }
  ...
  for(i=0;i<GetValue(IN_BDATA00,param);i++){
    ...
    for(j=0;j<GetValue(IN_BDATA01,param);j++){
            //File hedder
      fprintf(file_log,"TOPADDR = %d",GetValue(IN_TOPADDR0,param));
      fprintf(file_log,"TESTSIZE = %d",GetValue(IN_TESTSIZE0,param));
      fprintf(file_log,"Vcc=%d,Vddh=%d,Vdd=%d,",vcc0[j],vcc1[j],vcc2[j]);
      fprintf(file_log,"Iref=%d,Ireftmp=%d,",IrefResult[0],IrefResult[i+1]);
      fprintf(file_log,"ETLR09=%02X,ETLR12=%02X,ETLR25=%02X,ETLR26=%02X,ETLR27=%02X,", GetValue(IN_ETLR09,param),GetValue(IN_BDATA02,param)+i,GetValue(IN_ETLR25,param),GetValue(IN_ETLR26,param),GetValue(IN_ETLR27,param));
      fprintf(file_log,"ETLR28=%02X,ETLR29=%02X,ETLR30=%02X,ETLR32=%02X,ETLR33=%02X,ETLR34=%02X,", GetValue(IN_ETLR28,param),GetValue(IN_ETLR29,param),GetValue(IN_ETLR30,param),GetValue(IN_ETLR32,param),GetValue(IN_ETLR33,param),GetValue(IN_ETLR34,param));
      fprintf(file_log,"");
      ...
      NoNum = 0;
      for(k = 0;k < (GetValue(IN_TESTSIZE0,param)/GetValue(IN_LDATA0,param));k++){
        fprintf(file_log,"%d,%d,%d",NoNum,0,CountData[j][i][k]);
        NoNum++;
      }
      fprintf(file_log,"");
    }
  ...
  if((file_log=fopen(file_name,"rt")) == NULL) {
    ...
    if((file_log=fopen(file_name,"at")) == NULL) {
      printf(" -- FAIL -- not open file:%s !",file_name);
      return(FAIL);
    }
        //Initial Set
...
int MINORI_Shmoo_Loop(int mode, int vs[NUM_POWERSUPPLY],int waittime,interface_t* param,int temprature, char* pat){
  ...
  for(Loop = 0;Loop<=1;Loop++){
    ...
    fprintf(Fptdata," module,FLP0,patten,%s  vcc,%d,vddh,%d,vdd,%d,CLK-Mode,%s",pat,vs[0],vs[1],vs[2],clk_mode);
    fprintf(Fptdata,"*** etlr25<7:0>=0x%X (iref)",GetValue(IN_ETLR25,param));
    fprintf(Fptdata,"*** etlr26<7:0>=0x%X (PrechargeTime FLI1/FLP)",GetValue(IN_ETLR26,param));
//    fprintf(Fptdata,"*** etlr27<7:0>=0x%X (PrechargeTime FLI2)",GetValue(IN_ETLR27,param));
    fprintf(Fptdata,"*** etlr28<7:0>=0x%X (Discharge FLP0)",GetValue(IN_ETLR28,param));
//    fprintf(Fptdata,"*** etlr29<7:0>=0x%X (Discharge FLI1)",GetValue(IN_ETLR29,param));
//    fprintf(Fptdata,"*** etlr30<7:0>=0x%X (Discharge FLI2)",GetValue(IN_ETLR30,param));
    fprintf(Fptdata,"*** etlr32<7:0>=0x%X (T5 2nd-SA<7:4>/T3 saenb<3:0>)",GetValue(IN_ETLR32,param));
    fprintf(Fptdata,"*** etlr33<7:0>=0x%X (T4 saout FLI/FLP)",GetValue(IN_ETLR33,param));
    fprintf(Fptdata,"*** etlr34<7:0>=0x%X (T6 2nd-SA enable control time)",GetValue(IN_ETLR34,param));
      ...
      for(Freq = 0;Freq <= FreqList_max;Freq++){
        ...
        for(j=0;j<=9;j++){
          ...
          if(result == PASS)Pass_num++;
                ...
                //Result Set
        if(Pass_num == 10){
          printf("***PASS*** : %d",Pass_num);
          fprintf(Fptdata,"%d,",Pass_num);
        }else if(result == CFAIL){
          printf("---Error!!!--- : %d",Pass_num);
          fprintf(Fptdata,"E,");
        }else{
          printf("---FAIL--- : %d",Pass_num);
          fprintf(Fptdata,"%d,",Pass_num);
        }
      }
      fprintf(Fptdata,"");
...
int RomBoot_Shmoo_Loop(int mode, int vs[NUM_POWERSUPPLY],interface_t* param,int temprature){
  ...
  switch(mode & 0x000000FF){
    ...
    break;
    case 1:
      in_ldata[n] = InLdataSet_Reg(1,26,0xFA);
      //ETLR26 -> 0xFA(Iref -> 2.0uA)
      n++;
    break;
  }
  ...
  for(Loop = 0;Loop<=1;Loop++){
    ...
    fprintf(Fptdata," module,FLP0,mode,%s,%s,%s,0x%X  vcc,%d,vddh,%d,vdd,%d,",BootOption1,BootOption2,RegModeName,mode,vs[0],vs[1],vs[2]);
...
int RomBoot_Shmoo_Menu(int vs[NUM_POWERSUPPLY],interface_t* param){
  ...
  while(while_flag){
    ...
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|BootOption| OPE : %s(101)  HOCO : %s(102)",OPE_Name,HOCO_Name);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Power   | Vcc:%4dmV(vcc) Vddh:%4dmV(vddh) Vdd:%4dmV(vdd)", vs[0],vs[1],vs[2]);
    printf("+----------+------------------------------------------------------------------------------------------+");
    printf("|  Other   | temprature : %dC(500) ", temprature);
    printf("|          | ALL Exe(99) ");
    printf("+----------+------------------------------------------------------------------------------------------+");
