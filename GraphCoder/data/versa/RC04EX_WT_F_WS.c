 // ****************************************************************************** FILENAME RC04EX_WT.c  CONTENTS: This is the RC04EX(SOTB_IO) FLASH module function program for the 5000 test system COPYRIGHT 2019 Renesas Technology ALL RIGHTS RESERVED 1'st created by T.Morioka(SRM13) *****************************************************************************
...
int WT_WS_Flash(interface_t* param){
  ...
  //Extra Loop
   ...
   // ******************************** FLP0 CHK Check ********************************
  // extra_num = 0; SetValue(IN_AREA,param,extra_num); SetValue(IN_TOPADDR0,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,FLP0_SIZE); SetValue(IN_TOPADDR1,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2,param,0); SetValue(IN_TESTSIZE2,param,0); sprintf(mdl_name,"FLP");   //-------- FLP0 CHK-A Write --------// if(result == PASS){ sprintf(test_name,"%s%d-CHKA-Write",mdl_name,extra_num); SetValue(IN_TESTSEL,param,0); // CHKA Select SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*20,param,CreatePatFileName("programchecker_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }   //-------- FLP0 CHK-A Read --------// if(result == PASS){ sprintf(test_name,"%s%d-CHKA-Read",mdl_name,extra_num); SetValue(IN_TESTSEL,param,0); // CHKA Select SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*3,param,CreatePatFileName("readchecker_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }   //-------- FLP0 CHK-B Write --------// if(result == PASS){ sprintf(test_name,"%s%d-CHKB-Write",mdl_name,extra_num); SetValue(IN_TESTSEL,param,1); // CHKB Select SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*20,param,CreatePatFileName("programchecker_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }   //-------- FLP0 ALL0 Read --------// if(result == PASS){ sprintf(test_name,"%s%d-ReadALL0-2",mdl_name,extra_num); SetValue(IN_TESTSEL,param,0); SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*3,param,CreatePatFileName("readall_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }   //-------- FLP0 Erase --------// if(result == PASS){ sprintf(test_name,"%s%d-Erase-3",mdl_name,extra_num); SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*5,param,CreatePatFileName("erase_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }   //-------- FLP0 Read ALL"1" --------// if(result == PASS){ sprintf(test_name,"%s%d-ReadALL1-3",mdl_name,extra_num); SetValue(IN_TESTSEL,param,1); SetValue(IN_BDATA00,param,0); printf("============= %s Test =============",test_name); result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*3,param,CreatePatFileName("readall_code_at"),test_name); if (PASS != result ){ sprintf(status_name,"%s_FAIL",test_name); printf("------------- %s FAIL -------------",test_name); result = FAIL; }else{ printf("************* %s PASS *************",test_name); } }
