// ****************************************************************************
// ! @file RC04EX_Sample_Set.c @brief This is a 130nm(RC01SN) FLASH module user interface program for the 4000,5000 test system @author T.Morioka @date 2012.04.16 Rev.00
// ****************************************************************************
...
// ************************************************************************************
    ...
    // Vnoemi_rd 3.4V //
    SetValue(IN_ETLR19,param,(GetValue(IN_ETLR19,param)&0xF0)|0x0C);
    // Vnoemi_pe 3.6V //
...
// ************************************************************************************
  ...
  fprintf(file_log,"Before_Vhh_w/Vpp_w = ,%d,%d",result_vhhw_bef,result_vppw_bef);
  fprintf(file_log,"After_Vhh_w/Vpp_w = ,%d,%d" ,result_vhhw_aft,result_vppw_aft);
  fprintf(file_log,"After_Vnoemi = ,%d" ,result_vnoemi_aft);
  fprintf(file_log,"After_Vrsg   = ,%d" ,result_vrsg_aft);
  fprintf(file_log,"After_Vdemg  = ,%d" ,result_vdemg_aft);
  fprintf(file_log,"After_Vwi    = ,%d" ,result_vwi_aft);
  fprintf(file_log,"");
...
// ************************************************************************************
  ...
  printf("Write-Voltage(Before) = %d[mV]   (%d/%d)",result_wvol_bef,result_vhhw_bef,result_vppw_bef);
  printf("Write-Voltage(After ) = %d[mV]   (%d/%d)",result_wvol_aft,result_vhhw_aft,result_vppw_aft);
  printf("Vnoemi       (After ) = %d[mV]   Target: 3600mV",result_vnoemi_aft);
  printf("Vrsgi        (After ) = %d[mV]   Target: 3400mV",result_vrsg_aft);
  printf("Vwi          (After ) = %d[mV]   Target: 2000mV",result_vwi_aft);
  printf("Vdemg        (After ) = %d[mV]   Target:-1000mV",result_vdemg_aft);
  printf("");
