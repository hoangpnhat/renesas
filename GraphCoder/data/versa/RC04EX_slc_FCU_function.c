  ...
  while( test_cond != EXIT){
    ...
    switch(test_cond){
	  ...
	  case 101:
	    ...
	    tempo1 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA0,param) / 1000;
	    //time[us] = T[ns]*COUNT/1000
	    tempo2 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA1,param) / 1000;
	    //time[us] = T[ns]*COUNT/1000
	    tempo3 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA2,param) / 1000;
	    //time[us] = T[ns]*COUNT/1000
	    printf("FACI command Write Time(256B) = %d[us]",tempo1);
	    printf("FACI command Write Time(  8B) = %d[us]",tempo2);
	    printf("FACI command Erase Time       = %d[us]",tempo3);
	  ...
	  case 113:
	    ...
	    tempo1 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA0,param);
	    //time[ns] = T[ns]*COUNT
	    printf("FACI command PE StTime = %d[ns]",tempo1);
	  case 114:
	    result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("FlashSM_7_9_WaitTime"),"PE Time");
	    tempo1 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA0,param);
	    //time[us] = T[ns]*COUNT/1000
	    tempo2 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA1,param);
	    //time[us] = T[ns]*COUNT/1000
	    tempo3 = GetValue(IN_EXTAL1,param) * GetValue(OUT_LDATA2,param);
	    //time[us] = T[ns]*COUNT/1000
	    printf("Test command St Write Time(256B) = %d[ns]",tempo1);
	    printf("Test command St Erase Time       = %d[ns]",tempo2);
	    printf("Test command St PreWrite Time    = %d[ns]",tempo3);
		  ...
		  testflag = 0;
			while(testflag != 99 ){
		  	printf("=================================");
				...
				printf(                        "       (3)");
				printf("|  PE~Sus Wait : %5d [us]  (4)",GetValue(IN_LDATA0,param));
				printf("| Sus~Res Wait : %5d [us]  (5)",GetValue(IN_LDATA1,param));
		  	printf("=================================");
				printf("|       PE Start (99)");
		  	printf("=================================");
		...
		case 202:
		  ...
		  while(testflag != 99 ){
		  	...
		  	printf("|      ----- Write -----");
				...
				printf(                       "       (3)");
				printf("| Address      : 0x%6X   (4)",adr2);
		  	printf("|      ----- Wait  -----");
				printf("|  E ~Sus Wait : %5d [us] (5)",GetValue(IN_LDATA0,param));
				printf("| Sus~ P  Wait : %5d [us] (6)",GetValue(IN_LDATA1,param));
				printf("|  P ~Res Wait : %5d [us] (7)",GetValue(IN_LDATA2,param));
		  	printf("=================================");
				printf("|       PE Start (99)");
		  	printf("=================================");
        printf("Input Number -> ");DigitInput(&testflag);
      ...
      if((Fptdata=fopen(file_name,"at")) == NULL) {
        ...
        break;
			...
			fprintf(Fptdata,"TestInfo,T=%d[ns],F=%d[MHz]",GetValue(IN_EXTAL1,param),1000/GetValue(IN_EXTAL1,param));
			fprintf(Fptdata,"PE-Wait[us],Suspend-Wait[us],Pass/Fail,");
			fclose(Fptdata);
			Ffpt = 0;
			while(result !=55){
			
			printf(" ***** Wait Time = %d[us] ~ =+99[us] ***** ",GetValue(IN_LDATA0,param));
		...
		case 204:
		  ...
		  while(testflag != 99 ){
		  	printf("=================================");
				...
				printf(                        "       (3)");
				printf("|  PE~Sus Wait : %5d [us]  (4)",GetValue(IN_LDATA0,param));
				printf("|Tspd(Sus-Time): 0x%3X       (5)",tempo2);
		  	printf("=================================");
				printf("|       PE Start (99)");
		  	printf("=================================");
        printf("Input Number -> ");DigitInput(&testflag);
      ...
      if((Fptdata=fopen(file_name,"at")) == NULL) {
        ...
        fprintf(Fptdata,"TestInfo,T=%d[ns],F=%d[MHz],ETLR10=0x%X",GetValue(IN_EXTAL1,param),1000/GetValue(IN_EXTAL1,param),GetValue(IN_ETLR10,param));
			  fprintf(Fptdata,"PE-Wait[us],Suspend-Wait[us],Pass/Fail,");
			  fclose(Fptdata);
		...
		case 302:
		  ...
		  while(testflag != 99 ){
		  	printf("=================================");
				...
				printf(                       "        (1)");
				printf("| Address      : 0x%6X~0x%6X (2)",adr,adr+adr2-1);
				printf("| PE Mode      : ");
				switch(GetValue(IN_TESTSEL,param)){
					case 0 : printf(       "Write" );break;
					case 1 : printf(       "Erase" );break;
      ...
      sprintf( file_name, "%s%s%s.csv", glob_cdp, glob_datalog, item_name );
        ...
        if((Fptdata=fopen(file_name,"at")) == NULL) {
          ...
          break;
			  ...
			  fprintf(Fptdata,"TestInfo,T=%d[ns],F=%d[MHz],vcc=%d,vddh=%d,vdd=%d",GetValue(IN_EXTAL1,param),1000/GetValue(IN_EXTAL1,param),vs[0],vs[1],vs[2]);
    ...
    break;
        ...
        /			// 1=ALL Pattern  break; case 304: CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("programall_code_at"),"Program ALL"); SetValue(IN_TESTSEL,param,1); sprintf(item_name2,"ERASE"); search_mode = 1; //Gain Down vth_mode = 1; //VthRead1 max_vol = 0; //max = 0V min_vol = -2000; //min = -2.0V delta_vol = 100; //VthRead Step bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern break; }  //File Name Setting sprintf(item_name,"%s_W%d_X%dY%d_PE-BGO_Vcc%d_%dC_VthRead",lotname,atoi(wafername),xy_location[0],xy_location[1],vs[0],tempo1); /* sprintf( file_name, "%s%s%s.csv", glob_cdp, glob_datalog, item_name ); if((Fptdata=fopen(file_name,"rt")) == NULL) { if((Fptdata=fopen(file_name,"at")) == NULL) { printf(" -- FAIL -- not open file:%s !",file_name); break; } fprintf(Fptdata,"Temp[C],Vcc,Vddh,Vdd,PE-Module,PE-Address,Vth[mV],");  // ETLR Data Out for(i=IN_ETLR00;i<=IN_ETLR39;i++){ fprintf(Fptdata,"ETLR%2d,",(i - IN_ETLR00)); }  // ETCR Data Out for(i=IN_ETCR00;i<=IN_ETCR13;i++){ fprintf(Fptdata,"ETCR%2d,",(i - IN_ETCR00)); } fprintf(Fptdata,""); fclose(Fptdata); }
			...
			VthReadDist(vth_mode, bit_count, min_vol, delta_vol, &vs_bak[0],W1SEC*waittime, param, item_name);
			//Vthz
						//Test Main End

			SetValue(IN_EXTAL1,param,freq_bak);
			// if((Fptdata=fopen(file_name,"at")) == NULL) { //File Open printf(" -- FAIL -- not open file:%s !",file_name); break; } fprintf(Fptdata,"%d,%d,%d,%d,",tempo1,vs[0],vs[1]*25/10,vs[2]*158/100); //Temp,Vcc,Vddh,Vdd //Module & Address if(GetValue(IN_TESTSIZE0,param) !=0){ f tempo2 = GetValue(IN_TESTSIZE0,param); tempo4 = GetValue(IN_TOPADDR0,param); }else if(GetValue(IN_TESTSIZE1,param) !=0){ fprintf(Fptdata,"FLI1,"); tempo2 = GetValue(IN_TESTSIZE1,param); tempo4 = GetValue(IN_TOPADDR1,param); }else if(GetValue(IN_TESTSIZE2,param) !=0){ fprintf(Fptdata,"FLI2,"); tempo2 = GetValue(IN_TESTSIZE2,param); tempo4 = GetValue(IN_TOPADDR2,param); } fprintf(Fptdata,"0x%X~0x%X,",tempo4,tempo4+tempo2-1); fprintf(Fptdata,"%d,",result);  //ETLR Data Out for(i=IN_ETLR00;i<=IN_ETLR39;i++){ fprintf(Fptdata,"0x%X,",GetValue(i,param)); }  // ETCR Data Out for(i=IN_ETCR00;i<=IN_ETCR13;i++){ fprintf(Fptdata,"0x%X,",GetValue(i,param)); } fprintf(Fptdata,""); fclose(Fptdata);
		break;
		...
		case 401:
		  ...
		  while(testflag != 99 ){
		  	printf("=================================");
				...
				printf(                        "       (3)");
				printf("|  PE~Abo Wait : %5d [us]  (4)",GetValue(IN_LDATA0,param));
		  	printf("=================================");
				printf("|       PE Start (99)");
		  	printf("=================================");
        printf("Input Number -> ");DigitInput(&testflag);
      ...
      printf("================");
	    ...
	    SetValue(IN_LDATA0,param,10);
			...
			SetValue(IN_ETCR11,param,(GetValue(IN_ETCR11,param)&0x1F) | (2 << 5));
						...
						// Monitor Mode Set
			SetValue(IN_ETCR01,param,(GetValue(IN_ETCR01,param)&0xE0)|0x02);
			// Vnoemi monitor mode
      SetValue(IN_ETCR02,param,(GetValue(IN_ETCR02,param)&0xC0)|0x22);
      // VPP monitor mode
        ...
        printf("  --- TimeOut Check ERROR!!! --- ");
        printf(    "      ETCR16 = 0x%X",GetValue(OUT_ETCR16,param));
      }
