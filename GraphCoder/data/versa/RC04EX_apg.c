 // *********************************************************************** FILENAME RX200apg.c  CONTENTS: This is a 64Kbyte flash application program for the V3300 test system.  COPYRIGHT 1997 HITACHI LTD. ALL RIGHTS RESERVED.  2008/11/01  rev.0  H.Kobori ***********************************************************************
...
int Ecrsetup8(int ecrmode)
{
  ...
  comp_set_bm_address_mask(0,0xffc00000,0);
  // for 32Mbit only
  comp_set_ecr_address_mask(0,0xffc00000,0);
  // for 32Mbit only

  comp_set_ecr_write_mask(0xff);
  // enable write mask:0xff => 8bit write
  set_ecr_data_mask(0);
  // set overwrite mode
  comp_set_ecr_postdecode_mask(0xff00);
  // set D0-D7 care mode
  set_ecr_count_mask(1);
  // setcounter

  comp_set_ecr_mode(ecrmode);
  // set ecr mode
...
int DeviceLevelsPowerUp(int vcc0,int vcc1,int vcc2,int relay)
{
  ...
  if( CheckFK(FK_DISP_DEBUG) ){
    sprintf(outbuf,"Power Setting  Vcc[0]=%d, vcc[1]=%d, vcc[2]=%d", vcc0,vcc1,vcc2 );
    DataOut(Ffpt,outbuf);
  }

  if( PASS!=err_flag ){ printf("-- FAIL -- vcc, ext_vrefH, ext_vref level over limit Error[0x%X]", err_flag ); return( err_flag ); }
  // ** current control?? **
#else
  if( comp_set_i1limit( 400000000 ) ) err_flag |= 0x0020;
  // V4000 500mA / V5000 PPS12 200mA(100mA?) / V5000 PPS12E 400mA
#endif
  ...
  if( comp_set_i4limit( 200000000 ) ) err_flag |= 0x0100;
        ...
        // ** Power Setting **
  if( comp_set_v1(vcc0)  ){ err_flag |= 0x0200; DeviceLevelsPowerDown(); }
  //Vcc setting
  if( comp_set_v3(vcc2)  ){ err_flag |= 0x0400; DeviceLevelsPowerDown(); }
  //ext_vref setting
  if( set_vih2(vcc1)     ){ err_flag |= 0x1000; DeviceLevelsPowerDown(); }
  //ext_vrefh setting
  disconnect_pin(monitor_vdd_pin);
  disconnect_pin(monitor_vddh_pin);

  delay_timer(30*1000);
  // 100ms = 100*1000us Wait
  if( PASS!=err_flag ){ printf("-- FAIL -- Vcc Set Error[0x%X]", err_flag ); return( err_flag ); }
  // ** LogicPin Setting **
  if( PASS!=err_flag   )   { printf("-- FAIL -- vih1&2,vhh Set Error[0x%X]", err_flag ); return( err_flag ); }
  // ** voh,vol Setting **
  if( set_vol(vcc0*1/2) ){ err_flag |= 0x8000; DeviceLevelsPowerDown(); }
  if( PASS!=err_flag )  { printf("-- FAIL -- Voh Vol Set Error[0x%X]", err_flag );}
  // ** Relay Setting **
...
int DeviceSpecificPowerUp(void)
{
  ...
  set_vector_mode(control_pins);
  // set vector mode control pin, not select data pin
  set_vih1_mask(all_pins);
  // set vih1 mask : all_pin
  set_vih2_mask(ext_vrefh_pin);
  // set vih2 mask : ext_Vrefh
    //set_vhh_mask(lb_all_pins);          // set vhh  mask : relay_pin

  set_drive_time(all_pins,0,0,100);
  // set drive time V3300 2ch
  reconnect_pin(all_pins);
  // PPMUSEL=0,TRISTATE=1
  
  select_vector_mode(vec_fly_pin,_PE_VEC_ON_THE_FLY);
  // APG mode select
...
// ================================================================================================
{
  ...
  if( CheckFK(FK_DISP_DEBUG) ){
    sprintf(outbuf,"Change speed = %2dns",rate);
    DataOut(Ffpt,outbuf);
  }
  ...
  if( PASS!=err_flag ){
    printf( "SetTimming_NS Error [0x%X]", err_flag );
...
// ================================================================================================
{
  ...
  if( CheckFK(FK_DISP_DEBUG) ){
    sprintf(outbuf,"Change speed = %2dps",rate);
    DataOut(Ffpt,outbuf);
  }
  ...
  if( PASS!=err_flag ){
    printf( "SetTimming_PS Error [0x%X]", err_flag );
    DeviceLevelsPowerDown();
  }
  return( err_flag );
}
...
int SetVohVolVtt( int voh, int vol, int vtt, int *vttmaskpin ){
  int err_flag;
  err_flag = 0;
  if( set_voh( voh ) ){ printf("-- FAIL -- Voh level err"); DeviceLevelsPowerDown(); err_flag |= 0x01; } if( set_vol( vol ) ){
#ifdef V5KONLY
    //if( set_pe_resource( vttmaskpin, PE_VTT, vtt ) ){ printf("-- FAIL -- Vtt level err"); DeviceLevelsPowerDown(); err_flag |= 0x20; } //if( set_vtt_mask( vttmaskpin ) ){ 
#endif
  return( err_flag );
}
...
int SelectPowerSupply(int mode,int vcc[NUM_POWERSUPPLY])
{
  ...
  if((CRYSTAL_MODE & 0x10000) == (mode & 0x10000)){
    ...
    if((MNORMAL_CRS & 0xFF) == (mode & 0xFF))          relay = RVSSMON_20_CRS;
    //0xfff5
    else if((MVSSMON_CRS & 0xFF) == (mode & 0xFF))     relay = RVSSMON_CRS;
    //0xfff1
    else if((MVSSMON_20_CRS & 0xFF) == (mode & 0xFF))  relay = RVSSMON_20_CRS;
    //0xfff5
    else if((MVCCMON_CRS & 0xFF) == (mode & 0xFF))     relay = RVCCMON_CRS;
    //0xfff0
    else if((MVRSGMON_CRS & 0xFF) == (mode & 0xFF))    relay = RVRSGMON_CRS;
    //0xfff2
    else if((MVSSMON_CH_CRS & 0xFF) == (mode & 0xFF))  relay = RVSSMON_CH_CRS;
    //0xfff0
    else printf("-- FAIL -- mode set err");
    }
  else{
        //if(MNORMAL & mode)              relay = RNORMAL;        //0xfff8
    if(MNORMAL & mode)              relay = RVSSMON_20;
    //0xfffd                            // SW�ؑւ�������VCCMON,VSSMON���j�^�\�Ȃ悤�Ƀf�t�H���g�ύX�{SB��20k����R����O��Floating
    else if(MVSSMON & mode)         relay = RVSSMON;
    //0xfff9
    else if(MVSSMON_20 & mode)      relay = RVSSMON_20;
    //0xfffd
    else if(MVCCMON & mode)         relay = RVCCMON;
    //0xfff8
    else if(MVRSGMON & mode)        relay = RVRSGMON;
    //0xfffa
    else if(MVSSMON_CH & mode)      relay = RVSSMON_CH;
    //0xfff8
...
int status;
  ...
  reconnect_pin(os_pins);
  // set PPMUSEL=0,TRISTATE=1
  ...
  // max=25uA
  set_vlimit(-1500);
  // set input voltage = -1.5V
  comp_set_ilimit(10000);
  // I-Limit = 10000nA(10uA)
...
// ****** DISP TAITOL ******
  ...
  sprintf(outbuf,"TEST:%d [ %s ] vcc=%dmV ext_Vrefh=%dmV ext_Vref=%dmV F=%2dns PLL=%d(X%d) ", test++,mess,vcc[0],vcc[1],vcc[2],speed,GetValue(IN_PLLON,param),GetValue(IN_PLL_MULT,param));
  DataOut(Ffpt,outbuf);
  sprintf(outbuf,"Module:%s%s", DispMatName(mat_str,param),disp_addres);
  DataOut(Ffpt,outbuf);
    //Ldata�̌����ɕύX����K�v�L
  sprintf(outbuf,"BDATA00:%08X, LDATA0:%08X" ,GetValue(IN_BDATA00,param),GetValue(IN_LDATA0,param));
  DataOut(Ffpt,outbuf);
...
// ******* PowerUp ********
  ...
  if(SelectPowerSupply(mode,vcc)) {
  //�e�d���̋�����Level�̐ݒ�(DAXS�]���p)
    vcc[0]=vs0;
    vcc[1]=vs1;
    vcc[2]=vs2;
    return(HWFAIL);
  }
  SetVohVolVtt( daxs_trans.voh, daxs_trans.vol, daxs_trans.vtt, daxs_datao_pin );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
  vcc[0]=vs0;
  vcc[1]=vs1;
  vcc[2]=vs2;
...
// if(strcmp(pat,"devicefunction_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1060_V000100.pat" ); else if(strcmp(pat,"readall_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else if(strcmp(pat,"erase_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P2000_V000100.pat" ); else if(strcmp(pat,"programall_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1920_V000100.pat" ); else if(strcmp(pat,"program55AA_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1920_V000100.pat" ); else if(strcmp(pat,"readcount_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else if(strcmp(pat,"settrimingdata_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1100_V000100.pat" ); else if(strcmp(pat,"readdump_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else { printf("parameter file nothing!!"); return(FAIL); } CheckPatLength(binpat); comp_load_bm (binpat);

  WriteBmMode8( 0x3C00 , 0x00100000, SIZE_L );
  // Dummy Data 18/04/11 Aoki
...
// ****************************************************************************
  ...
  ParamConditionDisp(PDIN_ALL,0,param);
  //���͗pIF�̏o��
  ...
  DataOut(Ffpt,outbuf);
    ...
    result = RAMRunApg( 0, RAM_TOP, RAM_END );
    // DAXS transport 0=write command
  ...
  //Compare Setting command

    result = RAMRunApg( 2, RAM_TOP, OUTPUTIF_END );
    // DAXS transport 0=write command //
  ...
  for(i=RAM_TOP;i<=(RAM_TOP+0x100);i=i+4){
    ...
    if(bm_data!=ecr_data){
      if(result != FAIL)printf("BM <=> ECR Comp ERROR!!! Addr = %X, BM = %X, ECR = %X ",i,bm_data,ecr_data);
      result = FAIL;
    }
  }

  if(PASS!=result){
    printf(" ------ BM <=> ECR Comp ERROR!!! -------");
    return(RAMFAIL);
  }
...
// ****************************************************************************
  ...
  // ��rPin�̐ݒ�(RAMBoot�����܂ł̑ҋ@���ԂɎg�p)
  select_vector_mode(poll_pins,_PE_VECTOR_MODE);
  //180314 Add-morioka For RamBoot
  reconnect_pin(poll_pins);
  //180314 Add-morioka For RamBoot

    // ** Test Speed SetUp **
  SetTimming_NS(speed);
  ChangeTimming_GTSLTS( EXTCYLJ, 1, MODEENTRY_SPEED );
  // ModeEntry���g���ݒ� GTS=1,20MHz
...
// ******* APG SetUp(Wait) ********
  load_vlf_offset(_SUB_MODEENTRY_RAM,6,  1000/MODEENTRY_SPEED);
  // 1us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,1,300000/2/MODEENTRY_SPEED);
  // 300us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,2, 70000/MODEENTRY_SPEED);
  // 70ms Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,3,tempo1);
  // Vdd/Vddh On/Off
  load_vlf_offset(_SUB_RAMBT_END,0,500*waittime/speed);
  // RAMBootEnd Wait = waittime x(500x200x2000ns)

  SetVohVolVtt( 2*vcc[0]/10, 2*vcc[0]/10, 5*vcc[0]/10 , bistpoll_pins );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
...
// ******* Test Start ********
  ...
  Cputotal = (bentime()-start_time-init_bentime())/1000;
  // Get Test Total Time

  cpuerr = check_pin_errors( bistpoll_pins, _CHECK_FERR );
  // Boot����Error�o�͂�APG�p�^�����璊�o(DPASSDEND_CP�̔�r����)
  sprintf(outbuf,"cpuerr=%d ",cpuerr); DataOut(Ffpt,outbuf);
  sprintf(outbuf," ----- RAMBOOT End  -----"); DataOut(Ffpt,outbuf);
...
// DAXS Read
// ****************************************************************************
  sprintf(outbuf," ----- Result RAM Read Start -----");DataOut(Ffpt,outbuf);
...
// ******* PowerUp ********
  ...
  if(SelectPowerSupply(MNORMAL,vcc)) {
  //�e�d���̋�����Level�̐ݒ�(DAXS�]���p)
    vcc[0]=vs0;
    vcc[1]=vs1;
    vcc[2]=vs2;
  ...
  for( cnt=0x1000; cnt<=RAM_END; cnt=cnt+4 ){
    ...
    result = RAMRunApg( 2, 0x1000, RAM_END );
    // DAXS transport 0=write command
        //RAMRunApg( 2, FBM_READTOP ,(FBM_READTOP + FBM_READSIZE - 1) );    // Daxs Trans For FBM
  sprintf(outbuf," ----- Result RAM Read End -----");DataOut(Ffpt,outbuf);
...
// ******* end manage ********
  ...
  if(cpuerr == 1) {
    ...
    case CPASS:
      sprintf(outbuf,"** PASS ** Cpu Total=%dms",Cputotal);
      DataOut(Ffpt,outbuf);
      return(PASS);
    case CFAIL:
      sprintf(outbuf,"-- FAIL -- Cpu total=%dms",Cputotal);
      DataOut(Ffpt,outbuf);
      return(FAIL);
    case CFLAG:
      sprintf(outbuf,"-- FLAG_FAIL -- Cpu Total=%dms",Cputotal);
      DataOut(Ffpt,outbuf);
      return(FLGFAIL);
    case READFAIL:
      sprintf(outbuf,"-- READ_FAIL -- Cpu Total=%dms",Cputotal);
      DataOut(Ffpt,outbuf);
      return(FAIL);
    default:
      sprintf(outbuf,"-- FAIL -- None (%04X) %dms",((0x00FF&GetValue(OUT_JUDGE1,param))<<8)|((0xFF00&GetValue(OUT_JUDGE1,param))>>8),Cputotal);
      DataOut(Ffpt,outbuf);
      return(CPUFAIL);
    }
  }
  sprintf(outbuf,"-- FAIL -- !!! TimeOver !!! total=%dms",Cputotal);
  DataOut(Ffpt,outbuf);
...
// if(strcmp(pat,"devicefunction_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1060_V000100.pat" ); else if(strcmp(pat,"readall_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else if(strcmp(pat,"erase_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P2000_V000100.pat" ); else if(strcmp(pat,"programall_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1920_V000100.pat" ); else if(strcmp(pat,"program55AA_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1920_V000100.pat" ); else if(strcmp(pat,"readcount_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else if(strcmp(pat,"settrimingdata_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P1100_V000100.pat" ); else if(strcmp(pat,"readdump_code_at.pat")==0) strcat( binpat, "RC04EX_WT1_P3011_V000100.pat" ); else { printf("parameter file nothing!!"); return(FAIL); } CheckPatLength(binpat); comp_load_bm (binpat);
  ...
  ParamConditionDisp(PDALL,0,param);
  //���͗pIF�̏o��
...
// ****************************************************************************
  ...
  for(i=RAM_TOP;i<=OUTPUTIF_END;i=i+4){
    ...
    ecr_data=ReadEcrMode8(i,SIZE_L);
    if(bm_data!=ecr_data){
      printf("BM <=> ECR Comp ERROR!!! Addr = %X, BM = %X, ECR = %X ",i,bm_data,ecr_data);
      result = FAIL;
    }
  }
...
// ******* APG SetUp(Wait) ********
  load_vlf_offset(_SUB_MODEENTRY_RAM,1,  1000/2/speed);
  // 1us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,2,300000/2/speed);
  // 300us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,3, 70000/speed);
  // 70ms Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,4,tempo1);
  // Vdd/Vddh On/Off
    //load_vlf_offset(_SUB_RAMBT_END,0,500*waittime/speed);    // RAMBootEnd Wait = waittime x(500x200x2000ns)
  load_vlf_offset(_SUB_RAMBT_END_PE,0,500*waittime/speed);
  // RAMBootEnd Wait = waittime x(500x200x2000ns)
  load_vlf_offset(_SUB_RAMBT_END_PE,1,limittime*6);
  // 10min x 6(1hr) x limittime
...
// ******* PowerUp ********
  ...
  result = RAMRunApg( 2, 0x2A00, OUTPUTIF_END );
  // DAXS transport 0=write command
...
int CpuModeMonitor(int mode,int vcc[NUM_POWERSUPPLY],interface_t* param,int waittime,char *pat,char *mess,int *selvol)
{
  ...
  int flg,tempo1;
  // ** CPU drive recklessly flg **
  int result;
  // Test Result
  int selmode,pmu_range,bm_data,ecr_data;
  char mat_value[100];
  int start_time;
  // Start Time For bentime
  int vs0,vs1,vs2;
  // Power bak
  int old_reg;
  char mat_str[20];
...
// ****************************************************************************
  ...
  load_vlf_offset(_SUB_RAMBT_END,0,500*waittime/speed);
  // RAMBootEnd Wait = waittime x(500x200x2000ns)
  load_vlf_offset(_SUB_RAMBT_POLL,0,500*waittime/speed);
  // RAMBootPoll Wait = waittime x(500x200x200ns)
  ...
  if((mode & MONITOR_MODE) || (mode & CP_SUPPLY_MODE)){
    switch(selmode){
      ...
      case MONITOR_VPP_PW:
        set_vlimit(0);
        // Set V = 0V
        comp_set_ilimit(250000);
        // I-Limit = 250000nA(250uA) 
        delay_timer( 1000 );
      ...
      case MONITOR_READDELAY8:
      //Add 2018/0323
      case MONITOR_READDELAY10:
      //Add 2018/0323
      ...
      // Default Error
        DeviceLevelsPowerDown();
        printf("MONITOR_MODE PMU setting, Code ERROR!!! code=%X",selmode);
        //Define Error, Check selmode value return(FAIL);
        break;
    }
    ...
    switch(selmode){
      ...
      //Add 2018/0323
        ...
        comp_set_ilimit(*selvol);
        // I-Limit = 10.5uA(10.5uA)
        break;
      ...
      //Add 20180324
        ...
        // max = 250uA
        comp_set_ilimit(250000);
        // I-Limit = 250000nA(250u)
        break;

      case AUTOTRIM_READDELAY8:
      //Add READ Delay8 TRIM SETTING 2018/0322
      case AUTOTRIM_READDELAY10:
      //Add READ Delay10 TRIM SETTING 2018/0322
                //DelayTestSetting(*selvol,vcc[1]*25/10,vcc[2]*158/100);
        break;
...
//        break;
      ...
      // Default Error
        DeviceLevelsPowerDown();
        printf("APPLY_MODE PMU setting, Code ERROR!!! code=%X",selmode);
        //Define Error, Check selmode value return(FAIL);
        break;
    }
    ...
    if(mode & MONITOR_MODE){
      switch(selmode){
        case MONITOR_BGR:
          comp_set_ilimit(0);
          load_vlf_offset(_SUB_RAMBT_POLL,0,60000/speed);
          // 80ms fix(2000*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
	        delay_timer(100*1000);
	        // 100ms wait = 100*1000*1us
          SITE_MASK (1, read_pmu_status(0x20));
          // V measurement
        ...
        case MONITOR_VNOEMI_PE:
          ...
          comp_set_ilimit(0);
          load_vlf_offset(_SUB_RAMBT_POLL,0,500/speed);
          // 20ms fix(2000*200*200ns)
          if(GetValue(IN_BDATA16,param) == 12){
          //Vnoemi Leak Monitor
            set_vlimit(*selvol);
            run_apg(_RAM_BOOT_MONITOR_CTRL);
        ...
        case MONITOR_IMONI_FLASH_TEMP:
          load_vlf_offset(_SUB_RAMBT_POLL,0,500/speed);
          // 20ms fix(500*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
          SITE_MASK (1, read_pmu_status(0x08));
          // I measurement
        ...
        case MONITOR_READDELAY10:
        //Add 2018/0323
          load_vlf_offset(_SUB_RAMBT_POLL,0,1500/speed);
          // 60ms fix(1500*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
                    //DelayTestSetting(*selvol,vcc[1]*25/10,vcc[2]*158/100);//�p���X����ݒ�
          DelayTestSetting(*selvol,vcc[1]*25/10,vcc[0]);
          //�p���X����ݒ�=VCC level
          run_apg(_RAM_BOOT_DELAYTEST);
          //�p���X���
          DelayTestSetting(*selvol,vcc[0],vcc[0]);
          //Pin�ݒ�����ɖ߂�
          break;

        default:
          DeviceLevelsPowerDown();
          printf("MONITOR_MODE Run APG, Code ERROR!!! code=%X",selmode);
          //Define Error, Check selmode value return(FAIL);
          break;
      }
      ...
      switch(selmode){
        ...
        case AUTOTRIM_IMONI_FLASH_TEMP:
          ...
          load_vlf_offset(_SUB_RAMBT_POLL,0,100/speed);
          // 4ms fix(100*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
        ...
        case AUTOTRIM_VHH_P:
          ...
          set_vlimit( 1250 );
          //Set VDDH //add 2018/0328
          delay_timer( 100 );
          // wait 100us
          load_vlf_offset(_SUB_RAMBT_POLL,0,600/speed);
          // 24ms fix(600*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
          ...
          delay_timer( 100 );
          // wait 100us
          load_vlf_offset(_SUB_RAMBT_POLL,0,750/speed);
          // 30ms fix(750*200*200ns)
        ...
        default:
          DeviceLevelsPowerDown();
          printf("APPLY_MODE Run APG, Code ERROR!!! code=%X",selmode);
          //Define Error, Check selmode value return(FAIL);
          break;
      }
      ...
      printf("Mode Select Error!!! mode=%X",mode);
      return(FAIL);
    }
    ...
    cpuerr = check_pin_errors( bistpoll_pins, 4 );
    // Boot����Error�o�͂�APG�p�^�����璊�o(DPASSDEND_CP�̔�r����)
    Cputotal = (bentime()-start_time-init_bentime())/1000;
    // Get Test Total Time
...
// ****************************************************************************
  ...
  result = RAMRunApg( 2, FBM_READTOP, OUTPUTIF_END );
  // DAXS transport 0=write command
...
// ****************************************************************************
  ...
  if(mode & MONITOR_MODE){
    switch(mode & 0x0000FFFF){
      ...
      case  MVSSMON_CH:
      case  MVSSMON_20K:
        sprintf(outbuf,"  Monitor Result VSSMON = %dmV",*selvol);
        DataOut(Ffpt,outbuf);
        break;
      case  MVCCMON:
        sprintf(outbuf,"  Monitor Result VCCMON = %dmV",*selvol);
        DataOut(Ffpt,outbuf);
        break;
      case  MVRSGMON:
        sprintf(outbuf,"  Monitor Result VRSGMON = %dmV",*selvol);
        DataOut(Ffpt,outbuf);
        break;
      case  MVCLMON:
        sprintf(outbuf,"  Monitor Result VCLMON = %dmV",*selvol);
        DataOut(Ffpt,outbuf);
        break;
      default:
        printf("Result Setting Error!! mode code = %X",mode);
        return(FAIL);
        break;
    }
    selvol = center;
    ...
    switch(selmode){
      ...
      case  AUTOTRIM_READDELAY8:
      case  AUTOTRIM_READDELAY10:
        sprintf(outbuf,"  Target Read Delay = %dns",*selvol);
        DataOut(Ffpt,outbuf);
        break;
    }
    if(GetValue(OUT_JUDGE1,param) == 0x00){
      sprintf(outbuf," AutoTrim Pass Input Voltage = %d",*selvol);
      DataOut(Ffpt,outbuf);
      ...
      sprintf(outbuf," AutoTrim JUDGE = 0x%04X",((0x00FF&GetValue(OUT_JUDGE1,param))<<8)|((0xFF00&GetValue(OUT_JUDGE1,param))>>8));
      DataOut(Ffpt,outbuf);
    }
    if( ( ReadEcrMode8( 0x3D00, SIZE_L )==0x55555555
    // TEST_PASS
      switch( selmode ){
        case AUTOTRIM_VREF10:    printf( " ETLR11 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR11,param), GetValue(OUT_ETLR11,param) );
                                 SetValue(IN_ETLR11,param,GetValue(OUT_ETLR11,param)); break;
        case AUTOTRIM_VPP_E:     printf( " ETLR20 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR20,param), GetValue(OUT_ETLR20,param) );
                                 SetValue(IN_ETLR20,param,GetValue(OUT_ETLR20,param)); break;
        case AUTOTRIM_VPP_P:     printf( " ETLR20 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR20,param), GetValue(OUT_ETLR20,param) );
                                 SetValue(IN_ETLR20,param,GetValue(OUT_ETLR20,param)); break;
        case AUTOTRIM_VPP_PW:    printf( " ETLR21 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR21,param), GetValue(OUT_ETLR21,param) );
                                 SetValue(IN_ETLR21,param,GetValue(OUT_ETLR21,param)); break;
        case AUTOTRIM_VHH_E:     printf( " ETLR16 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR16,param), GetValue(OUT_ETLR16,param) );
                                 SetValue(IN_ETLR16,param,GetValue(OUT_ETLR16,param)); break;
        case AUTOTRIM_VHH_P:     printf( " ETLR17 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR17,param), GetValue(OUT_ETLR17,param) );
                                 SetValue(IN_ETLR17,param,GetValue(OUT_ETLR17,param)); break;
        case AUTOTRIM_VRSG:      printf( " ETLR18 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR18,param), GetValue(OUT_ETLR18,param) );
                                 SetValue(IN_ETLR18,param,GetValue(OUT_ETLR18,param)); break;
        case AUTOTRIM_V33R:      printf( " ETLR24 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR24,param), GetValue(OUT_ETLR24,param) );
                                 SetValue(IN_ETLR24,param,GetValue(OUT_ETLR24,param)); break;
        case AUTOTRIM_VWI:       printf( " ETLR19 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR19,param), GetValue(OUT_ETLR19,param) );
                                 SetValue(IN_ETLR19,param,GetValue(OUT_ETLR19,param)); break;
        case AUTOTRIM_VNOEMI_PE: printf( " ETLR19 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR19,param), GetValue(OUT_ETLR19,param) );
                                 SetValue(IN_ETLR19,param,GetValue(OUT_ETLR19,param)); break;
        case AUTOTRIM_IMONI_I01U_NOTEMP:
        case AUTOTRIM_IMONI_FLASH_NOTEMP:
        case AUTOTRIM_IMONI_I01U_TEMP:
        case AUTOTRIM_IMONI_FLASH_TEMP:
          printf( " ETLR08 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR08,param), GetValue(OUT_ETLR08,param) );
          SetValue(IN_ETLR08,param,GetValue(OUT_ETLR08,param));
          printf( " ETLR09 = 0x%02X ->> 0x%02X ", GetValue(IN_ETLR09,param), GetValue(OUT_ETLR09,param) );
...
int CpuModeMonitor_Repeat(int repeat_mode, int mode,int vcc[NUM_POWERSUPPLY],interface_t* param,int waittime,char *pat,char *mess,int *selvol)
{
 ...
 // Repeat��BM��ECR�̏����ݒ�͕s�v�H for( cnt=RAM_TOP; cnt<=RAM_END; cnt++ ){ WriteBmMode8( cnt, 0xFF, SIZE_B ); WriteEcrMode8( cnt, 0xFF, SIZE_B ); }
    // ****** GET PARAMETER ******
  speed=GetValue(IN_EXTAL1,param);
...
// ******* BGR Code Check ********
  BGR_REG_CHECK(vcc[0],param);
    ...
    // --------------Repeat-Set--------------
  if( ( repeat_mode & (INITIAL_SET | INIF_SET) ) !=0 ){
  // DAXS�]��(����)���s����
...
//      WriteBmMode8( cnt, 0xFFFFFFFF, SIZE_L );
    ...
    vcc[0]=daxs_trans.vcc;
  	...
  	// vs0=vcc[0]; vs1=vcc[1]; vs2=vcc[2];  vcc[0]=daxs_trans.vcc; vcc[1]=daxs_trans.vrefh; vcc[2]=daxs_trans.vref;
        // ******* APG SetUp(Power) ********
    tempo1=0;
    if((vcc[1] != 0) && (vcc[2] != 0)){
      tempo1 = tempo1 + 0x01;
...
// ****************************************************************************
    ...
    }else if(( repeat_mode & (INIF_SET) ) != 0 ){
    //����IF�ύX
...
// ****************************************************************************
    ...
    // --------------Repeat-Set--------------
  if( ( repeat_mode & (INITIAL_SET | INIF_SET | RAMBOOT_CHANGE) ) !=0 ){
  // RAMBOOT�ݒ�ύX����
...
//        break;
    ...
    if(mode & MONITOR_MODE){
      ...
      switch(selmode){
        ...
        case AUTOTRIM_IMONI_FLASH_TEMP:
                    //load_vlf_offset(_SUB_RAMBT_POLL,1,1000); // 50*cycle
          load_vlf_offset(_SUB_RAMBT_POLL,0,1200/speed);
          // 48ms fix(1200*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
        ...
        case AUTOTRIM_VHH_P:
          ...
          delay_timer( 100 );
          // wait 100us
          load_vlf_offset(_SUB_RAMBT_POLL,0,1800/speed);
          // 60ms fix(1500*200*200ns)
          run_apg(_RAM_BOOT_MONITOR_CTRL);
...
// ****************************************************************************
  ...
  // DAXS�]��(�o��)���s����
    ...
    result = RAMRunApg( 2, 0x1000, OUTPUTIF_END );
    // DAXS transport 0=write command
        //RAMRunApg( 2, FBM_READTOP ,(FBM_READTOP + FBM_READSIZE - 1) );    // Daxs Trans For FBM
...
int CpuModeMonitor_current2(int monitor_mode ,int mode,int vcc[NUM_POWERSUPPLY],interface_t* param,int waittime,char *pat,char *mess,int *selvol)
{
  ...
  ave_data=0;
  sample_num = 1023;
  sample_rate = GetValue(IN_BDATA10,param);
  //sample rate:5us
...
// ****************************************************************************
  ...
  //���͗pIF�̏o��
    ...
    result = RAMRunApg( 0, RAM_TOP, OUTPUTIF_END );
    // DAXS transport 0=write command
...
// ****************************************************************************
  ...
  if((vcc[1] != 0) && (vcc[2] != 0)){
    ...
    // ******* APG SetUp(Wait) ********
  load_vlf_offset(_SUB_MODEENTRY_RAM,6,  3*1000/MODEENTRY_SPEED);
  // 1us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,1,300000/2/MODEENTRY_SPEED);
  // 300us Wait
  load_vlf_offset(_SUB_MODEENTRY_RAM,2, 70000/MODEENTRY_SPEED);
  ...
  load_vlf_offset(_SUB_MODEENTRY_RAM,3,tempo1);
  // Vdd/Vddh On/Off
  load_vlf_offset(_SUB_RAMBT_END,0,1);
  // RAMBootEnd Wait = waittime x(500x200x2000ns)
  load_vlf_offset(_SUB_RAMBT_POLL,0,500*waittime/speed);
  // RAMBootPoll Wait = waittime x(500x200x200ns)


  SetVohVolVtt( 15*vcc[0]/100, 15*vcc[0]/100, 5*vcc[0]/10 , bistpoll_pins );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
  ...
  if((monitor_mode & ADC_DIGI) !=0){
    ...
    load_vlf_offset(_SUB_MODEENTRY_RAM_CRY,6,  3*1000/MODEENTRY_SPEED);
    // 1us Wait
    load_vlf_offset(_SUB_MODEENTRY_RAM_CRY,1,300000/2/MODEENTRY_SPEED);
    // 300us Wait
    load_vlf_offset(_SUB_MODEENTRY_RAM_CRY,2, 100/MODEENTRY_SPEED);
    // 70ms Wait
    load_vlf_offset(_SUB_MODEENTRY_RAM_CRY,3,tempo1);
    // Vdd/Vddh On/Off
  ...
  if((monitor_mode & CRY_CLK) !=0){
    ...
    set_pps_resource(PPS_PMU_IRNG,PPS_50mA_RNG);
    //PMU 50mA Setting
    set_pps_resource(PPS_PMU_ILIMIT,50*1000*1000);
    //PMU ILIMIT = 50mA

      set_pps_resource(PPS_PMU_SENSE_CTRL,
      // PPS_REMOTE_SENSE
      delay_timer(1*1000);
      //1ms wait
...
// ******* BGR Code Check ********
  ...
  //�e�d���̋�����Level�̐ݒ�(DAXS�]���p)
    ...
    set_pps_resource(PPS_PMU_ILIMIT,50*1000*1000);
    //PMU ILIMIT = 50mA
    set_pps_resource(PPS_PMU_VLIMIT,daxs_trans.vcc);
    //PMU V = Vcc
...
// ****************************************************************************
  ...
  SetVohVolVtt( 10*vcc[0]/100, 10*vcc[0]/100, 5*vcc[0]/10 , bistpoll_pins );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
  ...
  if(mode & MONITOR_MODE){
    ...
    switch(selmode){
      case AUTOTRIM_VRSG:
  load_vlf_offset(_SUB_RAMBT_POLL,0,2);
  // RAMBootPoll Wait = waittime x(500x200x200ns)
          run_apg(_RAM_BOOT_CTRL);
	  ...
	  run_apg(_RAM_BOOT_CTRL);
          ...
          load_vlf_offset(_SUB_RAMBT_POLL,0,1000*waittime/speed);
          // RAMBootPoll Wait = waittime x(500x200x200ns)
          set_vlimit(3800);
          // 3.8V���VrsgMaxOver���
	  run_apg(_RAM_BOOT_MONITOR_CTRL);
          load_vlf_offset(_SUB_RAMBT_DEND_POLL,0,1000*(waittime+4)/speed);
          // RAMBootPoll Wait = waittime x(500x200x200ns)
...
// ****************************************************************************
  ...
  set_strobe_mask(dpass_tcheck_pins);
  // dpass��TcheckPin�̔�r�ݒ�
  select_vector_mode(tcheck_poll_pins,_PE_APG_MODE);
  //Tcheck��ECR�ɓ]���̂��߁AAPG-Mode�ɕύX
  select_vector_mode(dpass_pins,_PE_VECTOR_MODE);
  //dpass��Vector-Mode�ɐݒ�(�ύX���������O�̂���)
...
// ******* APG SetUp(Wait) ********
  ...
  SetVohVolVtt( 7*vcc[0]/10, 7*vcc[0]/10, 5*vcc[0]/10 , dpass_pins );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)

// ******* Test Start ********

  for( cnt=0x0000; cnt<=0x8000; cnt=cnt+4 ){
  //�g�p�̈�(TCHECK�̃��O�p)��ECR���N���A
      WriteEcrMode8( cnt, 0xFFFFFFFF, SIZE_L );
  }
   
    start_time = bentime();
    // Get Test Start Time
    run_apg (_RAM_BOOT_TCHECK_MONITOR);
    //RamBoot��Tcheck��ECR�ɓ]���܂�
  ...
  //RamBoot�I��
  *selvol = 0;
  for( cnt=0x0000; cnt<=0x50FF; cnt++ ){
  //ECR�̈�̃A�h���X���[�v
    if((cnt%4==0) || (cnt%4)==1){
    //H/L���肪�A�h���X�ɂ���ċt�]���Ă����̂ŁA���}���u(�����s��)
	  ecr_data = read_ecr(cnt)&0xff;
    }else{
	  ecr_data = read_ecr(cnt)&0xff;
	  ecr_data = 0xFF - ecr_data;
	  //H/L�̋t�]�����{
	}
	if(ecr_data == 0) *selvol=*selvol + 1;
	//Tcheck��H���Ԃ̃N���b�N�����v�Z
    //���̓f�o�b�O�p
		//if((cnt % 0x100 == 0) || (cnt % 0x100 == 3)) printf("Addr = 0x%04X : %02X : %02X : %d",cnt,read_ecr(cnt)&0xff,ecr_data,*selvol);
  }
    //���̓f�o�b�O�p
...
// ******* PowerUp ********
  ...
  for( cnt=RAM_TOP; cnt<=RAM_END; cnt=cnt+4 ){
    ...
    result = RAMRunApg( 2, 0x2A00, OUTPUTIF_END );
    // DAXS transport 0=write command
    RAMRunApg( 2, FBM_READTOP ,(FBM_READTOP + FBM_READSIZE - 1) );
    // Daxs Trans For FBM
  sprintf(outbuf," ----- Result RAM Read End -----");DataOut(Ffpt,outbuf);
...
// ------------------------------------------------------------------------------ Function:   GlobalHardwareInit  Description:  Performs tester setup operations that are only required once, after program is loaded. ------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
    ...
    map_xover_connection ( _DUT_XOVER,       _YGEN, 0, 0 );
    //V4000
    ...
    map_bmecr_xover_connection ( ecr1Memory,       _YGEN, 0, 0 );
    //V4000
...
#if defined(V5KONLY)
    connect_ps (PPS_CON_VCC1_A, PPS_CMODE_SUPPLY);
    // V1 PPS
    connect_ps (PPS_CON_VPP1, PPS_CMODE_SUPPLY);
    // V2 PPS
    connect_ps (PPS_SITE(1,PPS_CON_VCC1_A), PPS_CMODE_SUPPLY);
    // V3 PPS
    connect_ps (PPS_SITE(1,PPS_CON_VPP1), PPS_CMODE_SUPPLY);
    // V4 PPS
    connect_ps (PPS_CON_VNN1, PPS_CMODE_SUPPLY);
    // v5 PPS

#elif defined(V4KONLY)
    connect_ps (PPS_CON_V1, PPS_CMODE_SUPPLY);
    // V1 PPS
    connect_ps (PPS_CON_V2, PPS_CMODE_SUPPLY);
    // V2 PPS
    connect_ps (PPS_CON_V3, PPS_CMODE_SUPPLY);
    // V3 PPS
    connect_ps (PPS_CON_V4, PPS_CMODE_SUPPLY);
    // V4 PPS
    connect_ps (PPS_CON_V5, PPS_CMODE_SUPPLY);
    // v5 PPS
...
// ================================================================================================
{
  ...
  case 1:
  // Read RAM
// load_vlf_offset(_SUB_DAXSTRANS_ADDR,0 ,(Byte)((entryaddr>>24)&0x3Ful)|0x80); load_vlf_offset(_SUB_DAXSTRANS_ADDR,6 ,(Byte)((entryaddr>>16)&0xFFul)); load_vlf_offset(_SUB_DAXSTRANS_ADDR,14,(Byte)((entryaddr>>8 )&0xFFul)); load_vlf_offset(_SUB_DAXSTRANS_ADDR,22,(Byte)((entryaddr    )&0xFFul)); load_vlf_offset(_DAXS_READ_BM,1,yinc_first); load_vlf_offset(_DAXS_READ_BM,2,xinc); load_vlf_offset(_DAXS_READ_BM,3,yst); load_vlf_offset(_DAXS_READ_BM,4,xst); load_vlf_offset(_DAXS_READ_BM,5,yinc_last); run_apg(_DAXS_READ_BM);
return(FAIL);
    break;
...
// ! Get Lot Name @param char *lotname [out] Lot Name @param int size [in] lotname Buffer Size @return void
...
// ! Get Wafer Name @param char *wafername [out] Wafer Name @param int size [in] wafername Buffer Size @return void
...
#ifdef BIGENDIAN
 // *********************************************************************** Read Error Catch RAM Mode BIG ENDIAN (addr:DeviceAddr size:4->longword / 2->word / 1->byte) ***********************************************************************
int ReadEcrMode8(int addr,int size)
{
 ...
 // *********************************************************************** Write Error Catch RAM Mode BIG ENDIAN (addr:DeviceAddr data:WriteData size:4->longword / 2->word / 1->byte) ***********************************************************************
void WriteEcrMode8(int addr,int data,int size)
{
 ...
 // *********************************************************************** Read Buffer Memory Mode BIG ENDIAN (addr:DeviceAddr size:4->longword / 2->word / 1->byte) ***********************************************************************
int ReadBmMode8(int addr,int size)
{
 ...
 // *********************************************************************** Write Buffer Memory Mode BIG ENDIAN (addr:DeviceAddr data:WriteData size:4->longword / 2->word / 1->byte) ***********************************************************************
...
#else
 // *********************************************************************** Read Error Catch RAM Mode LITTLE ENDIAN (addr:DeviceAddr size:4->longword / 2->word / 1->byte) ***********************************************************************
int ReadEcrMode8(int addr,int size)
{
  switch(size){
  case SIZE_B:
 ...
 // *********************************************************************** Write Error Catch RAM Mode LITTLE ENDIAN (addr:DeviceAddr data:WriteData size:4->longword / 2->word / 1->byte) ***********************************************************************
void WriteEcrMode8(int addr,int data,int size)
{
  switch(size){
  case SIZE_B:
 ...
 // *********************************************************************** Read Buffer Memory Mode LITTLE ENDIAN (addr:DeviceAddr size:4->longword / 2->word / 1->byte) ***********************************************************************
int ReadBmMode8(int addr,int size)
{
  switch(size){
  case SIZE_B:
 ...
 // *********************************************************************** Write Buffer Memory Mode LITTLE ENDIAN (addr:DeviceAddr data:WriteData size:4->longword / 2->word / 1->byte) ***********************************************************************
void WriteBmMode8(int addr,int data,int size)
{
  switch(size){
  case SIZE_B:
...
int ParamTransBM(interface_t* param )
{
  int cnt,param_data, bm_data;
    ...
    // ******* Parameter BM Write ********
  for(cnt=0;cnt<IN_BYTEEND;cnt++){
  // write register(byte) data
    WriteBmMode8((param + cnt) -> addr,(param + cnt) -> value,(param + cnt) -> size);
  }
  for(cnt=IN_BYTEEND+1;cnt<IN_LWORDEND;cnt++){
  // write other data
    WriteBmMode8((param + cnt) -> addr,(param + cnt) -> value,(param + cnt) -> size);
  }
  for(cnt=OUTPUTIF_TOP;cnt<OUTPUTIF_END;cnt+=4){
  // write dummy data
    WriteBmMode8(cnt,0xFFFFFFFF,SIZE_L);
  }
    ...
    // *** Comp parameter <=> BM ***
  for(cnt=0;cnt<IN_BYTEEND;cnt++){
  // Comp register(byte) data
    param_data = ((param + cnt) -> value);
    bm_data    = ReadBmMode8((param + cnt) -> addr,(param + cnt) -> size);
    if( param_data != bm_data ){
      sprintf(outbuf,"  TRANS ERROR1!!(param => BM) addr=H'%X data: H'%X(param)<=>H'%X(BM)", (param + cnt) -> addr,param_data, bm_data);
      DataOut(Ffpt,outbuf);
      return(FAIL);
    }
  }
  ...
  // comp other data
    ...
    bm_data    = ReadBmMode8((param + cnt) -> addr,(param + cnt) -> size);
    if( param_data != bm_data ){
      sprintf(outbuf,"  TRANS ERROR2!!(param => BM) addr=H'%X data: H'%X(param)<=>H'%X(BM)", (param + cnt) -> addr,param_data, bm_data);
      DataOut(Ffpt,outbuf);
      return(FAIL);
    }
  }
  ...
  // comp dummy data
    ...
    if( param_data != bm_data ){
      sprintf(outbuf,"  TRANS ERROR3!!(param => BM) addr=H'%X data: H'%X(param)<=>H'%X(BM)", (param + cnt) -> addr,param_data, bm_data);
      DataOut(Ffpt,outbuf);
      return(FAIL);
    }
  }
 ...
 // ****************************************************************** Vth-Search Function judge = 0:����Search judge = 1:�㐞Search ******************************************************************
...
int VthReadSearch(int judge, int vcc[NUM_POWERSUPPLY],interface_t* param,int waittime,int delta)
{
  ...
  if(judge == 0){
  // - => +
    bdata01_bak = SetValue(IN_BDATA01,param,0x00);
    //All"0" judge
    delta_vol =   delta*1;
    //Step : +0.5V
    start_vol =   0;
  ...
  // + => -
    bdata01_bak = SetValue(IN_BDATA01,param,0x01);
    //All"1" judge
    delta_vol =  delta*-1;
    //Step : +0.5V
    start_vol =  0;
  ...
  // Error
    printf("judge number Error!!! judge = %d",judge);
    return(FAIL);
  }

    // param Set 
  speed = SetValue(IN_EXTAL1,param,500);
  //2MHz CLK SET
  ...
  for (k = 1; k <= 1; k++){
    ...
    printf("Start =%dmV, End = %dmV, Step = %dmV",start_vol,end_vol,delta_vol);
    for (i = 0; i <= step; i++){
      selvol = start_vol + i * delta_vol;
	  	  //printf("enter selvol ->");DigitInput(&selvol);

      if(selvol <=-3500){
        printf("VthRead level Error!!! level = %dmV",selvol);
        return(FAIL);
      }else if(selvol <= -2000){
        SetValue(IN_BDATA00,param,0x01);
        //Select Vth-Read1
        printf("Caution! VthRead1 level < -2.0V , level = %dmV",selvol);
	mode = VSSMON_APP;
      }else if(selvol <= 0){
        SetValue(IN_BDATA00,param,0x01);
        //Select Vth-Read1
      ...
      printf("Vth-Read Start level = %dmV",selvol);
	  SetValue(OUT_JUDGE1,param,0);
      result = CpuModeMonitor(mode, vcc, param,waittime,CreatePatFileName("readall_code_at"),"CPU READ VthSearch", selvol);
      // Vth-Read ���s
...
int VthReadDist(int vth_mode, int bit_count, int min_vol, int delta_vol, int vcc[NUM_POWERSUPPLY],int waittime, interface_t* param, char file_name[128])
{
  ...
  if(GetValue(IN_BDATA08,param) == 0){
  //BDATA08=0 -> Iref=2.5uA
...
//#if(0)
  ...
  i = 0;
  printf("speed = %d",GetValue(IN_EXTAL1,param));
  switch (vth_mode){
    case 1:
      mode = VSSMON_APP;
      //Connect PMU <=> VSSMON
  ...
  printf("vth_mode = %d, min_vol = %d, File name = %s",vth_mode,min_vol,file_name);

  bdata00_bak = SetValue(IN_BDATA00,param,vth_mode);
  // VthRead num select
  ...
  //Vth-Read Loop min_vol~max_bit Hit(max_vol)

    if ( test_item == 12 ) {
      if ( ( selvol > 0 ) && ( vth_mode !=2 ) ) {
        vth_mode = 2;
        SetValue(IN_BDATA00,param,vth_mode);
        // VthRead num select  
		inif_flag = 1;
      }
    }
    ...
    if(inif_flag ==1){
      inif_flag = 0;
      repeat_mode = repeat_mode | INIF_SET | RAMBOOT_CHANGE;
      //����IF�ύXMode�ǉ�
    }

        //result = CpuModeMonitor(mode, vcc, param,waittime,CreatePatFileName(pat_name),"CPU READ VthRead", &selvol); // Vth-Read ���s
    result = CpuModeMonitor_Repeat(repeat_mode,mode, vcc, param,waittime,CreatePatFileName(pat_name),"CPU READ VthRead", &selvol);
    // Vth-Read ���s
    ...
    vth_data[1][i] = GetValue(OUT_LDATA0,param);
    //Vth Read Count Set
    vth_data[0][i] = selvol;
    //Vth Read level Set
    
	i=i+1;
    if(bit_count <= vth_data[1][i-1]){
    //Max Bit Hit
      printf(" Bit Count Hit Vol = %dmV, count=%d <= %d",selvol,bit_count,vth_data[1][i-1]);
      break;
    }
  }

  repeat_total = (bentime()-start_time-init_bentime())/1000;
  // Get Test Total Time
  printf("Total Time %dms",repeat_total);
  DeviceLevelsPowerDown();
  //�e�d��OFF�R�}���h
...
int VthEdgeSearch(int vth_mode,int min_deltavol, int vcc[NUM_POWERSUPPLY],int waittime, interface_t* param)
{
  ...
  stack1 = SetValue(IN_BDATA01,param,0);
  //BDATA01 Clear
...
//#if(0)
  ...
  while(delta_vol >= min_deltavol){
    for (selvol = min_vol; selvol <= max_vol; selvol = selvol + delta_vol){
      printf("selvol=%d[mV],maxvol=%d[mV],minvol=%d[mV],deltavol=%d[mV]",selvol,max_vol,min_vol,delta_vol);
// range change
...
//      }
  ...
  switch(vth_mode){
    case 1:
    //0V->-0.5V->-1.0V
      stack5 = SetValue(IN_LDATA0,param,0);
      //Initial Voltage
      stack6 = SetValue(IN_LDATA1,param,delta_vol * (-1));
      //First Delta Voltage
      stack7 = SetValue(IN_LDATA2,param,min_vol * (-1));
      //Last Delta Voltage
      SetValue(IN_BDATA01,param,0);
      //ExpReadData 0:0x00000000, 1:0xFFFFFFFF
    break;
    case 2:
    //-2.5V->-2.0V->-1.5V
      stack5 = SetValue(IN_LDATA0,param,-2500);
      //Initial Voltage
      stack6 = SetValue(IN_LDATA1,param,delta_vol * (+1));
      //First Delta Voltage
      stack7 = SetValue(IN_LDATA2,param,min_vol * (+1));
 ...
 // ******************************************************************* Make Vth Read Bunpu File vth_mode : VthRead mode number vth_data : VthRead Count data_num : Bunpu number step     : VthRead step number *******************************************************************
...
int DistributionDataOut(int vth_mode ,int vth_data[2][105] ,int data_num ,int step ,char file_name[128], interface_t* param)
{
  ...
  printf("vth_mode_list = %d",vth_mode);
  if(result != PASS){
  // File Name Check
  ...
  for(i=step;i>=0;i--){
    printf("  %5dmV, %9d",vth_data[(k*2)][i],vth_data[(k*2)+1][i]);
  }
  printf("----------------------------");
...
//      sprintf(outbuf,"VthRead%d",vth_mode); //1��� s
      DataOut(Ffpt,outbuf);
    }
...
//  printf("PatSize[0] = 0x%X",PatSize[0]);
//  printf("PatSize[1] = 0x%X",PatSize[1]);
  ...
  }else if(GetValue(IN_BDATA09,param) == 3){
  //Minori Read for Cry
        //-- Addr Set for BM --//
	...
	delay_timer(waittime*100);
	//waittime * 0.1ms Wait
    comp_write_lbreg_pe(RNORMAL);
    run_apg(_MINORI_CRY_END);

  }
        //printf("  Ipar_judge = %X",ipar_judge);
  ...
  ALL_H_flag = 1;
	for(i=MINORI_BM_TOP_ADDR_OUT;i<MINORI_BM_TOP_ADDR_OUT+PatSize[1];i++){
      ...
      if( (buff_bm & 0x10) != ((0xFF - buff_ecr) << 1) ){
		    		    //printf("MINORI-FAIL Addr = 0x%X, BM = 0x%X, ECR = 0x%X",i,(buff_bm & 0x30),(0xFF - buff_ecr)>>3);
		    result = FAIL;
  		}
	  }
	}
...
int MinoriMonitor_Vrsg_First (int mode,int vcc[NUM_POWERSUPPLY],int waittime,interface_t* param,char *pat,char *mess, int *selvol)
{
  ...
  fill_bmecr(bm1Memory, 0xff);
  // full bm with 0xFF
  fill_bmecr(ecr1Memory, 0xff);
  // full ecr with 0xFF
// ******* BGR Code Check ********
  BGR_REG_CHECK(vcc[0],param);
  ...
  set_drive_time_ns( clk_pin, 2, 0, speed/8 );
    ...
    if((ReadEcrMode8(MINORI_BM_TOP_ADDR+0x1,SIZE_L)== 0x00)&&
      ...
      result = PASS;
             ...
             (fail_data != 0x00)){
      sprintf(outbuf," -- Minori Read Fail --    Fail Data = 0x%X",fail_data);
      result = FAIL;
    }else{
      sprintf(outbuf," -- Minori Read Error!! --");
      DispEcrBmData( MINORI_BM_TOP_ADDR,0x40 , DISP_ECR|DISP_1 );
...
// ******* PowerUp ********
  ...
  set_drive_time_ns( clk_pin, 2, 0, speed/8 );
    ...
    if(GetValue(IN_BDATA09,param) == 0){
	  // //-- Addr Set for BM --// load_vlf_offset(_MINORI_MAIN,1,addr_yin); load_vlf_offset(_MINORI_MAIN,2,addr_xin); load_vlf_offset(_MINORI_MAIN,0,addr_inc); load_vlf_offset(_MINORI_MAIN,13,waittime);   run_apg(_MINORI_MAIN);
      load_vlf_offset(_MINORI_MAIN_BI_STA,1,addr_yin);
      load_vlf_offset(_MINORI_MAIN_BI_STA,2,addr_xin);
      load_vlf_offset(_MINORI_MAIN_BI_STA,0,addr_inc);
      load_vlf_offset(_MINORI_MAIN_BI_STA,13,waittime);
	...
	//10ms Delay
      ...
      load_vlf_offset(_MINORI_LOOP,1,1);
		...
		//printf("MinoriMonitor_Vrsg_Repeat : selvol = %dmV",*selvol);
    sprintf(outbuf,"<<  MINORI End  >>");
    DataOut(Ffpt,outbuf);
...
int TDR210(interface_t* param)
{
  ...
  else{ printf("Mat Select Error!!!"); return;}  tempo2 = 0x0;

  tdr00_data = tdr00_data + (GetValue(IN_BDATA03,param) << 30);
  // Bdata03 : CLK Setting

  tdr01_data = tdr01_data + (tempo2 << 30);
  // Module Select(FLP0 ~ FLI3)
  tdr01_data = tdr01_data + (tempo1 << 27);
  // Module Select(user or extra)
  tdr01_data = tdr01_data + (GetValue(IN_TESTSEL,param) << 24);
  // TestSel : Check Pattern
  tdr01_data = tdr01_data + (GetValue(IN_BDATA00,param) << 23);
  // Bdata00 : �X�N�����u��
  tdr01_data = tdr01_data + (GetValue(IN_BDATA01,param) << 14);
  // Bdata01 : LowPowerRead
  
    //tdr02_data = tdr02_data + (GetValue(IN_BDATA02,param) << 26); // Bdata02 : Back-Bias
  
    //For ReadBI-Debug
  if(GetValue(IN_BDATA09,param) == 1){
    tdr00_data = tdr00_data + (0x1 << 6);
    //TDR00[6]     =    1
    tdr00_data = tdr00_data + (0xB << 7);
    //TDR00[10:7]  = 1011
    tdr00_data = tdr00_data + (0x1 << 11);
    //TDR00[11]    =    1
    tdr00_data = tdr00_data + (0xE << 12);
    //TDR00[15:12] = 1110
    tdr00_data = tdr00_data + (0x1 << 23);
    //TDR00[23]    =    1
  }
    //For ReadBI-Debug
...
// ******* PowerUp ********
  ...
  for(addr=MINORI_BM_TOP_ADDR_OUT;addr<=(MINORI_BM_END_ADDR_OUT+0xFF);addr=addr+4){
  //BM All"0" Set
...
int TDR_BGR_Trim(int Target,int vcc[NUM_POWERSUPPLY],int waittime,interface_t* param)
{
  ...
  VrsgDelta = 10;
   ...
   while(end_flag != 1){
     ...
     vrsg_vol = vnum = 0;
     for(i=1;i<=3;i++){
       // load_vlf_offset(_MINORI_TDR04,18,0x05); //ETLR Entry Code run_apg(_MINORI_TDR04);  load_vlf_offset(_MINORI_TDR03,33,0x00); //ETLR00~01 Address load_vlf_offset(_MINORI_TDR03,17,GetValue(IN_ETLR00,param) ); run_apg(_MINORI_TDR03);  load_vlf_offset(_MINORI_TDR04,18,0x07); //ETLR Close Code run_apg(_MINORI_TDR04);  delay_timer(10*1000); //10ms Wait selvol[i-1] = read_adc_ave(ADC_VMEAS,0,100);
    ...
    SetValue(IN_BDATA00,param,0);
     ...
     printf("Debug1 : HighReg0x%X LowReg0x%X Reg0x%X[0x%X] vrsg_vol = %dmV ",HighReg,LowReg,SetReg,GetValue(IN_ETLR00,param),vrsg_vol);
     
     if((vrsg_vol <= (Target + (VrsgDelta/2))) && (vrsg_vol >= (Target - (VrsgDelta/2)))){
     //�g���~���OPass
       end_flag = 1;
...
int CpuModeFuncLogic(int mode,int mode_sram,int vcc[NUM_POWERSUPPLY],int waittime,interface_t* param,char *pat,char *mess, int in_ldata[16])
{
  ...
  speed=GetValue(IN_EXTAL_PS,param);
    ...
    // ****** DISP TAITOL ******
    //Vref��VrefH�̋L�q�ɕύX�K�v�H
  sprintf(outbuf,"TEST:%d [ %s ] vcc=%dmV ext_Vrefh=%dmV ext_Vref=%dmV F=%2dps PLL=%d(X%d) ", test++,mess,vcc[0],vcc[1],vcc[2],speed,GetValue(IN_PLLON,param),GetValue(IN_PLL_MULT,param)); DataOut(Ffpt,outbuf);
  sprintf(outbuf,"Module:%s%s", DispMatName(mat_str,param),disp_addres); DataOut(Ffpt,outbuf);
    //Ldata�̌����ɕύX����K�v�L
  sprintf(outbuf,"BDATA00:%08X, LDATA0:%08X", GetValue(IN_BDATA00,param),GetValue(IN_LDATA0,param)); DataOut(Ffpt,outbuf);
...
// ****************************************************************************
  ...
  if(FAIL==datasize) {
  // path/fail check
    sprintf(outbuf," !!! NOT OPEN FILE PATH -> %s (checksum:H'%X) !!!",binpat,datasize); DataOut(Ffpt,outbuf); DeviceLevelsPowerDown();
    return(NOPATFAIL);
  }else{
    sprintf(outbuf," LOAD FILE PATH -> %s (checksum:H'%X)",binpat,datasize); DataOut(Ffpt,outbuf);
  }
...
// ****************************************************************************
    // ******* Start tests ********
    //printf("Write RAM ");               // Print Retry Count  RAMRunApg( 0, RAM_TOP, OUTPUTIF_END );   /* DAXS transport 0=write command */


    //printf("Read RAM ");                // Print Retry Count  SetTimming_NS( daxs_trans.read_rate );

  set_strobe_mask(daxs_datao_pin);
  //Compare Setting command

  result = RAMRunApg( 2, RAM_TOP, OUTPUTIF_END );
  // DAXS transport 2=ecr dump command

  for(i=RAM_TOP;i<=OUTPUTIF_END;i=i+4){
    bm_data=ReadBmMode8(i,SIZE_L);
    ecr_data=ReadEcrMode8(i,SIZE_L);
    if(bm_data!=ecr_data){
      sprintf( outbuf, "BM <=> ECR Comp ERROR!!! Addr = %X, BM = %X, ECR = %X ",i,bm_data,ecr_data ); DataOut(Ffpt,outbuf);
      result = FAIL;  break;
    }
  }
...
DAXS_RAMBOOT:
  ...
  if( NULL!=strstr( pat, "bgr_monitor") ){
    run_apg (_RAM_BOOT_CTRL_RESH);
    // Execute Test Pattern
    set_pmu_range( UA250 );
    // range NA2500, UA25, UA250, UA2500, MA25, MA250 MA1000
    comp_set_ilimit( 200000 );
    // PMU +-xxmA clamp  200uA
    comp_set_iparh( 200000 );
    // PMU meas Hi current judgh
    set_vlimit( 0 );
    pmu_delay( 1000 );
    // PMU Conect delay(us) 1ms
    ...
    sprintf( outbuf,"ATBM1(DATAI) = %d[mV]  %d[nA]", tempo2, tempo1 ); DataOut( Ffpt, outbuf );
    reconnect_pin( daxs_datai_pin );
    connect_pmu( vcl1_pin );
    SITE_MASK( 1, tempo1 = read_pmu_status( 0x08 ) );
    SITE_MASK( 1, tempo2 = read_adc_ave( ADC_VMEAS, 0, 10 ) );
    SITE_MASK( 1, tempo1 = read_adc_ave( ADC_IMEAS, 0, 10 ) );
    sprintf( outbuf,"VCL1 = %d[mV]  %d[nA]", tempo2, tempo1 ); DataOut( Ffpt, outbuf );
    reconnect_pin( vcl1_pin );
    connect_pmu( vcl2_pin );
    SITE_MASK( 1, tempo1 = read_pmu_status( 0x08 ) );
    SITE_MASK( 1, tempo2 = read_adc_ave( ADC_VMEAS, 0, 10 ) );
    SITE_MASK( 1, tempo1 = read_adc_ave( ADC_IMEAS, 0, 10 ) );
    sprintf( outbuf,"VCL2 = %d[mV]  %d[nA]", tempo2, tempo1 ); DataOut( Ffpt, outbuf );
    ...
    sprintf( outbuf,"VBB(VBP) = %d[mV]  %d[nA]", tempo1, tempo2 ); DataOut( Ffpt, outbuf );
    reconnect_pin( vbp_pin );
    connect_pmu( vbn_pin );
    SITE_MASK( 1, tempo1 = read_adc_ave( ADC_VMEAS, 0, 10 ) );
    SITE_MASK( 1, tempo2 = read_adc_ave( ADC_IMEAS, 0, 10 ) );
    sprintf( outbuf,"VBB(VBN) = %d[mV]  %d[nA]", tempo1, tempo2 ); DataOut( Ffpt, outbuf );
    reconnect_pin( vbn_pin );
    run_apg (_RAM_BOOT_CTRL_RESL);
  }else if( NULL!=strstr( mess, "UserBoot" ) ){
    load_vlf_offset(_SUB_MODEENTRY_ROM,1,  1000/2/speed);
    // 1us Wait
    load_vlf_offset(_SUB_MODEENTRY_ROM,2,300000/2/speed);
    // 300us Wait
    load_vlf_offset(_SUB_MODEENTRY_ROM,3, 70000/speed);
    // 70ms Wait
    load_vlf_offset(_SUB_MODEENTRY_ROM,4,tempo1);
    // Vdd/Vddh On/Off
  ...
  // Boot����Error�o�͂�APG�p�^�����璊�o(DPASSDEND_CP�̔�r����)
  Cpuferr = cpuerr;
  if( CheckFK(FK_DISP_DEBUG) ){ sprintf(outbuf,"cpuerr=%d ",cpuerr); DataOut(Ffpt,outbuf); }
  // ****************************************************************************
  select_vector_mode(poll_pins,_PE_APG_MODE);
  reconnect_pin(poll_pins);
  set_strobe_mask(daxs_datao_pin);
...
// ******* PowerUp ********
  ...
  if( NULL!=strstr( pat, "read_dump") ){
    ...
    else                                        { tempo1 = 0x28000; tempo2 = 512; }
    // UIAREA
    for( cnt=RAM_TOP+0x8000; cnt<=RAM_TOP+tempo1; cnt++ ) write_ecr( cnt, 0xFF );
    RAMRunApg( 2, RAM_TOP + 0x8000, RAM_TOP + 0x8000 + tempo2*0x100 + 0xFF );
    // RAM->ECR ReadOut
  }
...
// ******* end manage ********
  ...
  if( cpuerr==1 ){
    if( ReadEcrMode8(0x3D00,SIZE_L)==0xAAAAAAAA ){
              //printf("Test FAIL!!!  %08X %d", ReadEcrMode8(0x3D00,SIZE_L), cpuerr );
       printf("-- FAIL -- Cpu total=%dms",Cputotal);
       return(FAIL);
    }else if( ReadEcrMode8(0x3D00,SIZE_L)==0x55555555 ){
              //printf("Test PASS!!!  %08X %d", ReadEcrMode8(0x3D00,SIZE_L), cpuerr );
       printf("** PASS ** Cpu Total=%dms",Cputotal);
       return(PASS);
    }else{
              //printf("Test ECR data ERROR!!!");
       DispEcrBmData(0x3D00 , 0x0100, DISP_ECR|DISP_1 );
       printf("-- FLAG_FAIL -- Cpu Total=%dms",Cputotal);
       return(FLGFAIL);
    }
  }
  printf("-- FAIL -- !!! TimeOver !!! total=%dms",Cputotal);
...
// ****** SETUP ECR/BM ******
  ...
  SetTimming_NS( 80 );
  // 80ns->12.5MHz
  ChangeTimming_GTSLTS_LP( EXTCYLJ , 1, 3000  );
  // 3000ns  -> 333KHz
    //ChangeTimming_GTSLTS_LP( EXTCYLM4, 2, speed );// 30000ns ->  33KHz
  ChangeTimming_GTSLTS_LP2( EXTCYLM4, 2, speed );
  // 30000ns ->  33KHz
...
// ******* FILE BM WRITE *********
  ...
  if(GetValue(IN_BDATA00,param) == 1){
    ...
    printf("
    /// ----- Debug Mode ----- ///");
    printf("0x%X = %X",TS15_13_addr + 0x4C,ReadEcrMode8(TS15_13_addr + 0x4C,SIZE_L));
	  printf("0x%X = %X",TS15_13_addr + 0x93,ReadEcrMode8(TS15_13_addr +0x93,SIZE_L));
...
// ****************************************************************************
  ...
  reconnect_pin(poll_pins);
  //180314 Add-morioka For RamBoot
  select_vector_mode(lp_clk,_PE_VECTOR_MODE);
  // APG mode select
  ...
  ChangeTimming_GTSLTS( EXTCYLJ, 1, MODEENTRY_SPEED );
  // ModeEntry���g���ݒ� GTS=1,20MHz
  ChangeTimming_GTSLTS( EXTCYLM, 0, speed );
  // ModeEntry���g���ݒ� GTS=1,20MHz
...
// ******* APG SetUp(Wait) ********
  ...
  load_vlf_offset(_SUB_MODEENTRY_RAM,3,tempo1);
  // Vdd/Vddh On/Off
  load_vlf_offset(_SUB_RAMBT_END_LPREAD,0,waittime);
  // RAMBootEnd Wait = waittime x(500x200x2000ns)
...
// ****************************************************************************
  ...
  // I-Limit = 0 �d����Chip������������邽�� 
    ...
    // ** Comp Pin SetUp **
  set_strobe_mask(dpass_pins);
  // ��rPin�̐ݒ�(RAMBoot�����܂ł̑ҋ@���ԂɎg�p)
  select_vector_mode(dpass_pins,_PE_VECTOR_MODE);
  //180314 Add-morioka For RamBoot
  SetVohVolVtt( 75*vcc[0]/100, 75*vcc[0]/100, 5*vcc[0]/10 , bistpoll_pins );
  //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
  reconnect_pin(dpass_pins);
    ...
    load_vlf_offset(_SUB_RAMBT_POLL,0,500/speed);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_DPASS_H_POLL,0,0xFF/speed);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_DPASS_H_POLL,1,100*0x5FB/100);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_DPASS_L_POLL,0,0xFF/speed);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_DPASS_L_POLL,1,100*0x5FB/100);
    // 20ms fix(2000*200*200ns)
    run_apg(_RAM_BOOT_MONITOR_CTRL);
    delay_timer( 2000 );
    ...
    //Bdata05�̐��l�����j�^�[���[�v�����s����
	    if(i == GetValue(IN_LDATA1,param)){
	    //LDATA1�̃^�C�~���O��VCCMON����VSSMON�̐ڑ��ɕύX����
	      comp_write_lbreg_pe(RVSSMON);
	    }
	    if(i >= GetValue(IN_LDATA0,param)){
	    //LDATA0�ȏ�̎���Iref���j�^�[�ɐ؂�ւ���
	      set_pmu_range(UA25);
		    comp_set_ilimit(25000);
		    // 25000nA = 25uA
		    set_vlimit(650);
      ...
      run_apg(_RAM_BOOT_DPASS_H_POLL);
      //DPASS�M����H�҂�
      delay_timer( 1000 );
      if(i < GetValue(IN_LDATA0,param)){
      //LDATA0�������͓d�����j�^�[
      ...
      //LDATA0�ȏ��Iref���j�^�[
	    ...
	    printf("Monitor %04d : %d",i,selvol[i]);
      run_apg(_RAM_BOOT_DPASS_L_POLL);
      //DPASS�M����L�҂�
    }
...
// ****************************************************************************
  ...
  reconnect_pin(dend_pin);
    ...
    start_time = bentime();
    // Get Test Start Time
    load_vlf_offset(_SUB_RAMBT_POLL,0,5);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_DEND_H_POLL,0,6666/speed);
    // 1ms fix
    load_vlf_offset(_SUB_RAMBT_DEND_H_POLL,1,700);
    // 50*1ms
    load_vlf_offset(_SUB_RAMBT_DEND_L_POLL,0,6666/speed);
    // 1ms fix
    load_vlf_offset(_SUB_RAMBT_DEND_L_POLL,1,700);
    // 50*1ms
    ...
    set_vlimit(GetValue(IN_LDATA0,param));
    // Set V = 0V
    comp_set_ilimit(250000);
    // I-Limit = 250000nA(250uA) 
    ...
    for(j=0;j<3;j++){
	  ...
	  if(absolute(vol_d) <= absolute(GetValue(IN_LDATA2,param))){
	  //Last Loop�p
      ...
      sprintf(outbuf,"Voltage = %d",vol);
      DataOut(Ffpt,outbuf);
		  set_vlimit(vol);
		  delay_timer(500);
		  //�d���ύX��̈��莞��(�e�X�^�[��)
      run_apg(_RAM_BOOT_DEND_L_WAIT);
      //DPASS�M����L�҂�
		  delay_timer(500);
      run_apg(_RAM_BOOT_DENDWAIT_POLLCHECK);
      //DPASS�M����H�҂�
      sprintf(outbuf,"RESULT = %X",check_pin_errors( dpass_pins, 4 ));
      DataOut(Ffpt,outbuf);
    ...
    }else if(GetValue(IN_TESTSEL,param) == 1){
    //VthCount Mode
      for(i = 0;i < GetValue(IN_BDATA00,param);i++){
        ...
        run_apg(_RAM_BOOT_DENDWAIT_POLLCHECK);
        //DPASS�M����H�҂�
		    		    //printf("RESULT = %X",check_pin_errors( dpass_pins, 4 ));
		    if(check_pin_errors( dpass_pins, 4 ) == 1)break;
      }
    }
    run_apg(_RAM_BOOT_DEND_L_WAIT);
    ...
    //�I�[��R�⃍�W�J��Pin��H/L����̓d���ݒ�(DAXS�]���p)
            ...
            //load_vlf_offset(_SUB_RAMBT_END,1,1000); // 50*cycle
      load_vlf_offset(_SUB_RAMBT_END,0,500*W1SEC*2/speed);
      // RAMBootEnd Wait = waittime x(500x200x2000ns)
      run_apg(_RAM_BOOT_MONITOR_END);
      // RAM Boot End(APG)
    set_vlimit(0);
...
// ****************************************************************************
  ...
  result = RAMRunApg( 2, 0x2000, OUTPUTIF_END );
  // DAXS transport 0=write command
...
// ****************************************************************************
  ...
  reconnect_pin(dend_pin);
    ...
    load_vlf_offset(_SUB_RAMBT_POLL,0,5);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_END_RES_H,0,5);
    // 20ms fix(2000*200*200ns)
    load_vlf_offset(_SUB_RAMBT_END_RES_H,2,0xFF);
    // 20ms fix(2000*200*200ns)
...
//    if(GetValue(IN_TESTSEL,param) == 0){//CPcurrent ALL Mode
      for(j=0;j<GetValue(IN_BDATA01,param);j++){
        switch(pmu_range){
          ...
          default:
            printf("Error!!! PMU range Setting = 0x%X",pmu_range);
            return(FAIL);
          break;
        }
        comp_set_ilimit(ilimit);
        set_vlimit(vol_s + (vol_d * j));
        // CP Voltage
        SITE_MASK (1, read_pmu_status(0x08));
        // I measurement
        ...
        if((ilimit/25*24) <= absolute((*(selvol+j)))){
        //Current Result limit over(+side)
          pmu_range++;
          ilimit = ilimit*10;
          tempo2 = 1;
        }else if((ilimit/25*2) >= absolute((*(selvol+j)))){
        //Current Result limit over(-side)
          pmu_range--;
          ilimit = ilimit/10;
          tempo2 = 1;
...
int CpuMode_Monitor_PC(int mode,interface_t* param,int waittime,char *pat,char *mess,
  ...
  int start_time;
  // Start Time For bentime
  int vs0,vs1,vs2,vcc[NUM_POWERSUPPLY];
  // Power bak
  int old_reg;
  char mat_str[20];
  char disp_addres[256];
...
// ****************************************************************************
  ...
  reconnect_pin(dpass_pins);
    ...
    for(i=0;i<GetValue(IN_BDATA00,param);i++){
      ...
      if((*(selvol+i+1) >= 24000) && (pmu_range == UA25)){
      //�d���l��24uA�ȏ�̎��͓d�����j�^�[�����Up���đ���
	      pmu_range = UA250;
        set_pmu_range(pmu_range);
		    comp_set_ilimit(250000);
	  ...
	  printf("%dkai : Iref_T = %dnA",i,*(selvol+1+i));
      ...
      for(j=0;j<GetValue(IN_BDATA01,param);j++){
        ...
        if((j+1) == GetValue(IN_BDATA01,param)){
          ...
          set_vih2(*(vc2+0));
          comp_set_v3(*(vc3+0));
	    printf("%dkai : PowerChange Vcc = %d, Vddh = %d, Vdd = %d",j,*(vc1),*(vc2),*(vc3));
        }else{
          VccSlow(*(vc1+j+1),*(vc1+j));
          set_vih2(*(vc2+j+1));
          comp_set_v3(*(vc3+j+1));
	    printf("%dkai : PowerChange Vcc = %d, Vddh = %d, Vdd = %d",j,*(vc1+j+1),*(vc2+j+1),*(vc3+j+1));
        }
        delay_timer( 1000 );
