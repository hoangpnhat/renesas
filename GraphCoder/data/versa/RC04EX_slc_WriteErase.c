// ****************************************************************************
// ! @file RC04EXslc_WriteErase.c @brief This is a 130nm(RC01SN) FLASH module user interface program for the 4000,5000 test system @author T.Morioka @date 2012.04.16 Rev.00
// ****************************************************************************
int WriteErase_Tokusei(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param)
{
  ...
  int atreg[3];
  //After Trimming Reg
  int result1[5][5],result2[5][5],result3[5][5];
  //middle suso search result[target_vhh][pulse_reg]
  char file_name[128];
...
// ************************************************************************************
  ...
  SetValue(IN_ETLR09,param, etlr_09 & 0x7F );
  // Iref  Temp ON
  ...
  vs[0] = 3300;
    for(m=0; m<3; m++){
      ...
      atreg[target_vhh] = GetValue(IN_ETLR17,param);
      //Get VhhW(afterTrim)
    }
  ...
  fprintf(file_log,"<<Write>> Vcc=%d,",set_vcc);
    ...
    // ** Pulse Loop **
      switch (j) {
        case 0: pulse_reg = 0x4; break;
        // 1ms
        case 1: pulse_reg = 0x6; break;
        // 3ms(+2ms)
        case 2: pulse_reg = 0x6; break;
        // 5ms(+2ms)
        case 3: pulse_reg = 0x6; break;
        // 7ms(+2ms)
        default: break;
      }
      ...
      if ( mat==0 ) {
        ...
        SetValue(IN_TOPADDR1 ,param,FLI1_TOP);
        SetValue(IN_TESTSIZE1,param,0x1000);
        // 4KB
      } else if ( mat==2 ) {
        SetValue(IN_TOPADDR2 ,param,FLI2_TOP);
        SetValue(IN_TESTSIZE2,param,0x1000);
        // 4KB
      }
...
// ************************************************************************************
  ...
  CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Erase");
    ...
    for(m=0; m<3; m++){
      ...
      atreg[m] = GetValue(IN_ETLR16,param);
      //Get VhhE(afterTrim)
    }
  ...
  fprintf(file_log,"<<Erase>> Vcc=%d,",set_vcc);
    ...
    // ** Pulse Loop **
      switch (j) {
        case 0: pulse_reg = 0x3; break;
        // 1ms
        case 1: pulse_reg = 0x4; break;
        // 3ms(+2ms)
        case 2: pulse_reg = 0x4; break;
        // 5ms(+2ms)
        case 3: pulse_reg = 0x4; break;
        // 7ms(+2ms)
        case 4: pulse_reg = 0x5; break;
        // 10ms(+3ms)
        default: break;
      }
...
// ************************************************************************************
  ...
  fprintf(file_log,"<<Erase-Houwa>> Vcc=%d,",set_vcc);
    ...
    // ** Pulse Loop **
      switch (j) {
        case 0: pulse_reg = 0x3; loop = 0; break;
        // 1ms
        case 1: pulse_reg = 0x4; loop = 0; break;
        // 3ms(+ 2ms)
        case 2: pulse_reg = 0x4; loop = 0; break;
        // 5ms(+ 2ms)
        case 3: pulse_reg = 0xF; loop = 0; break;
        // 10ms(+ 5ms)
        case 4: pulse_reg = 0xC; loop = 0; break;
        // 20ms(+10ms)
        case 5: pulse_reg = 0xC; loop = 3; break;
        // 50ms(+10ms*3)
        case 6: pulse_reg = 0xC; loop = 5; break;
        // 100ms(+10ms*5)
        default: break;
      }
      SetValue(IN_ETLR37,param,(GetValue(IN_ETLR37,param)&0xF0) | pulse_reg);
...
// ************************************************************************************
  ...
  for ( i=0 ; i<10 ; i++ ) {
    ...
    switch(i) {
      case 0: SetValue(IN_ETCR06,param,0x60);                               add_size=0x04000 ; min_vol = 300; fprintf(file_log,"MG64 ");  break;
      // MG64Line
      case 2: SetValue(IN_ETCR08,param,0x01); SetValue(IN_BDATA02,param,1); add_size=0x80000 ; min_vol = 500; fprintf(file_log,"4kB  ");  break;
      // 4kB
      case 4: SetValue(IN_ETCR08,param,0x03); SetValue(IN_BDATA02,param,3); add_size=0x80000 ; min_vol = 300; fprintf(file_log,"16kB ");  break;
      // 6kB
      case 6: SetValue(IN_ETCR08,param,0x05); SetValue(IN_BDATA02,param,5); add_size=0x80000 ; min_vol =   0; fprintf(file_log,"64kB ");  break;
      // 64kB
      case 8: SetValue(IN_ETCR08,param,0x07); SetValue(IN_BDATA02,param,7); add_size=0x80000 ; min_vol =   0; fprintf(file_log,"256kB");  break;
      // 256kB
      default: break;
    }
...
// **   Initialization                                                               **
// ************************************************************************************
	// 1=ALL Pattern printf( "### Distribution ###" ); printf("bit_count = %d, vth_mode = %d, min_vol = %d",bit_count,vth_mode,min_vol); result = VthReadDist(vth_mode, bit_count, min_vol, 100, &vs[0],W1SEC, param, file_name2); SetValue(IN_EXTAL1,param,stack10);  SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  } }     if ((cond0 == 5) | (cond0 == 45)) {  SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,0); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0);   if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  set_vcc = 3300; fprintf(file_log,"<<Mulch-Erase>> Vcc=%d,Distribution:0V~0.1VStep,Mat=%d(0x0~0x7FFFF)",set_vcc,mat);  fprintf(file_log,""); fclose( file_log );  for ( i=0 ; i<8 ; i++ ) {   if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x100); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x100); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x100); }  SetValue(IN_ETCR08,param,0x20); SetValue(IN_TESTSEL,param,0); vs[0] = 3300; CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("programall_code_at"),"Program"); SetValue(IN_ETCR08,param,0x00);  if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  switch(i) { case 0: SetValue(IN_ETCR08,param,0x00); SetValue(IN_BDATA02,param,0); add_size=0x80000 ; fprintf(file_log,"4kB  ");  break; case 2: SetValue(IN_ETCR08,param,0x03); SetValue(IN_BDATA02,param,3); add_size=0x80000 ; fprintf(file_log,"16kB ");  break; case 4: SetValue(IN_ETCR08,param,0x05); SetValue(IN_BDATA02,param,5); add_size=0x80000 ; fprintf(file_log,"64kB ");  break; case 6: SetValue(IN_ETCR08,param,0x07); SetValue(IN_BDATA02,param,7); add_size=0x80000 ; fprintf(file_log,"256kB");  break; default: break; }  fclose( file_log );  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,add_size); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,add_size); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,add_size); }  SetValue(IN_TESTSEL,param,0); vs[0] = 3300; CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Erase ALL"); SetValue(IN_ETCR06,param,0x00); SetValue(IN_ETCR08,param,0x00); SetValue(IN_BDATA02,param,0x00);   vs[0] = 3300;  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x80000); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x80000); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x80000); }  SetValue(IN_BDATA01,param,0); // ALL Count SetValue(IN_BDATA02,param,99); // Log Headder SetValue(IN_BDATA04,param,1); // Log Output -> DATALOG SetValue(IN_BDATA05,param,1); // Vth-Level-Log:OFF vth_mode = 1; // VthRead2&3(0~2.5V) min_vol = -2000; stack10 = SetValue(IN_EXTAL1,param,VTH_FREQ);   //  min_vol = VthEdgeSearch(3,100,&vs[0],W1SEC, param);  // Loss Search bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern printf( "### Distribution ###" ); printf("bit_count = %d, vth_mode = %d, min_vol = %d",bit_count,vth_mode,min_vol); result = VthReadDist(vth_mode, bit_count, min_vol, 100, &vs[0],W1SEC, param, file_name2); SetValue(IN_EXTAL1,param,stack10);  SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  } }     if ((cond0 == 6) | (cond0 == 67)) {  SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,0); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0);   if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  set_vcc = 3300; fprintf(file_log,"<<Separate-Write>> Vcc=%d,Distribution:2.5V~0.1VStep,Mat=%d(0x0~0xFFF)",set_vcc,mat);  fprintf(file_log,""); fclose( file_log );  for ( i=0 ; i<5 ; i++ ) {   if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x100); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x100); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x100); }  SetValue(IN_ETCR08,param,0x20); SetValue(IN_TESTSEL,param,0); vs[0] = 3300; CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Erase"); SetValue(IN_ETCR08,param,0x00);  if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  switch(i) { case 0: loop=1 ; fprintf(file_log,"3ms_x1 ");   break; case 1: loop=3 ; fprintf(file_log,"1ms_x3 ");   break; case 2: loop=2 ; fprintf(file_log,"1ms+2ms");   break; case 3: loop=2 ; fprintf(file_log,"2ms+1ms");   break; case 4: loop=1 ; fprintf(file_log,"3ms_x1 ");   break; default: break; }  fclose( file_log );  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x1000); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x1000); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x1000); }  SetValue(IN_TESTSEL,param,0); vs[0] = 3300;  for ( j=0 ; j<loop ; j++ ) {  if ( (i==0) || (i==4) ) { SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0xF); } if ( i==1 ) { SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0x4); } if ( i==2 ) { if ( j==0 ) SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0x4); if ( j==1 ) SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0x6); } if ( i==3 ) { if ( j==0 ) SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0x6); if ( j==1 ) SetValue(IN_ETLR36,param,(etlr_36&0xF0)|0x4); }  CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("programall_code_at"),"Program ALL"); } SetValue(IN_ETLR36,param,etlr_36);   vs[0] = 3300;  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x1000); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x1000); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x1000); }  SetValue(IN_BDATA01,param,0); // ALL Count SetValue(IN_BDATA02,param,99); // Log Headder SetValue(IN_BDATA04,param,1); // Log Output -> DATALOG SetValue(IN_BDATA05,param,1); // Vth-Level-Log:OFF vth_mode = 23; // VthRead2&3(0~2.5V) min_vol = 500; stack10 = SetValue(IN_EXTAL1,param,VTH_FREQ);   //  min_vol = VthEdgeSearch(3,100,&vs[0],W1SEC, param);  // Loss Search bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern printf( "### Distribution ###" ); printf("bit_count = %d, vth_mode = %d, min_vol = %d",bit_count,vth_mode,min_vol); result = VthReadDist(vth_mode, bit_count, min_vol, 100, &vs[0],W1SEC, param, file_name2); SetValue(IN_EXTAL1,param,stack10);  SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  } }     if ((cond0 == 7) | (cond0 == 67)) {  SetValue(IN_REG,param,0x03); SetValue(IN_AREA,param,0); SetValue(IN_TOPADDR0 ,param,0); SetValue(IN_TESTSIZE0,param,0); SetValue(IN_TOPADDR1 ,param,0); SetValue(IN_TESTSIZE1,param,0); SetValue(IN_TOPADDR2 ,param,0); SetValue(IN_TESTSIZE2,param,0);   if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  set_vcc = 3300; fprintf(file_log,"<<Separate-Erase>> Vcc=%d,Distribution:0V~0.1VStep,Mat=%d(0x0~0xFFF)",set_vcc,mat);  fprintf(file_log,""); fclose( file_log );  for ( i=0 ; i<4 ; i++ ) {   if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x100); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x100); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x100); }  SetValue(IN_ETCR08,param,0x20); SetValue(IN_TESTSEL,param,0); vs[0] = 3300; CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("programall_code_at"),"Program"); SetValue(IN_ETCR08,param,0x00);  if( NULL==(file_log = fopen( file_name, "at" )) ){ printf( "OpenDataOutFile Fptdata Open Err[%s]", file_name ); }  switch(i) { case 0: loop=1 ; fprintf(file_log,"5ms_x1 ");   break; case 1: loop=5 ; fprintf(file_log,"1ms_x5 ");   break; case 2: loop=2 ; fprintf(file_log,"1ms+4ms");   break; case 3: loop=2 ; fprintf(file_log,"4ms+1ms");   break; default: break; }  fclose( file_log );  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x1000); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x1000); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x1000); }  SetValue(IN_TESTSEL,param,0); vs[0] = 3300;  for ( j=0 ; j<loop ; j++ ) {  if ( i==0 ) { SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0xF); } if ( i==1 ) { SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0x3); } if ( i==2 ) { if ( j==0 ) SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0x3); if ( j==1 ) SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0x6); } if ( i==3 ) { if ( j==0 ) SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0x6); if ( j==1 ) SetValue(IN_ETLR37,param,(etlr_37&0xF0)|0x3); }  if( j==0 ) SetValue(IN_BDATA00,param,0); else       SetValue(IN_BDATA00,param,1); CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("erase_code_at"),"Erase ALL"); } SetValue(IN_BDATA00,param,0x00); SetValue(IN_ETLR37,param,etlr_37);   vs[0] = 3300;  if ( mat==0 ) { SetValue(IN_TOPADDR0 ,param,FLP0_TOP); SetValue(IN_TESTSIZE0,param,0x1000); } else if ( mat==1 ) { SetValue(IN_TOPADDR1 ,param,FLI1_TOP); SetValue(IN_TESTSIZE1,param,0x1000); } else if ( mat==2 ) { SetValue(IN_TOPADDR2 ,param,FLI2_TOP); SetValue(IN_TESTSIZE2,param,0x1000); }  SetValue(IN_BDATA01,param,0); // ALL Count SetValue(IN_BDATA02,param,99); // Log Headder SetValue(IN_BDATA04,param,1); // Log Output -> DATALOG SetValue(IN_BDATA05,param,1); // Vth-Level-Log:OFF vth_mode = 1; // VthRead2&3(0~2.5V) min_vol = -2000; stack10 = SetValue(IN_EXTAL1,param,VTH_FREQ);   //  min_vol = VthEdgeSearch(3,100,&vs[0],W1SEC, param);  // Loss Search bit_count = (GetValue(IN_TESTSIZE0,param)+GetValue(IN_TESTSIZE1,param)+GetValue(IN_TESTSIZE2,param)) * 8 * 1; //*1=ALL Pattern printf( "### Distribution ###" ); printf("bit_count = %d, vth_mode = %d, min_vol = %d",bit_count,vth_mode,min_vol); result = VthReadDist(vth_mode, bit_count, min_vol, 100, &vs[0],W1SEC, param, file_name2); SetValue(IN_EXTAL1,param,stack10);  SetValue(IN_BDATA01,param,0); SetValue(IN_BDATA02,param,0); SetValue(IN_BDATA04,param,0); SetValue(IN_BDATA05,param,0);  } }     printf(""); printf("**************************"); printf("*** TEST END           ***"); printf("*** TEST END           ***"); printf("*** TEST END           ***"); printf("**************************");  time( &time_list2 ); // time_list += V4V5K_TIME_OFS; printf("Test-Time:%d[s]",time_list2-time_list); printf("");   SetValue(IN_AREA,param,stack0); SetValue(IN_TOPADDR0 ,param,stack1); SetValue(IN_TESTSIZE0,param,stack2); SetValue(IN_TOPADDR1 ,param,stack3); SetValue(IN_TESTSIZE1,param,stack4); SetValue(IN_TOPADDR2 ,param,stack5); SetValue(IN_TESTSIZE2,param,stack6); SetValue(IN_EXTAL1,param,stack7); SetValue(IN_REG,param,stack8); vs[0] = stack9;   }   int WriteErase_Tokusei_Kani(int vs[NUM_POWERSUPPLY],int waittime,interface_t* param) { int tempo,tempo0,tempo1,tempo2,tempo3,tempo4,tempo5,tempo6; int stack0,stack1,stack2,stack3,stack4,stack5,stack6,stack7,stack8,stack9,stack10; int result,selvol,pulse; int i,j,k,l,m,n,cond0,cond1,cond2,cond3,cond4,mat,loop; int judg,target_vhh,pulse_reg,set_vcc; int etlr_16,etlr_17,etlr_25,etlr_36,etlr_37; int info[32]; int atreg[3]; //After Trimming Reg int result1[5][5],result2[5][5],result3[5][5]; //middle suso search result[target_vhh][pulse_reg] char file_name[128]; char sample_name[128]; char test_temp[128]; FILE_T *file_log;  cond1 = cond2 = cond3 = cond4 = 0; result = 0;  printf("Enter X-location->");DigitInput(&tempo0); printf("Enter Y-location->");DigitInput(&tempo1); switch(get_site_number()){ case 0: xy_location[0] = tempo0; xy_location[1] = tempo1; break; case 1: xy_location[0] = tempo0+1; xy_location[1] = tempo1; break; case 2: xy_location[0] = tempo0; xy_location[1] = tempo1+1; break; case 3: xy_location[0] = tempo0+1; xy_location[1] = tempo1+1; break; }     stack0 = GetValue(IN_AREA,param); stack1 = GetValue(IN_TOPADDR0,param); stack2 = GetValue(IN_TESTSIZE0,param); stack3 = GetValue(IN_TOPADDR1,param); stack4 = GetValue(IN_TESTSIZE1,param); stack5 = GetValue(IN_TOPADDR2,param); stack6 = GetValue(IN_TESTSIZE2,param); stack7 = SetValue(IN_EXTAL1,param,31); stack8 = GetValue(IN_REG,param); stack9 = vs[0]; etlr_16 = GetValue(IN_ETLR16,param); etlr_17 = GetValue(IN_ETLR17,param); etlr_25 = GetValue(IN_ETLR25,param); etlr_36 = GetValue(IN_ETLR36,param); etlr_37 = GetValue(IN_ETLR37,param);     if ( PASS == OpensShorts() ) { DeviceSpecificPowerUp(); /* SetValue(IN_REG,param,CHIPDATA); SetValue(IN_TESTSEL,param,0x00); if ( PASS == CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC,param,CreatePatFileName("devicefunction_at"),"DEBUG PASS") ) { SetValue(IN_ETLR00,param,GetValue(OUT_ETLR00,param)); SetValue(IN_ETLR01,param,GetValue(OUT_ETLR01,param)); SetValue(IN_ETLR02,param,GetValue(OUT_ETLR02,param)); SetValue(IN_ETLR03,param,GetValue(OUT_ETLR03,param)); SetValue(IN_ETLR04,param,GetValue(OUT_ETLR04,param)); SetValue(IN_ETLR05,param,GetValue(OUT_ETLR05,param)); SetValue(IN_ETLR06,param,GetValue(OUT_ETLR06,param)); SetValue(IN_ETLR07,param,GetValue(OUT_ETLR07,param)); SetValue(IN_ETLR08,param,GetValue(OUT_ETLR08,param)); SetValue(IN_ETLR09,param,GetValue(OUT_ETLR09,param)); SetValue(IN_ETLR10,param,GetValue(OUT_ETLR10,param)); SetValue(IN_ETLR11,param,GetValue(OUT_ETLR11,param)); SetValue(IN_ETLR12,param,GetValue(OUT_ETLR12,param)); SetValue(IN_ETLR13,param,GetValue(OUT_ETLR13,param)); SetValue(IN_ETLR14,param,GetValue(OUT_ETLR14,param)); SetValue(IN_ETLR15,param,GetValue(OUT_ETLR15,param)); SetValue(IN_ETLR16,param,GetValue(OUT_ETLR16,param)); SetValue(IN_ETLR17,param,GetValue(OUT_ETLR17,param)); SetValue(IN_ETLR18,param,GetValue(OUT_ETLR18,param)); SetValue(IN_ETLR19,param,GetValue(OUT_ETLR19,param)); SetValue(IN_ETLR20,param,GetValue(OUT_ETLR20,param)); SetValue(IN_ETLR21,param,GetValue(OUT_ETLR21,param)); SetValue(IN_ETLR22,param,GetValue(OUT_ETLR22,param)); SetValue(IN_ETLR23,param,GetValue(OUT_ETLR23,param)); SetValue(IN_ETLR24,param,GetValue(OUT_ETLR24,param)); SetValue(IN_ETLR25,param,GetValue(OUT_ETLR25,param)); SetValue(IN_ETLR26,param,GetValue(OUT_ETLR26,param)); SetValue(IN_ETLR27,param,GetValue(OUT_ETLR27,param)); SetValue(IN_ETLR28,param,GetValue(OUT_ETLR28,param)); SetValue(IN_ETLR29,param,GetValue(OUT_ETLR29,param)); SetValue(IN_ETLR30,param,GetValue(OUT_ETLR30,param)); SetValue(IN_ETLR31,param,GetValue(OUT_ETLR31,param)); SetValue(IN_ETLR32,param,GetValue(OUT_ETLR32,param)); SetValue(IN_ETLR33,param,GetValue(OUT_ETLR33,param)); SetValue(IN_ETLR34,param,GetValue(OUT_ETLR34,param)); SetValue(IN_ETLR35,param,GetValue(OUT_ETLR35,param)); SetValue(IN_ETLR36,param,GetValue(OUT_ETLR36,param)); SetValue(IN_ETLR37,param,GetValue(OUT_ETLR37,param)); SetValue(IN_ETLR38,param,GetValue(OUT_ETLR38,param)); SetValue(IN_ETLR39,param,GetValue(OUT_ETLR39,param)); printf("************************************"); printf("***   INIT PASS                  ***"); printf("************************************"); printf(""); } else { printf("------------------------------------"); printf("---   INIT FAIL                  ---"); printf("------------------------------------"); printf(""); return(PASS); } } else { printf("------------------------------------"); printf("---   PIN FAIL                  ---"); printf("------------------------------------"); printf(""); return(PASS);
      }
...
// ************************************************************************************
  ...
  cond1 = 1;
  // Iref 2.5uA Only
  if ( cond1 == 1 ) SetValue(IN_ETLR25,param,0xFD);
  // 2.5uA
  if ( cond1 == 2 ) SetValue(IN_ETLR25,param,0xFF);
  // 3.5uA
...
// ************************************************************************************
  ...
  //Other -40C
    for(m=0; m<3; m++){
      ...
      atreg[m] = GetValue(IN_ETLR17,param);
      //Get VhhW(afterTrim)
            // debug
            //printf("atreg[%d]=%X",target_vhh,atreg[target_vhh]);
            //WaitHitKey( "\nPlease Hit Any Key\n" );
            // debug
      }
    }
  ...
  SetValue(IN_ETLR17,param,atreg[i]);
    ...
    // ** Pulse Loop **
      switch (j) {
        case 0: pulse_reg = 0x4; break;
        // 1ms
        case 1: pulse_reg = 0xF; break;
        // 3ms
        case 2: pulse_reg = 0xA; break;
        // 5ms
        case 3: pulse_reg = 0xC; break;
        // 7ms
        case 4: pulse_reg = 0xE; break;
        // 10ms
        default: break;
      }
    if(pulse_reg != 0xE){
...
// ************************************************************************************
  ...
  // ** Voltage Loop **
    ...
    // ** Pulse Loop **
      switch (j) {
        case 0: pulse_reg = 0x3; break;
        // 1ms
        case 1: pulse_reg = 0x5; break;
        // 3ms
        case 2: pulse_reg = 0xF; break;
        // 5ms
        case 3: pulse_reg = 0xA; break;
        // 7ms
        case 4: pulse_reg = 0xC; break;
        // 10ms
        default: break;
      }
    if(pulse_reg != 0x3){
      ...
      vs[0] = 3300;
      stack10 = SetValue(IN_EXTAL1,param,VTH_FREQ);
      result1[i][j] = VthEdgeSearch(2,20,&vs[0],W1SEC*waittime, param);
      //1st suso search
      result2[i][j] = VthEdgeSearch(2,20,&vs[0],W1SEC*waittime, param);
      //2nd suso search
      result3[i][j] = VthEdgeSearch(2,20,&vs[0],W1SEC*waittime, param);
      //3rd suso search
      SetValue(IN_EXTAL1,param,stack10);
      result = (result1[i][j]+result2[i][j]+result3[i][j])/3;
      //average suso search
      printf(" ----result---- %dmV",result);
  ...
  for(n=6500; n<=7100; n+=300){
    ...
    for(l=0; l<4; l++){
      switch(l){
        case 0: pulse_reg = 0x5; pulse=3; break;
        // 3ms
        case 1: pulse_reg = 0xF; pulse=5; break;
        // 5ms
        case 2: pulse_reg = 0xA; pulse=7; break;
        // 7ms
        case 3: pulse_reg = 0xC; pulse=10; break;
        // 10ms
        default: break;
        }
...
//      fprintf(file_log,",");//dummy space //      } }    // j f
...
// ************************************************************************************
  ...
  printf("*** Test Temp   [25C:25 / -40:40 / 95C:95 ] ->");
  //DigitInput(&cond2);
  cond2 = temprature;
