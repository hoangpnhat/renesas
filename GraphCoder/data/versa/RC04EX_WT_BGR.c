...
int BGR_WT(interface_t* param , int wt_num)
{
  ...
  if(result == PASS){
    ...
    stack1 = SetValue(IN_AREA,param,5);
    // Extra5
  ...
  if((Fptdata=fopen(buff,"at")) == NULL) {
    ...
    // Initial Set
  fprintf(Fptdata,"%d,%d,%s,%-24.24s,%-24.24s,",xy_location[0],xy_location[1],status_name,time_buf,time_buf2);
...
int BGR_WT_HT(interface_t* param)
{
  ...
  if(result == PASS){
    ...
    result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*2,param,CreatePatFileName("settrimingdata_code_at"),"Write Triming REGISTER");
    // t@[ŕύX\
...
int WT_Def(interface_t* param)
{
  ...
  // vȂH if(result == PASS){ sprintf(test_name,"PassCode"); if ( info[19] != 0x40){ sprintf(status_name,"%s_FAIL",test_name); printf("----------- %s FAIL -----------",test_name); result = FAIL; }else{ printf("*********** %s PASS ***********",test_name); } }
   // *********************************** Main Test * ***********************************
  DeviceSpecificPowerUp();
