...
int BGR_QT_NoSet(interface_t* param , int sample_no)
{
  ...
  SetValue(IN_REG,param,0);//Use Chip Data
  ...
  if(result == PASS){
    ...
    //--- Register Setting ---//
      SetValue(IN_REG,param,3);//Use Versa Data
      stack0 = SetValue(IN_TESTSEL,param,0);
      stack1 = SetValue(IN_BDATA00,param,1);
      stack2 = SetValue(IN_BDATA01,param,0);
      result = CpuModeFunc(MNORMAL,MSNORMAL,&vs[0],W1SEC*waittime,param,CreatePatFileName("settrimingdata_code_at"),"Write Triming REGISTER"); /*ファーム次第で変更する可能性あり*/
  ...
  // Initial Set
  fprintf(Fptdata,"\n%s,%d,%d,%d,%d,%s,%-24.24s,%-24.24s,",lotname,atoi(wafername),xy_location[0],xy_location[1],sample_no_read,status_name,time_buf,time_buf2);
