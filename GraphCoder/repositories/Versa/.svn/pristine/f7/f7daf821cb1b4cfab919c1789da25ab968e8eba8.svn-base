...
/**************************************************************************************/
  ...
  mat = 0;//Io‚ÍFLI1/2‚Í–³‚µ
  printf("*** Test Data-Area [Loss&Gain:0 / Loss:1 / Gain:2] ->"); DigitInput(&area);

  /*
  printf("*** Vth-Read_I(ETLR25)  [Chip-Data:0 / 2.5uA:1 / 3.5uA:2 ] ->"); DigitInput(&cond0);
  if ( cond0 == 1 ) SetValue(IN_ETLR25,param,0xFD);   // 2.5uA //
  if ( cond0 == 2 ) SetValue(IN_ETLR25,param,0xFF);   // 3.5uA //
  */
  SetValue(IN_ETLR25,param,0xFA);//Iref = 2.0uA
  ...
  if ( cond3 == 0 ) {
    ...
    SetValue(IN_ETLR15,param,(GetValue(IN_ETLR15,param) & 0x3F) | 0xC0);   /* Temp_ON  */
  } 
  if ( cond3 == 1 ) {
    SetValue(IN_ETLR15,param,0x3F);   /* Temp_OFF */
  } 

/*
  printf("*** Delay fixed Trimming   [WS1:0 / WS2:1 ] ->"); DigitInput(&cond4);
	if ( cond4 == 0 ){
...
/**************************************************************************************/
  ...
  printf("*** Test MAT       [FLP0:0 / FLI1:1 / FLI2:2 / FLP0&FLI1:3 / FLP0&FLI1&FLI2:4] ->");
  //DigitInput(&mat);
  mat = 0;
...
/*
    ...
    SetValue(IN_ETLR18,param,(GetValue(IN_ETLR18,param) & 0xC7) | 0x30); // Vnoemi_R => 3.4V
    SetValue(IN_ETLR20,param,(GetValue(IN_ETLR20,param) & 0xF0) | 0x09); // Vpp_w => -3.0V
...
///// 8nsDelay 10nsDelay Const regset ///
    SetValue(IN_ETLR26,param,0x33);  // T1 Precharge DelayTime (min+4Tap) //
    SetValue(IN_ETLR27,param,0xF3);  // T1 Precharge DelayTime (min+4Tap) //
    SetValue(IN_ETLR28,param,0xFB); // T2 Discharge DelayTime (min+1Tap) //
    SetValue(IN_ETLR29,param,0xFB); // T2 Discharge DelayTime (min+1Tap) //
    SetValue(IN_ETLR30,param,0xFB); // T2 Discharge DelayTime (min+1Tap) //
    SetValue(IN_ETLR32,param,0x3E);   // T5 saenb2 (min+3Tap) , T3 saenb DelayTime (min+1Tap) //
    SetValue(IN_ETLR33,param,0x11);   // T4 saout DelayTime (min+1Tap) //
    SetValue(IN_ETLR34,param,0x11);   // T6 T4-T5 DelayTime (min+1Tap) //
    printf("*** WS2 DelayTrim Condition is evaluating now. ");
    printf("*** WS2  = WS1 DelayTrim setting!!! ");
...
//    SetValue(IN_ETLR15,param,0x3F);   // Temp_OFF
  ...
  printf("*** cond4=%d ***\n",cond4);
	if ( cond4 == 0 ){
    SetValue(IN_ETLR32,param,0x3E);  // T5 saenb2 (min+3Tap) , T3 saenb DelayTime (max-1Tap) //
    SetValue(IN_ETLR33,param,0x11);  // T4 saout DelayTime (min+1Tap) //
    SetValue(IN_ETLR34,param,0x11);  // T6 T4-T5 DelayTime (min+1Tap) //
	}
	if ( cond4 == 1 ){
  ...
  */
  ...
  printf("*** trtf Setting 100us -> 300us  [trtf=100us:0 / trtf=300us:1 ] ->"); //DigitInput(&cond5);
  cond5=1;//trtf=300us
  printf("*** cond5=%d ***\n",cond5);
	if ( cond5 == 0 ){
	...
	if ( cond5 == 1 ){
    SetValue(IN_ETLR15,param,0xFB);// trtf=300us //
    printf("*** etlr15<7:0>=0x%X (trtf setting)\n",GetValue(IN_ETLR15,param));
	}
...
/**************************************************************************************/
  if (cond2 == 0) {
    SetValue(IN_REG,param,0x03);
    tempo = SetValue(IN_ETLR25,param,0xFA);//Auto Trimming‚ÍIref2.0uA
    SetValue(IN_TESTSEL,param,AUTOTRIM_IMONI_FLASH_NOTEMP);
      SetValue(IN_BDATA15,param,UA25);
    CpuModeMonitor(VSSMON_APP,&vs[0],param,W1SEC*waittime,CreatePatFileName("trimingcurrent_at"),"AUTOTRIM_IMONI_FLASH_NOTEMP",&trimvalue[I_IMONI_FLASH_NOTEMP]);
  ...
  SetValue(IN_ETLR22,param,0xFB);//Pumpup(8MHz->16MHz) vppe_clk
  printf("ETLR22;%02X\n",GetValue(IN_ETLR22,param));
...
/**************************************************************************************/
  if ((area == 0) || (area == 1)) {
  //fprintf(file_log,"<<Loss>> ETLR25=0x%X,",GetValue(IN_ETLR25,param)); //0xFD=2.5uA, 0xFA=2.0uA
  SetValue(IN_REG,param,0x03);
  for ( i=0 ; i<8 ; i++ ) {
      SetValue(IN_AREA,param,0);
      SetValue(IN_TOPADDR0 ,param,0);
...
/**************************************************************************************/
  ...
  for ( i=0 ; i<8 ; i++ ) {
    ...
    /* Vth-Search ************************************************/
    vs[0] = 3300;
    result = VthEdgeSearch(2,20,&vs[0],W1SEC*waittime, param); //20mV step
    printf(" ----result---- %dmV\n",result);
  	if(i==0) {
  		result1st = result;
    printf(" ====result 1st = %dmV\n",result1st);
    ...
    if      ((result-result1st-deltacond1) >= (  2000)) judg =999;
    ...
    else if ((result-result1st-deltacond1) <= ( - 35)) judg = -1; //vhhe50mVstep/1tap 50mVx0.7=vth35mV/1tap -50mV<Target<0
  	...
  	printf(" ==== Calc (result-result1st) = %dmV\n",tempo7);
  	printf(" ==== Setting deltacond1      = %dmV\n",deltacond1);
