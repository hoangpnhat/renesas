/*
    FILENAME  fshell.c
    CONTENTS:   standard main program shell for test
            programs.  This module handles the system
            interface functions for the user test program.
            The user test program must provide the following
            public funtions and variables for use by this module:
    int DoTestProgram(void);        -- does test flow
    int DoProberAlignment(void);        -- does parametrics tests
    int DeviceSpecificInit(void);       -- called during init
    void DeviceSpecificPowerDown(void); -- called during PowerDown
    int DoSummary(void);            -- print and clear (begin)
    int DoPrintSummary(void);       -- print only (summary)
    int UserExCommand(int argc, char *argv[]);
    int REJ_BIN_LIST[];         -- list of pass bins.
    bin, rej, bin, rej, -1
rev 2.3
    int  val_progrev;           -- program revision or -1.
    char *val_devtype           -- NULL or device type.
    char *program_name;         -- test program name.
    REVISION: 1.0 2/19/92 dja Versatest.
        - added slave mode support.
        1.1 3/2/92 aka, dja Versatest.
        - modified PowerDown.
        2.0 9/23/92 aka, dja Versatest.
        - changed PASS_BIN_LIST to REJ_BIN_LIST for
          compatability with future handlers where
          reject bin must be specified.
        - fixed bugs with debug variable.  == not =, and 0x80 not 80.
        - added debug test to initialize(0) in "init" section.
        - corrected semaphore use to semaphore 3. 4 d/n exist.
        - got rid of all if (xxx != FAIL) syntax. (!xxx).
        - added DIAGNOSTICS [LOT | WAFER | num] switch to
          option file so option file controls diag execution.
    2.1   1/20/93
        - Corrected slave mode / disable_unused_sites incompatability.
          System must not ever execute disable_slave_mode() command, nor
          run diagnostics in slave mode.
    2.2   3/22/93
        - Added user initialize functions after calls to diagnostics.
        - Added code to ignor "0", "1" values for X, Y ASR file names.
        - Added check for device type, revision.
    2.3 7/1/93
        - Added EX command support from Paul, Srini.
        - Cleaned up val_progrev problems.
    2.4 7/7/93
        - Modified the PowerDown routine to send reject bin instead of
          0, or 1.
    2.5 8/25/93
        - Modified the PowerDown routine to allow both pass and fail
          bins to be specified in REJ_BIN_LIST without invalid operation
          when forced flow is enabled.  Reject bins specified in
          REJ_BIN_LIST will be treated as failure bins if any bit greater
          than 0xff is set.
        - Included shvar ex command to display the current status of the
          wafseq variables to the terminal.
        - Added check for argc >= 2 to all modify wafseq ex commands.
    2.6 9/9/93
        - modified "begin" to execute diags, calibration before
          host_begin() is executed.  This will insure that the next
          wafer is not loaded before diagnostics complete.
    COPYRIGHT (c) 1990, 92, 93 VERSATEST INC. ALL RIGHTS RESERVED.
*/
/* First thing in all programs should be dummy variable to avoid
   NULL pointers
*/
...
/****************** added hitachi maicon ltd 1994/12/27 **************
 * extern int   fof;
 * extern int   relief;
...
/* Public variables */
void PowerDown(int bin);    /* Power down routine */
int ovrhead;            /* timer variables */
...
static  int diag_mode;  /* 0 = never, 1=lot, 2=wafer, 3=count */
static  int diag_count; /* count for mode 3 */
static  int diag_wafer; /* current count */
/*  PowerDown()
    Called at end of test to bin out device.  If bin is in REJ_BIN_LIST
    Then the DeviceSpecificPowerDown is called and the device is
    binned out as a pass.  If the bin is not a pass bin then the
    forced_flow option will be tested. If forced_flow is not set, then
    the device will be binned out as a fail.
    bin -1 is used to cause a power down without stoping the test.
*/
...
int device_failed;
    ...
    if ( forced_flow && device_failed ) {
        ...
        printf("\nTest Failed, bin %d, elapsed time: %d.%dms\n",
                bin,tmptime/1000,tmptime%1000);
    ...
    if ( ! device_failed ) {
        if (dlog || var1)
            printf("\n** PASS **, Bin=%d, Testtime: %d.%03dsec\n",
                bin,curtime3,curtime2);
        end_test(bin, device_rej);
    } else {
        if (dlog || var1)
              printf("\n-- FAIL --, Bin=%d, Testtime: %d.%03dsec\n",
                bin, curtime3,curtime2);
        end_test(bin, device_rej);
    }
...
/*  LoadHardwareFiles()
    This routine loads the pattern generator and buffer memory
    files as requested in the option file.
    returns:    0   files loaded ok.
                -1  file load error.
*/
...
int expect_sum;
    ...
    {
        printf("loading Crossover configuration file: %s\n", optbuf);
    ...
    if ( (!read_option("XASR", optbuf)) && optbuf[0] )
    {
        if ( (sscanf(optbuf, "%i", &loadsum) != 1) ||
             ((loadsum != 0) && (loadsum != 1)) )
        {
            printf("loading X address scramble file: %s\n", optbuf);
            if (load_xasr(optbuf))
            {
        ...
        printf("No X ASR file found in option file\n", optbuf);
    }
    /* check for YASR file */
    if ( (!read_option("YASR", optbuf)) && optbuf[0] )
    {
        if ( (sscanf(optbuf, "%i", &loadsum) != 1) ||
             ((loadsum != 0) && (loadsum != 1)) )
        {
            printf("loading Y address scramble file: %s\n", optbuf);
            if (load_yasr(optbuf))
            {
    ...
    if ( ( !read_option("PATTERN", optbuf) ) && optbuf[0] )
    {
        if ( (sscanf(optbuf, "%i", &loadsum) != 1) ||
             ((loadsum != 0) && (loadsum != 1)) )
        {
            printf("Loading buffer memory pattern: %s\n", optbuf);
            ...
            if (!read_option("PATSUM", optbuf))
            {
                ...
                {
                    if (loadsum == FAIL)
                        printf("\nFatal error loading pattern file\n");
                    else
                    {
                        printf("\nPattern checksum does not match expected value\n");
                        printf("expected: %X, actual: %X\n", expect_sum, loadsum);
                    }
                    return(FAIL);
                }
            } else
                printf("\n!!! Warning: No checksum specified for pattern file\n");
        }
    } else {
    ...
    {
        printf("loading APG vector pattern file: %s\n", optbuf);
        if (load_apg(optbuf))
        {
...
/* DoInitialSetup
    This function implements the "init" flow from the main test
    program.  The operatinig system will execute this flow when the
    test program is first loaded or the option file is changed.
    This routine should be used to re-initialize the system and
    place it in a known state ready to run the test program.
*/
...
int     i;
    ...
    test_site = get_site_number();     /* init site number */
    ...
    {
        err = 1;
        printf("\n!!! Warning:  Test program device type does not match Plan file.\n");
    }
    if ( ( val_progrev >= 0 ) && ( val_progrev != _READ_VERSION() ) )
    {
        err = 1;
        printf("\n!!! Warning: Test program revision does not match Plan file.\n");
    }
...
/************************************************************************
 *  Main test program entry point.    Put main at end of program    *
 *  so we don't have to forward reference all of the mode routines. *
 ************************************************************************/
...
int  i, cnt;
    ...
    curtime = bentime();
    /*****************************************************************
     *  Re-initialize test site hardware so that test site is    *
     *  in a known state at the start of each test.      *
     *  (skip if debug bit 7 is set)                 *
     *****************************************************************/
     if (!(debug & 0x80))
        initialize(0x10);

    /*****************************************************************
     *  Load program control variables from the 386 station  *
     *  control software.  This data will be available to the    *
     *  test program in the form of public variables.        *
     *****************************************************************/
    /* Load program control variables */
    test_mode = read_mode();
    /* DLOG, LYA, VAR1, VAR2 are set by the IO process depending
       upon the current test mode.  They can also be set from the
       terminal with the corresponding commands below.  These variables
       are intended to be used as bit masks with each bit enableing a
       different feature in the test program.  The terminal value and
       the mode dependant value are OR'ed togeather to generate the
       value used by the rest of the program.
    */
    ...
    /* These values are set when the plan file is loaded
       they can be changed from the terminal, but will remain
       changed until the plan file is re-loaded.
       These are short (16 bit) variables on the 386, but the
       V1000 only supports 32 bit integers so the data must be
       masked to 16 bits.
    */
     ...
     *****************************************************************/
    /* "test" is used to tell us to execute the
        test flow.  It is the normal test flow */
    if ( strnicmp(argv[0], "test", 4) == 0 )
    {
        DoTestProgram();
        printf("End of Program\n");
        exit(PASS); /* just in case */
    }
    /* "init" is used when the program is first loaded or
        the option file is changed.  It is executed rarely
        to initialize or change the program setup.
    */
    ...
    /* "begin" is executed when the user does a print and
        clear summary operation or the prober sends an
        end of wafer signal.  It is used for the test program
        to print summary information and reset any variables
        for the next wafer.
    */
    ...
    /* "print" is executed when the user does a print
       operation.  This should execute the same as the
       begin routine but should not clear or reset any
       variables.
    */
    ...
    /*  "funct0" is sent when the operator presses the
         function button on the front pannel.  This is
         intended for use as a prober alignment tool.
    */
    ...
    /*****************************************************************
     *  The following commands are used to modify the setup  *
     *  from the terminal.  They are not allowed if validate     *
     *  bit 0x8 is set in the current plan file.        *
     *****************************************************************/
    _read_wafseq(_VALIDATE_OFS, 0x2, (char *) &i);
    if ( ( (i & 0x8) == 0 ) && ( argc >= 2 ) )
    {
