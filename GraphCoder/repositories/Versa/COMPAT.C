 ...
 *  FILENAME COMPAT.c
 *
 *  CONTENTS: This file is used for compatibility for V3k to V5k
 *          test program.
 *
 *  COPYRIGHT 1997 HITACHI LTD. ALL RIGHTS RESERVED.
 *
 *  1998/08/19  rev.1.0  Created by Ken-K
 ******************************************************************************/
...
#else
  set_bmecr_8bit(); /* ECR/BM 8bit mode set */
#endif
}
...
#else
  set_bmecr_16bit(); /* ECR/BM 8bit mode set */
#endif
}
...
#else
  status = load_bm(filename);  /* LEGACY COMMAND */
#endif
  return (status);
}
...
#else
  status = load_ecr(filename);  /* LEGACY COMMAND */
#endif
  return (status);
}
...
int comp_set_iparh( int value ){
  int status;
#ifdef V4K5KTESTER
  status = set_iparh( value );  /* select 256K_APG */ /* V4000 */
#else
  if( slot_info[_PPS_SLOT].board_type==_PPS_PMU_BTYP ){
     status = set_ppiparh( value ); /* select 64K_APG */
  }else{
     status = set_iparh( value );  /* select 256K_APG */
  }
#endif
  return( status );
}
...
int comp_set_cvpar( int value ){
  int status;
#ifdef V4K5KTESTER
  status = set_cvpar( value );  /* select 256K_APG */
#else
  if( slot_info[_PPS_SLOT].board_type==_PPS_PMU_BTYP ){
    status = set_ppvpar( value ); /* select 64K_APG */
  }else{
    status = set_cvpar( value );  /* select 256K_APG */
  }
#endif
...
#ifdef V5KONLY
  status = PASS;
  SITE_MASK (1, status = set_pps_resource (PPS_VNN_V, value));  // caution!!
#else
  status = set_v5( value );  // caution!!
#endif
  return( status );
}
...
void comp_write_lbreg(int value){
  ...
  int i;
  /* set cs low */
  write_lbreg( 0xc );          // /RESET: High,  /CS: High
  delay_timer( 1 usec_delay );
  write_lbreg( 0x8 );          // /RESET: High,  /CS: Low
  inv_value = ~value;          // Need inverte value
...
void comp_system(char* input_str){
  ...
  temp_buf[255] = '\0';
  /*
  Replace dbm,fbm, and mbm command. Because BM feature in V5k was replaced ECR.
  if you want to look true bm value, please type tdbm or tfbm.
  */
...
#ifdef V4K5KTESTER
  timer += V4V5K_TIME_OFS; /* Add Second of 16Hour (From U.S.) */
#endif
  if( tp!=NULL ){
    *tp=timer;
  }
