...
function TipTapEditorInner(props: TipTapEditorProps) {
  ...
  // Hard-block input when the model that will actually be streamed is
  // rate-limited. When a multi-agent session lock is set, the LOCKED
  // model is the source of truth for the stream target — not
  // selectedChatModel, which can drift if MultiAgentModelSelect's sync
  // effect hasn't yet synced chat-role to the lock (race on session
  // restore). Match streamNormalInput.ts and the picker chip's lookup
  // so the input lock, the chip, and the actual stream target all agree
  // on which model is being checked.
  const sessionModelLock = useAppSelector(
    (s) => s.session.sessionModelLock,
  );
  const chatModelsList = useAppSelector(
  ...
  const blockedUntil = activeModel?.consumptionLimit?.blockedUntil ?? null;
  ...
  useEffect(() => {
    ...
    let placeholder = getPlaceholderText(props.placeholder, historyLength);
    if (isModelBlocked) {
      const countdown = formatBlockedCountdown(blockedUntil);
      placeholder = `Model rate-limited (${countdown}). Start a new session or wait for unlock.`;
    }
  ...
  }, [editor, props.placeholder, historyLength, isModelBlocked, blockedUntil]);

  // Toggle editor read-only based on lock state. setEditable schedules a
  // re-render of the editor view so the visual disabled state takes effect.
  // Also blur the editor if it's currently focused — TipTap doesn't drop
  // focus automatically when transitioning to non-editable, and a focused
  // contenteditable still accepts keystrokes even with editable=false in
  // some cases. Belt-and-suspenders: CSS pointer-events:none on the
  // wrapper handles the mouse path; this handles the keyboard path.
  useEffect(() => {
    if (!editor) {
      return;
    }
    editor.setEditable(!isModelBlocked);
    if (isModelBlocked) {
      editor.commands.blur();
    }
  }, [editor, isModelBlocked]);
    ...
    >
      <div className="px-2.5 pb-1 pt-2">
        ...
        <InputToolbar
          ...
          disabled={isStreaming || isModelBlocked}
        />
      </div>
