...
import { MultiAgentModelSelect } from "../modelSelection/MultiAgentModelSelect";
import { ModeSelect } from "../ModeSelect";
import { WorkflowPatternSelect } from "../ModeSelect/WorkflowPatternSelect";
...
function InputToolbar(props: InputToolbarProps) {
  ...
  // Phase 5.x — reasoning lightbulb removed from UI for v1.2.0 visual parity.
  // Body wire was reverted (commits f548cef34 + 7d8d0cc88) because the
  // reasoning_content streamed chunks crash refork's chunk decoder. Restore
  // both the UI element and the body wire together when chunk decoding lands.
    ...
    <>
      ...
      >
        <div className="xs:gap-1.5 flex flex-row items-center gap-1">
          ...
          {!isInEdit && mode === "multi-agent" && (
            <ToolTip place="top" content="Workflow Pattern">
              <HoverItem className="!p-0">
                <WorkflowPatternSelect />
              </HoverItem>
            </ToolTip>
          )}
          <ToolTip
            ...
            content={
              mode === "multi-agent"
                ? sessionModelLock
                  ...
                  : "Pick a model — locks for the session once chosen"
                : "Select Model"
            }
          >
            <HoverItem className="!p-0">
              {mode === "multi-agent" ? (
                <MultiAgentModelSelect />
              ) : (
                <ModelSelect />
              )}
            </HoverItem>
          ...
          <div className="xs:flex text-description -mb-1 hidden items-center transition-colors duration-200">
            ...
            {/* Phase 5.x — reasoning lightbulb removed (see header comment). */}
          </div>
        </div>
