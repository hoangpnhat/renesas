...
export function CliInstallBanner({
  ...
  // RICA: the Continue CLI is not part of the RICA VS Code product (separate,
  // unbranded npm package), so this banner is hidden entirely via the
  // `RICA_HIDE_CLI_BANNER` flag below. To re-enable, set it to false.
  // (Phase 5.5 branding — folded into the existing guard to avoid unreachable code.)
  const RICA_HIDE_CLI_BANNER = true;

  // Don't show if:
  // - RICA has hidden the banner
  // - Still loading CLI status
  // - CLI is already installed
  // - User has dismissed it
  // - Session threshold not met (if threshold is set)
  if (
    RICA_HIDE_CLI_BANNER ||
    cliInstalled === null ||
    cliInstalled === true ||
    dismissed ||
