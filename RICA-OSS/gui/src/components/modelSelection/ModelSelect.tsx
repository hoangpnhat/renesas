...
interface Option {
  ...
  // RICA: per-model consumption window so the chat/agent dropdown can show the
  // same "NN% blocked" badge + disable a rate-limited model as MultiAgentModelSelect.
  consumptionLimit?: {
    enabled?: boolean;
    blocked?: boolean;
...
function ModelOption({
  ...
  // RICA: rate-limit lock parity with MultiAgentModelSelect. When the backend
  // reports this model's consumption window as blocked, show a "NN% blocked
  // (countdown)" badge and prevent selection.
  const limit = option.consumptionLimit;
  const showUsage = limit?.enabled === true;
  const percent = showUsage ? getUsagePercent(limit as any) : 0;
  const isBlocked = showUsage && limit?.blocked === true;
  const optionDisabled = showMissingApiKeyMsg || isBlocked;
  ...
  return (
    <ListboxOption
      ...
      className={`group ${isSelected ? "bg-list-active text-list-active-foreground" : ""} ${isBlocked ? "opacity-50 cursor-not-allowed" : ""}`}
    >
      <div className="flex w-full items-center justify-between gap-5">
        <div className="flex items-center gap-2 py-0.5">
          ...
          <span className="line-clamp-1">
            ...
            {showUsage && (
              <span className={`ml-1.5 ${getUsageColor(percent, isBlocked)}`}>
                {percent}%
              </span>
            )}
            {isBlocked && (
              <span className="text-red-500 ml-1.5 text-[10px] italic">
                {formatBlockedCountdown(limit?.blockedUntil ?? null)}
              </span>
            )}
          </span>
        </div>
    ...
    setOptions(
      allModels.map((model) => {
        return {
          ...
          consumptionLimit: (model as any).consumptionLimit,
        };
      }),
    );
  }, [allModels]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (
        ...
        // RICA: skip blocked models when cycling with the keyboard shortcut,
        // mirroring the disabled state in the dropdown. Walk until we find a
        // selectable model or come full circle.
        const isOptionBlocked = (o: Option) =>
          o.consumptionLimit?.enabled === true &&
          o.consumptionLimit?.blocked === true;
        let nextIndex = (currentIndex + 1 * direction) % options.length;
        if (nextIndex < 0) nextIndex = options.length - 1;
        for (let i = 0; i < options.length; i++) {
          if (!isOptionBlocked(options[nextIndex])) break;
