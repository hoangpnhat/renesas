  ...
  const [reactContent, setMarkdownSource] = useRemark({
    ...
    rehypeReactOptions: {
      components: {
        ...
        p: ({ ...pProps }) => {
          // Multi-agent: detect ASCII tree-rendered output (agent graph drawings,
          // tree command, etc.) and preserve column alignment by wrapping in <pre>.
          // Without this the markdown renderer collapses whitespace and the tree
          // breaks. From v1.2.1 — see RICA Multi-Agent guided-tour rendering.
          const text =
            typeof pProps.children === "string" ? pProps.children : "";
          const hasTreeChars =
            /[├└│─┌┐┘┬┴┤┼]/.test(text) ||
            (/[|]/.test(text) && /[├└─]/.test(text));
          if (hasTreeChars) {
