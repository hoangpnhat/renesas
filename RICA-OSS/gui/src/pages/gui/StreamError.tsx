...
const StreamErrorDialog = ({ error }: StreamErrorProps) => {
  ...
  // JSON parse errors on tool-call args usually mean the model's response was
  // truncated mid tool-call — typically because the session is approaching the
  // model's context window. Surface a user-actionable hint instead of the raw
  // "Expected ',' or '}'" SyntaxError text. (RICA v1.2.1)
  const isJsonTruncation =
    !!message &&
    (message.includes("Expected ',' or '}'") ||
      message.includes("JSON at position") ||
      message.includes("after property value in JSON") ||
  ...
  if (isJsonTruncation) {
    errorContent = (
      <div className="flex flex-col gap-2">
        <span>
          The model's response was cut off mid tool-call, so RICA couldn't
          parse the arguments. This usually means the session is approaching
          its context limit.
        </span>
        <span>
          <strong>Try:</strong> start a new session, or compact this one
          (clear earlier turns) and re-send your last message.
        </span>
      </div>
    );
  }
