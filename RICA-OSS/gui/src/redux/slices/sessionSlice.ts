...
type SessionState = {
  ...
  // Multi-agent: live sub-agent status keyed by agentId for the GUI tree view.
  subAgentProgress: Record<string, SubAgentProgress>;
  // Multi-agent: model locked for the session. Set when user enters multi-agent
  // mode in this session; cleared on session reset (new chat).
  sessionModelLock: { modelTitle: string; lockedAt: number } | null;
};
...
export const sessionSlice = createSlice({
  ...
  reducers: {
      ...
      if (state.history.length) {
        for (const message of action.payload) {
          ...
          if (
            ...
            // Multi-agent: large tool outputs (e.g. sub-agent transcripts) can
            // overrun the chat surface. Cap the displayed copy to 10k chars
            // and append a truncation marker. The full content still flows
            // through the LLM via the underlying message.
            const MAX_TOOL_DISPLAY = 10_000;
            let initialContent = "";
            if (message.role === "tool" && messageContent) {
              initialContent =
                messageContent.length > MAX_TOOL_DISPLAY
                  ? messageContent.slice(0, MAX_TOOL_DISPLAY) +
                    `\n\n[... truncated for display: ${messageContent.length.toLocaleString()} total chars ...]`
                  : messageContent;
            }
            const historyItem: ChatHistoryItemWithMessageId = {
              message: {
                ...message,
                content: initialContent, // Tool message content set up-front (with truncation); non-tool starts empty for accumulation
                id: uuidv4(),
              },
              contextItems: [],
            };
            ...
            lastItem = state.history[state.history.length - 1];
            lastMessage = lastItem.message;
            // Tool messages are inserted complete — skip the accumulation block below.
          ...
          // (tool messages were inserted with their full content above and `continue`d past this block)
          if (messageContent) {
    ...
    newSession: (state, { payload }: PayloadAction<Session | undefined>) => {
      ...
      if (payload) {
        // Loading a saved session — any tool calls left in flight at save time
        // can never resume. Mark them canceled so the UI doesn't render them
        // as "still running".
        state.history = (payload.history as any[]).map((item: any) => {
        ...
        // Restore agent progress — mark any "running" agents as canceled
        // since they can't survive a session switch or extension restart.
        const restored = (payload as any).subAgentProgress ?? {};
        ...
        // Restore session model lock if present in saved payload.
        const restoredLock = (payload as any).sessionModelLock ?? null;
        state.sessionModelLock = restoredLock;
        // Restore mode so multi-agent sessions reload as multi-agent (and
        // the lock chip + Tier S picker render correctly without the user
        // having to manually switch back).
        if (payload.mode) {
          state.mode = payload.mode;
        } else if (restoredLock) {
          // Backwards-compat: sessions saved before mode persistence have
          // no `mode` field, but a session lock can only have been set in
          // multi-agent mode. Infer it so the lock chip renders.
    ...
    setSessionModelLock: (state, action: PayloadAction<string>) => {
      state.sessionModelLock = {
        modelTitle: action.payload,
        lockedAt: Date.now(),
      };
    },
    clearSessionModelLock: (state) => {
      state.sessionModelLock = null;
    },
    setWorkflowPattern: (state, action: PayloadAction<WorkflowPattern>) => {
      state.workflowPattern = action.payload;
    },
    updateSubAgentProgress: (
      state,
      action: PayloadAction<SubAgentProgress>,
    ) => {
      state.subAgentProgress[action.payload.subAgentId] = action.payload;
    },
