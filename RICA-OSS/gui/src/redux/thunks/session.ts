...
>(
  ...
  async ({ openNewSession, generateTitle }, { dispatch, extra, getState }) => {
    ...
    // Always write subAgentProgress + sessionModelLock explicitly, never via
    // spread-if-truthy. Spreading conditionally made absent JSON keys
    // ambiguous with null values, which masked save-time race bugs where a
    // key could be silently dropped (Bruce: lock disappears across past
    // sessions; agent history inconsistent — May 20).
    const updatedSession: Session = {
      ...
      subAgentProgress: session.subAgentProgress ?? {},
      sessionModelLock: session.sessionModelLock ?? undefined,
    };
