...
>(
  ...
  async ({ toolCallId, depth = 0 }, { dispatch, getState, extra }) => {
    await dispatch(
      streamThunkWrapper(async () => {
        ...
        // Multi-agent: log orchestrator tool results to the session log so
        // the export bundle can show the full tool flow alongside the
        // sub-agent activity. resultPreview is capped at 5000 chars — full
        // content is already in the chat history.
        if (state.session.mode === "multi-agent" && state.session.id) {
          extra.ideMessenger.post("multiagent/logToolResult", {
            sessionId: state.session.id,
            toolName: toolCallState.toolCall?.function?.name || "unknown",
            toolCallId: toolCallState.toolCallId,
            resultPreview: renderContextItems(toolOutput).slice(0, 5000),
          });
        }
