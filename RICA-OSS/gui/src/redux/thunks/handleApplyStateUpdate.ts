...
>("apply/editTool", async (payload, { dispatch, getState, extra }) => {
  ...
  // 5-min cap (vs no cap previously) — large files were timing out mid-apply
  // upstream-default IPC layers, and an indefinite hang here would leave the
  // tool call stuck in "calling" status forever. Acts as a ceiling, not a
  // fixed wait: small edits still return in ms. From RICA v1.2.1 May 20 fix
  // 1 (project-session-may20-pt2-triage-fixes / [[project-migration-master]]
  // verification scenario "editImpl 5-min cap").
  let didError = false;
  try {
    const response = await Promise.race([
      extra.ideMessenger.request("applyToFile", payload),
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("applyToFile request timed out after 5 min")),
          300_000,
        ),
      ),
    ]);
