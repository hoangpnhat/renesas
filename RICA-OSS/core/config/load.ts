...
function llmToSerializedModelDescription(llm: ILLM): ModelDescription {
  return {
    ...
    // RICA: forward the per-model consumption/multi-agent fields to the
    // browser config. Without them the GUI's rate-limit lock + Tier-S gate
    // never see `blocked: true` from the backend, so a blocked model stays
    // selectable.
    consumptionLimit: llm.consumptionLimit,
    multiAgentEligible: llm.multiAgentEligible,
  };
}
