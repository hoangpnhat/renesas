...
import { EXTENSION_NAME, getControlPlaneEnv } from "../control-plane/env.js";
...
export class ConfigHandler {
  ...
  async openConfigProfile(
    ...
    } else if (EXTENSION_NAME === "rica") {
      // RICA: short-circuit non-local "open profile" navigation
      // for the RICA build. Upstream Continue routes platform profiles to
      // `${env.APP_URL}${profileId}` → `https://continue.dev/<owner>/<pkg>`.
      // RICA's profile is `rica/dynamic-models`, served by Sang's backend
      // — there is no per-user editable view to open. Closes the funnel
      // for 6 callers (config webview, useEditBlock new-version link,
      // StreamError "View config", InlineErrorMessage banner, etc.).
      // Pair with the user-facing toast in InlineErrorMessage — the toast is
      // the primary signal; this short-circuit is defense-in-depth for any
      // caller that bypasses it.
      await this.ide.showToast(
        "info",
        "RICA models are managed centrally by Renesas — no local config to open.",
      );
    } else {
      const env = await getControlPlaneEnv(this.ide.getIdeSettings());
