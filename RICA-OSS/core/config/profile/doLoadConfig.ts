...
export default async function doLoadConfig(options: {
  ...
  // RICA: stash Sang's backend dynamic-config block on the
  // runtime ContinueConfig so multi-agent UI code (Tier-S lock,
  // SessionUsageBar, auto-switch-blocked-model) can read it directly —
  // same `config.entraIDDynamicConfig` channel RICA uses.
  //
  // Note: we do NOT re-run mergeEntraIDDynamicConfig against newConfig
  // because in this architecture the dynamic config IS the assistant —
  // RicaProfileLoader.loadDynamicAssistant already built newConfig from
  // the dynamic snapshot in RicaControlPlaneClient.buildDynamicAssistant.
  // Re-merging would be a redundant no-op for models / embeddings /
  // autocomplete, and ui / experimental / analytics / completionOptions /
  // requestOptions / docs / context already flowed through that path.
  // The merge helper remains exported for callers that need the RICA
  // semantic against a foreign config object.
  try {
    const cachedEntraIDDynamicConfig = (
      controlPlaneClient as unknown as {
        getCachedEntraIDDynamicConfig?: () => unknown;
      }
    ).getCachedEntraIDDynamicConfig?.();
    if (cachedEntraIDDynamicConfig) {
      newConfig.entraIDDynamicConfig =
        cachedEntraIDDynamicConfig as typeof newConfig.entraIDDynamicConfig;
    }
  } catch (e) {
    console.warn(
      "[RICA] Failed to stash EntraIDDynamicConfig on ContinueConfig:",
      e,
    );
  }
...
async function injectControlPlaneProxyInfo(
  ...
  // RICA: also rotate apiKey on rica-proxy models. The upstream predicate
  // only matched the continue-proxy literal, which severs the rotation pipe
  // for Rica chat models. Without this re-broadening to include rica-proxy,
  // model.apiKey freezes at construction-time MDP token and
  // 401s once the token's exp claim passes (~60-75 min). RicaProxy's setter
  // (rica/adapters/llm/RicaProxy.ts:22-24) does the right thing already —
  // just needs to be invoked. workOsAccessToken in this code path IS the
  // MDP token (controlPlaneClient.getAccessToken() returns sessionInfo
  // .accessToken which is set to mdpToken in EntraIDAuthProvider:534), so
  // the audience contract for Databricks proxy is preserved. Confirmed
  // by 5-lens auth audit Jun 12.
  const isProxyProvider = (p: string | undefined): boolean =>
    p === "continue-proxy" || p === "rica-proxy";
  Object.keys(config.modelsByRole).forEach((key) => {
    config.modelsByRole[key as ModelRole].forEach((model) => {
      if (isProxyProvider(model.providerName)) {
  ...
  // [RICA-DYNAMIC] diagnostic — confirms which trap fires.
  // Trap A: providerName==="continue-proxy" + apiBase contains login.microsoftonline.com.
  // Trap B: providerName==="openai" + apiBase missing or api.openai.com.
  // RICA: also dump contextLength + maxTokens from
  // the resolved model — needed to debug "Message exceeds context limit"
  // toast on small messages (compileChatMessages throws when
  // contextLength - safety - maxTokens - tools - system - history < 0).
  // Permanent (RICA diagnostics are kept by design); do not strip.
  try {
    config.modelsByRole.chat.forEach((model) => {
      const m = model as {
        apiBase?: string;
        contextLength?: number;
        completionOptions?: { maxTokens?: number; contextLength?: number };
      ...
      console.warn(
        ...
        JSON.stringify({
          ...
          _contextLength: m._contextLength,
          completionOptionsContextLength: m.completionOptions?.contextLength,
          maxTokens: m.completionOptions?.maxTokens,
        }),
      );
    });
  } catch {
    // diagnostic — never block config load on it
  }
