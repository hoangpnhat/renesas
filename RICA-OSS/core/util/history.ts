...
export class HistoryManager {
  ...
  save(session: Session) {
    ...
    if (session.sessionModelLock) {
      orderedSession.sessionModelLock = session.sessionModelLock;
    }
    ...
    // Persist the multi-agent tree so it survives reload / extension restart.
    // The GUI sends this in the save payload, but the disk serializer rebuilds
    // orderedSession field-by-field — without this copy the agent tree was
    // silently dropped at write time. Only write when non-empty to keep
    // chat/agent session files clean.
    if (
      session.subAgentProgress &&
      Object.keys(session.subAgentProgress).length > 0
    ) {
      orderedSession.subAgentProgress = session.subAgentProgress;
    }
