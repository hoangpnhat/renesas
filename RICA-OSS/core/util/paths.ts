...
const CONTINUE_GLOBAL_DIR = (() => {
  // RICA: route all global data (sessions, index, dev_data, logs, …) under the
  // RICA global dir. Prefer RICA_GLOBAL_DIR, keep CONTINUE_GLOBAL_DIR only as a
  // back-compat fallback for the test harness (jest/vitest set it to
  // .continue-test), and default to ~/.rica — never ~/.continue. This keeps
  // sessions in the same root as the debug log (~/.rica → .rica-debug under F5)
  // and removes the last continue.dev-named default path.
  const configPath =
    process.env.RICA_GLOBAL_DIR ?? process.env.CONTINUE_GLOBAL_DIR;
  if (configPath) {
