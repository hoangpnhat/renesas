...
import { analyzeTaskImpl } from "./implementations/analyzeTask";
import { spawnAgentImpl } from "./implementations/spawnAgent";
import { spawnAgentsImpl } from "./implementations/spawnAgents";
...
export async function callBuiltInTool(
  ...
  switch (functionName) {
    ...
    // RICA multi-agent: dispatch arms for the orchestrator's spawn tools.
    // Without these the orchestrator's tool calls fall through to the default
    // throw and multi-agent silently degrades to single-threaded execution.
    case BuiltInToolNames.SpawnAgent:
      return await spawnAgentImpl(args, extras);
    case BuiltInToolNames.SpawnAgents:
      return await spawnAgentsImpl(args, extras);
    case BuiltInToolNames.AnalyzeTask:
      return await analyzeTaskImpl(args, extras);
    default:
