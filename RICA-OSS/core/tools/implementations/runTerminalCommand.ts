...
/**
 * Build an environment for spawned processes that ensures node/npm are on PATH.
 * The VS Code extension host runs on Electron, so process.execPath points to Code.exe,
 * not node.exe. We probe common node locations and the user's USERPROFILE path.
 *
 * Required because RICA users frequently lack node on the system PATH; without
 * this, npm/node-based commands spawned via runTerminalCommand fail with ENOENT.
 */
function getEnrichedEnv(): NodeJS.ProcessEnv {
  ...
  if (process.platform === "win32") {
    ...
    const candidates = [
      path.join(userProfile, "nodejs"),
      path.join(userProfile, "AppData", "Roaming", "nvm"),
      "C:\\Program Files\\nodejs",
      "C:\\Program Files (x86)\\nodejs",
    ];

    // Also check subdirectories of userProfile/nodejs (e.g. node-v20.19.0-win-x64)
    try {
      const userNodeDir = path.join(userProfile, "nodejs");
      const fs = require("fs");
      if (fs.existsSync(userNodeDir)) {
        const entries = fs.readdirSync(userNodeDir);
        for (const entry of entries) {
          const full = path.join(userNodeDir, entry);
          if (fs.statSync(full).isDirectory() && entry.startsWith("node-")) {
    ...
    const candidates = [
      "/usr/local/bin",
      "/opt/homebrew/bin",
      path.join(env.HOME || "", ".nvm/current/bin"),
    ];
    for (const dir of candidates) {
      if (!currentPath.includes(dir)) {
        try {
  ...
  if (extraDirs.length > 0) {
    env.PATH = `${extraDirs.join(sep)}${sep}${currentPath}`;
  }
...
// Add color-supporting environment variables. Builds on top of getEnrichedEnv()
// so spawned shells also have node/npm on PATH.
const getColorEnv = () => ({
  ...getEnrichedEnv(),
  FORCE_COLOR: "1",
  COLORTERM: "truecolor",
