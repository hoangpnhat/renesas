...
// RICA Multi-Agent: in-process registries used by core handlers.
import { ContextBridge } from "./tools/multiagent/ContextBridge";
import { FileTracker } from "./tools/multiagent/FileTracker";
import { MultiAgentLogger } from "./tools/multiagent/MultiAgentLogger";
import { SubAgentRegistry } from "./tools/multiagent/SubAgentRegistry";
...
export class Core {
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("tools/call", async ({ data: { toolCall }, messageId }) =>
      this.handleToolCall(toolCall, messageId),
    );

    // RICA Multi-Agent: cancel one sub-agent by id.
    on("cancelSubAgent", async (msg) => {
      const { subAgentId } = msg.data;
      const success = SubAgentRegistry.getInstance().cancelAgent(subAgentId);
      console.log(
        `[Core] Cancel sub-agent ${subAgentId.slice(0, 8)}: ${success ? "ok" : "not found"}`,
      );
      return { success };
    });

    // RICA Multi-Agent: kill all sub-agents for a session and clean up state.
    on("killSession", async (msg) => {
      const { sessionId } = msg.data;
      const killedCount =
        SubAgentRegistry.getInstance().cancelAllForSession(sessionId);
      ContextBridge.destroySession(sessionId);
      FileTracker.destroySession(sessionId);
      // Finalize and write logger before cleanup so the log is saved.
      try {
      ...
      MultiAgentLogger.cleanup(sessionId);
      console.log(
        `[Core] Killed ${killedCount} agents for session ${sessionId.slice(0, 8)}, cleaned up ContextBridge/FileTracker/Logger`,
      );
      return { killedCount };
    });

    // RICA Multi-Agent: copy live/final session log into the workspace for
    // the user to download. Called from the gui ⬇ button (the inline button
    // is currently disabled but the handler is kept available).
    on(
      "multiagent/getSessionLog",
      async ({ data: { sessionId, workspacePath } }) => {
        const os = await import("os");
        const path = await import("path");
        const fs = await import("fs");
        try {
          const logDir = path.join(os.homedir(), ".rica-multiagent-logs");
          ...
          const files = fs
            .readdirSync(logDir)
            .filter(
              (f: string) => f.endsWith(".json") && f.includes(shortId),
            )
            .sort()
            .reverse();
          const finalFile = files.find((f: string) => !f.startsWith("live_"));
          const liveFile = files.find((f: string) => f.startsWith("live_"));
          const target = finalFile || liveFile;
          if (!target) {
            return { found: false };
          }
          const content = fs.readFileSync(path.join(logDir, target), "utf8");
          let baseDir = workspacePath || process.cwd();
          if (baseDir.startsWith("file://")) {
            baseDir = decodeURIComponent(new URL(baseDir).pathname);
            if (process.platform === "win32" && baseDir.startsWith("/")) {
              baseDir = baseDir.slice(1);
            }
          }
          const outDir = path.join(baseDir, "session_logs");
          if (!fs.existsSync(outDir)) {
            fs.mkdirSync(outDir, { recursive: true });
          }
          const outFile = path.join(outDir, `${shortId}.json`);
          fs.writeFileSync(outFile, content, "utf8");
          console.log(`[MultiAgent] Session log saved to: ${outFile}`);
          return { found: true, savedTo: outFile };
        } catch (e) {
          console.error(`[MultiAgent] Failed to save session log:`, e);
          return { found: false };
        }
      },
    );

    on("multiagent/logMainModelTurn", async ({ data }) => {
      const {
        sessionId,
        prompt,
        completion,
    ...
    on("multiagent/logToolResult", async ({ data }) => {
      const { sessionId, toolName, toolCallId, resultPreview, durationMs } =
        data;
      if (!sessionId) return;
      const logger = MultiAgentLogger.forSession(sessionId);
      logger.orchestratorToolResult(
        toolName,
        toolCallId,
        resultPreview,
        durationMs,
    ...
    on("multiagent/logError", async ({ data }) => {
      const { sessionId, errorType, message, durationMs } = data;
      if (!sessionId) return;
      const logger = MultiAgentLogger.forSession(sessionId);
      logger.orchestratorError(errorType, message, durationMs);
    });
  ...
  private async handleToolCall(toolCall: ToolCall, messageId?: string) {
    const { config } = await this.configHandler.loadConfig();
    if (!config) {
      throw new Error("Config not loaded");
    }
    ...
    // RICA Multi-Agent: tag this tool call with an abort controller so the
    // GUI can cancel it (and any spawned sub-agents) via the "abort" handler.
    const abortController = messageId
      ? this.addMessageAbortController(messageId)
      : new AbortController();

    // Capture the GUI session id for sub-agent isolation. Best-effort: if
    // the request fails, sub-agents simply run without per-session scoping.
    let sessionId: string | undefined;
    try {
      sessionId = await this.messenger.request(
        "getCurrentSessionId",
        undefined,
      );
    } catch {
    ...
    // RICA Multi-Agent: per-progress callback tagged with the active session
    // so the gui can filter events to its own MultiAgentPanel.
    const sendSubAgentProgress = (progress: any) => {
      this.messenger.send("subAgentProgress", { ...progress, sessionId });
    };
