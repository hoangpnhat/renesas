...
export type ToCoreFromIdeOrWebviewProtocol = {
  ...
  // RICA Multi-Agent: GUI → Core control + logging.
  // Cancel one sub-agent (pulls SubAgentRegistry by id and aborts).
  cancelSubAgent: [{ subAgentId: string }, { success: boolean }];
  // Kill all sub-agents for a session and finalize/clean up the logger.
  killSession: [{ sessionId: string }, { killedCount: number }];
  // GUI → Core session logging. Backend handler is in
  // core/tools/multiagent/MultiAgentLogger.ts, wired into the core router.
  "multiagent/getSessionLog": [
    { sessionId: string; workspacePath?: string },
    { found: boolean; savedTo?: string },
  ];
  "multiagent/logMainModelTurn": [
    {
      sessionId: string;
      prompt: string;
      completion: string;
