...
// ---- Multi-agent type surface (required by core/tools/multiagent/*) ----
...
export interface Session {
  ...
  usage?: SessionUsage;
  /**
   * Multi-agent: model committed for the lifetime of this session.
   * Set after the first successful LLM turn in multi-agent mode. Persists
   * across save/load so returning to the chat preserves the lock.
   */
  sessionModelLock?: { modelTitle: string; lockedAt: number };
  /** Multi-agent: live sub-agent status keyed by agentId, used by GUI tree view. */
...
export interface PromptLog {
  ...
  prompt: string;
  completion: string;
  /** Multi-agent: token counts captured from the LLM response, used for
   * the orchestrator's per-turn session log entry (see streamNormalInput
   * → multiagent/logMainModelTurn). Optional because not all providers
   * surface usage on streaming. */
  promptTokens?: number;
  completionTokens?: number;
}
...
export interface LLMOptions {
  model: string;
  model_id?: string; // Backend model ID for consumption tracking
  ...
  /** Multi-agent: per-model consumption snapshot from RICA backend. */
  consumptionLimit?: ConsumptionLimit;

  /** Multi-agent: whether this model is eligible for the Tier S picker. */
...
export interface ToolExtras {
  ...
  /** Multi-agent: cancellation signal propagated to spawned sub-agent runs. */
  abortSignal?: AbortSignal;
  /** Multi-agent: callback for sub-agent live progress updates streamed back to GUI. */
  sendSubAgentProgress?: (progress: SubAgentProgress) => void;
  /** Multi-agent: parent session id, threaded through to sub-agents for log scoping. */
  sessionId?: string;
}
...
export interface ModelCapability {
  ...
  nextEdit?: boolean;
  /**
   * RICA: backend-declared reasoning support. Drives the
   * lightbulb in InputToolbar via modelSupportsReasoning. When set,
   * takes precedence over the upstream model-name regex fallback.
   * Source: rica/adapters/config/modelMapping.ts (capabilities array
   * "reasoning"/"thinking" or capabilities object {reasoning: true}).
   */
  reasoning?: boolean;
  /**
   * Optional per-model cap for the reasoning budget. UI can clamp
   * options.reasoningBudgetTokens to this; RicaProxy passes through
   * to body.thinking.budget_tokens.
   */
...
export interface ModelDescription {
  ...
  /**
   * Multi-agent: per-model consumption gating snapshot (populated by the
   * RICA backend; absent for non-RICA models). Drives the SessionUsageBar,
   * the auto-switch path in ParallelListeners, and the submit-time refusal
   * in streamNormalInput.
   */
  consumptionLimit?: ConsumptionLimit;
...
/**
 * Backend-driven dynamic config block delivered by Sang's
 * /api/user-model/configs endpoint. Carried so every
 * field the backend can populate (embeddingsProvider, tabAutocompleteModel,
 * systemMessage, completionOptions, requestOptions, ui, experimental,
 * analytics, docs, context) flows through to the runtime ContinueConfig
 * via mergeEntraIDDynamicConfig. Multi-agent lock + Tier-S gating depend
 * on the per-model consumptionLimit + multiAgentEligible fields that ride
 * along on `models[]`.
 */
export interface EntraIDDynamicConfig {
  models?: any[];
...
export interface ContinueConfig {
  ...
  data?: DataDestination[];
  /** Raw dynamic config block from Sang's backend. Mirrors the RICA backend contract. */
  entraIDDynamicConfig?: EntraIDDynamicConfig;
}
