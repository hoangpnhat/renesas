...
function compileChatMessages({
  ...
  // [CONTEXT-DEBUG] RICA diagnostic — dumps every input that feeds the
  // inputTokensAvailable calculation BEFORE the throw, so we can tell why a
  // small user message hit the "Not enough context" toast. Permanent (RICA
  // diagnostics are kept by design); do not strip.
  try {
    console.warn(
      "[CONTEXT-DEBUG]",
      JSON.stringify({
        ...
        systemMsgTokens,
        lastMessagesTokens,
        toolSequenceLen: toolSequence.length,
        msgsCopyLen: msgsCopy.length,
        inputTokensAvailable,
        wouldThrow: knownContextLength !== undefined && inputTokensAvailable < 0,
      }),
    );
  } catch {
    // diagnostic — never block compileChat on it
  }
