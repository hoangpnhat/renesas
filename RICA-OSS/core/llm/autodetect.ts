...
function modelSupportsReasoning(
  ...
  // RICA: backend-declared capability is the source of truth.
  // RICA Dynamic config carries `capabilities.reasoning` per model
  // (rica/adapters/config/modelMapping.ts normalises both the array
  // form ["reasoning"/"thinking"] and the object form {reasoning: true}).
  // Backend can flip this on for new models without a client release.
  // The model-name regex below stays as a fallback so models that
  // pre-date the backend flag still surface the toggle.
  ...
  // GPT-5 / RICA Claude detection reverted alongside the body wire (commits
  // f548cef34 + 7d8d0cc88). Lightbulb stays visible for upstream's existing
  // claude/o1/o3/o4 patterns + the data-driven capability flag (kept above);
  // re-add when reasoning_content chunk decoding lands.
