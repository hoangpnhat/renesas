...
export abstract class BaseLLM implements ILLM {
  ...
  model: string;
  model_id?: string;
  // RICA: per-model consumption window + multi-agent eligibility, carried from
  // the model description so llmToSerializedModelDescription can forward them
  // to the GUI (rate-limit lock + Tier-S gate).
  ...
  constructor(_options: LLMOptions) {
    ...
    // RICA: carry consumption window + multi-agent eligibility from the
    // description onto the instance so the serializer forwards them to the GUI.
    this.consumptionLimit = options.consumptionLimit;
    this.multiAgentEligible = options.multiAgentEligible;
  ...
  private _logEnd(
    ...
    // Prefer real API-reported counts when available — these are what the
    // backend actually saw in tokens_in/tokens_out, so SQLite (which gets
    // POSTed via /api/user-model/configs) stays consistent with the badge
    // display and with backend's `consumed` math. Fall back to tiktoken
    // estimates only when the provider didn't report usage.
  ...
  */
    ...
    // RICA: populate token-usage fields on the streamChat PromptLog so
    // SessionTokenBadge can render. Refork's base return only had 4 fields
    // (modelTitle/modelProvider/prompt/completion); selectSessionTokens reads
    // 7 more. Prefer the streamed `usage` object (Anthropic/Databricks ship
    // a final usage chunk) → fall back to RicaProxy's `_lastApiUsage`
    // (non-streaming path or providers that report on body.usage) → fall
    // back to tiktoken estimate. Mirrors the RICA backend contract's logic with refork's
    // already-collected `usage` variable threaded in.
    const completionStr = completion.join("");
    const apiUsage = (this as any)._lastApiUsage as
      | {
          promptTokens?: number;
      ...
      | undefined;
    const hasApiUsage = !!(usage || apiUsage);
    const promptTokens =
      usage?.promptTokens ??
    ...
    const completionTokens =
      ...
      this.countTokens(completionStr);
    ...
    const cacheCreationTokens =
      (usage as any)?.cacheWriteInputTokens ??
      (usage as any)?.cacheCreationTokens ??
