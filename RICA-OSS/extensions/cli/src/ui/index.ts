...
export async function startTUIChat(
  ...
  // Immediately register the exit promise before any async work can trigger unmount
  ...
  // Keepalive: prevent event loop from draining before Ink's useInput activates stdin
  const keepalive = setInterval(() => {}, 60_000);

  // Block until the Ink app exits
  await exitPromise;
