...
// RICA overlay: branding strings (single import for all user-facing text)
...
export function enableSigintHandler() {
  // Remove only the default non-TUI handler; do NOT use removeAllListeners
  // because signal-exit (used by Ink) registers its own SIGINT listener
  // and removing it causes premature unmount.
  process.off("SIGINT", defaultSigintHandler);
...
const defaultSigintHandler = async () => {
...
program
  .name(cliStrings.binaryName)
  .description(cliStrings.description)
...
program
  .command("login")
  .description(`Authenticate with ${cliStrings.productName}`)
  .action(async () => {
    // Telemetry: record command invocation
...
program
  .command("logout")
  .description(`Log out from ${cliStrings.productName}`)
  .action(async () => {
    // Telemetry: record command invocation
...
export async function runCli(): Promise<void> {
  ...
  // Parse arguments and handle errors (parseAsync awaits async action handlers)
  try {
