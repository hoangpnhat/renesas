  ...
  implements vscode.WebviewViewProvider
{
  ...
  getSidebarContent(
    ...
    // F5 dev-host force-loads the static bundle from extensions/vscode/gui/
    // (Vite dev server at :5173 is unreachable from the webview iframe in this
    // tree — we already rebuild the bundle on every change, so HMR isn't worth
    // the wiring). Flip RICA_DEV_VITE=1 in launch.json env to opt back in.
    const inDevelopmentMode =
      context?.extensionMode === vscode.ExtensionMode.Development &&
      process.env.RICA_DEV_VITE === "1";
