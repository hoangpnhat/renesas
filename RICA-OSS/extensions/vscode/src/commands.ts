...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("ricaGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand(cmd("ricaGUIView.focus"));
...
const getCommandsMap: (
  ...
  return {
    ...
    "continue.defaultQuickAction": async (args: QuickEditShowParams) => {
      ...
      vscode.commands.executeCommand(cmd("focusEdit"), args);
    },
    ...
    "continue.viewHistory": () => {
      vscode.commands.executeCommand(cmd("navigateTo"), "/history", true);
    },
    ...
    "continue.openConfigPage": () => {
      vscode.commands.executeCommand(cmd("navigateTo"), "/config", false);
    },
    ...
    "continue.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand(cmd("focusContinueInput"));
          ...
          vscode.commands.executeCommand(cmd("navigateTo"), "/config");
        }
    ...
    "continue.openInNewWindow": async () => {
      ...
      vscode.commands.executeCommand(cmd("newSession"));
...
export function registerAllCommands(
  ...
  for (const [command, callback] of Object.entries(
    ...
    // Replace "continue." prefix with current extension name for RICA compatibility
    const actualCommand = command.replace(/^continue\./, `${EXTENSION_NAME}.`);
    context.subscriptions.push(
      vscode.commands.registerCommand(actualCommand, callback),
    );
  }
}
