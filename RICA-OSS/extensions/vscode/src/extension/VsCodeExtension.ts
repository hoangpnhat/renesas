...
import { EXTENSION_NAME, getControlPlaneEnv } from "../control-plane-env";
// Use RicaCore instead of upstream Core to inject RICA adapters
...
export class VsCodeExtension {
  ...
  private ricaAuthProvider: RicaAuthProvider;
  ...
  /**
   * Initialize authentication context on extension startup.
   * This ensures the signed-in state is properly restored when VS Code restarts.
   */
  private async initializeAuthContext() {
    try {
      // Check for existing sessions without forcing a refresh
      const sessionInfo = await getRicaControlPlaneSessionInfo(true, false);
      const isSignedIn = sessionInfo !== undefined;

      vscode.commands.executeCommand(
        "setContext",
        "rica.isSignedInToControlPlane",
        isSignedIn,
      );

      if (isSignedIn && this.core) {
        // Notify core about the session info if signed in
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.ricaAuthProvider = new RicaAuthProvider(context, this.uriHandler);

    void this.ricaAuthProvider.refreshSessions();
    context.subscriptions.push(this.ricaAuthProvider);
    ...
    new VsCodeMessenger(
      ...
      this.ricaAuthProvider,
      this.editDecorationManager,
      context,
      this,
    );

    // Use RicaCore to inject RICA control plane client and config handler
    ...
    // Initialize auth context to restore sign-in state
    void this.initializeAuthContext();
    ...
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider(
        ContinueConsoleWebviewViewProvider.viewType,
        this.consoleView,
      ),
    );
