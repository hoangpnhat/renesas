...
export class ContinueProxyApi extends OpenAIApi {
  ...
  constructor(config: z.infer<typeof ContinueProxyConfigSchema>) {
    // Phase 5.1.4: refuse to silently fall back to api.continue.dev. The
    // previous behaviour shipped a hardcoded `https://api.continue.dev/
    // model-proxy/v1/` whenever `env.proxyUrl` was unset — meaning a
    // single config drift (Sang backend forgets to send proxyUrl, Rica
    // env.ts hardcode regresses) would silently exfiltrate every prompt
    // to Continue's public proxy. Throw instead — defense-in-depth on
    // top of the Trap A scrub in 5.1.2 and the Trap B alias removal in
    // 5.1.3. See recon-supplement Bug 2 fix #3.
    if (!config.env?.proxyUrl) {
      throw new Error(
        "ContinueProxy: refusing to instantiate without env.proxyUrl. " +
          "RICA must route through its own proxy; api.continue.dev fallback is disabled.",
      );
    }
    ...
    const openaiConfig = {
      ...
      apiBase: new URL("model-proxy/v1/", config.env.proxyUrl).toString(),
