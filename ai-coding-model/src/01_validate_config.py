...
if os.path.exists(CHAIN_CODE_FILE):
    ...
    print(
        f"FAIL: Chain file `{CHAIN_CODE_FILE}` does not exist - make sure you have copied the chain file from the examples directory."
    )
...
def check_endpoint(endpoint_name):
    try:
        ...
        if llm_endpoint.state.ready != EndpointStateReady.READY:
            ...
            print(
                f"PASS: Model serving endpoint {endpoint_name} is online & ready.  Details at: https://{browser_url}/ml/endpoints/{endpoint_name}"
            )
