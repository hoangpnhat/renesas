...
mlflow.log_params(
    _flatten_nested_params({"destination_tables": destination_tables_config})
)
...
mlflow.log_input(
    mlflow.data.load_delta(
        table_name=destination_tables_config.get("raw_files_table_name")
    ),
    context="raw_files",
)
...
# MAGIC %md
# MAGIC ## Step 2: Parse the PDF files into text (extracting)
# MAGIC
...
df_raw_bronze_pd = raw_files_df.toPandas()
df_raw_bronze_pd["extract"] = df_raw_bronze_pd["content"].apply(
    lambda x: parse_bytes_adi(x)
)


# COMMAND ----------

df_raw_bronze_pd["doc_parsed_contents"] = df_raw_bronze_pd["extract"].apply(
    lambda x: x["doc_parsed_contents"]
)
silver_df = df_raw_bronze_pd[["path", "doc_parsed_contents"]]

# COMMAND ----------

# Write to Delta Table
spark.createDataFrame(silver_df).write.mode("overwrite").option(
    "overwriteSchema", "true"
).saveAsTable(destination_tables_config["parsed_docs_table_name"])

# reload to get correct lineage in UC
silver_df = spark.table(destination_tables_config["parsed_docs_table_name"])

# Display for debugging
print(f"Parsed {silver_df.count()} documents.")


tag_delta_table(
    destination_tables_config["parsed_docs_table_name"], data_pipeline_config
)
mlflow.log_input(
    mlflow.data.load_delta(
        table_name=destination_tables_config.get("parsed_docs_table_name")
    ),
    context="parsed_docs",
)
...
silver_df = silver_df.withColumn(
    "chunked",
    chunker_udf("doc_parsed_contents"),
)

# Check and warn on any errors
errors_df = silver_df.filter(silver_df.chunked.chunker_status != "SUCCESS")
...
gold_df = silver_df.filter(silver_df.chunked.chunker_status == "SUCCESS").select(
    "path",
...
print(f"Produced a total of {gold_df.count()} chunks.")

# Display without the parent document text - this is saved to the Delta Table
# display(gold_df.limit(2))

tag_delta_table(
    destination_tables_config["chunked_docs_table_name"], data_pipeline_config
)

mlflow.log_input(
    mlflow.data.load_delta(
        table_name=destination_tables_config.get("chunked_docs_table_name")
    ),
...
def find_index(endpoint_name, index_name):
    ...
    return destination_tables_config["vectorsearch_index_name"] in map(
        lambda i: i.get("name"), all_indexes
    )
...
if create_index:
    print(
        "Embedding docs & creating Vector Search Index, this can take 15 minutes or much longer if you have a larger number of documents."
    )
    print(
        f'Check status at: {get_table_url(destination_tables_config["vectorsearch_index_name"])}'
    )
...
tag_delta_table(
    destination_tables_config["vectorsearch_index_table_name"], data_pipeline_config
)
...
print(
    f'Vector index: {get_table_url(destination_tables_config["vectorsearch_index_name"])}'
)
print("\nOutput tables:\n")
print(
    f"Bronze Delta Table w/ raw files: {get_table_url(destination_tables_config['raw_files_table_name'])}"
)
print(
    f"Silver Delta Table w/ parsed files: {get_table_url(destination_tables_config['parsed_docs_table_name'])}"
)
print(
    f"Gold Delta Table w/ chunked files: {get_table_url(destination_tables_config['chunked_docs_table_name'])}"
)
...
index.similarity_search(
    columns=["chunked_text", "chunk_id", "path"], query_text="what's read_adc_digitizer"
)
